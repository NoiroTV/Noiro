...
Noiro source code is licensed under **GPL-3.0-or-later** (see [LICENSE](LICENSE)).
The Noiro name and logo are separate from the code licence and are covered by the
...
`Noiro.xcodeproj` is generated from it and is gitignored, so never edit the
