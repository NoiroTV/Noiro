...
struct PlayerScreen: View {
    ...
    @AppStorage("noiro.videoSize") private var videoSize = "original"   // whole frame, correct aspect
    @AppStorage("noiro.seekStep") private var seekStep = "10"            // skip-button step in seconds ("10"/"15"/"30")
    ...
    @AppStorage("noiro.playerMuted") private var playerMuted = false
    ...
    #if !os(tvOS)
    // Skip-segment editor state (iOS/Mac only). Submits keyless to vortexo.app/api/noiro/v1/edge/skip; also to skipdb.tv
    ...
    private func grabFrame() {
        coordinator.player?.captureFrameJPEGData(maxWidth: 2560) { data in
            ...
            let raw = recordMeta?.name ?? "Noiro"
            let base = raw.components(separatedBy: CharacterSet(charactersIn: "/:\\?%*|\"<>")).joined()
            let name = "Noiro-\(base.isEmpty ? "frame" : base)-\(Int(Date().timeIntervalSince1970)).jpg"
    ...
    private var topBar: some View {
        HStack(spacing: 12) {
            ...
            // vortexo.app/api/noiro/v1/edge/skip worker, so no third-party key is required to open or use it.
            if let m = curMeta,
    ...
    /// Submit the edited segment. Always goes keyless to vortexo.app/api/noiro/v1/edge/skip; also to skipdb.tv when the
    ...
    private func autoSelectAddonSubtitleIfNeeded() {
        ...
        if PlaybackSettings.isSubtitleTranslationEnabled, ApiKeys.shared.hasGemini, !subtitleTracks.isEmpty {
    ...
    private func rows(for p: Panel) -> [Row] {
        ...
        case .subtitleSettings:
            ...
            rs.append(Row(label: String(localized: "Position"), isHeader: true))
            rs.append(Row(label: String(localized: "Move up"), detail: position) { adjustSubtitleVerticalOffset(16) })
            rs.append(Row(label: String(localized: "Move down"), detail: position) { adjustSubtitleVerticalOffset(-16) })
            if subtitleVerticalOffset != 0 {
                rs.append(Row(label: String(localized: "Reset position")) { adjustSubtitleVerticalOffset(-subtitleVerticalOffset) })
            }
        ...
        case .playerSettings:
            ...
            // hunting the top-bar icon. Any tt####### title qualifies (keyless submit via vortexo.app/api/noiro/v1/edge/skip).
            if let m = curMeta,
               m.libraryId.range(of: #"^tt\d{7,8}$"#, options: .regularExpression) != nil {
    ...
    private func adjustSubtitleVerticalOffset(_ delta: Double) {
        ...
        if panel == .subtitleSettings { panelRows = rows(for: .subtitleSettings) }
    }
    ...
    private func autoSelectTracks() {
        ...
        let subtitlePick = PlaybackSettings.isSubtitleTranslationEnabled && ApiKeys.shared.hasGemini
            ...
            : pick.subtitle
        if let s = subtitlePick { coordinator.player?.setSubtitleTrack(s) }   // -1 = off
        VXProbe.log("subs", "auto-select sub=\(subtitlePick.map(String.init) ?? "none") audio=\(pick.audio.map(String.init) ?? "none")")
