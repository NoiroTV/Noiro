...
enum NodeServer {
    ...
    private static func runNode(_ scriptPath: String) {
        ...
        if getrlimit(RLIMIT_NOFILE, &lim) == 0 {
            ...
            if lim.rlim_cur < want {
                lim.rlim_cur = want
                if setrlimit(RLIMIT_NOFILE, &lim) == 0 { NSLog("Noiro: RLIMIT_NOFILE raised to \(want)") }
            }
        }
        ...
        NSLog("Noiro: starting node streaming server (HOME=\(caches), log=\(logPath))")
        ...
        exitCode = rc
        NSLog("Noiro: node server exited rc=\(rc)")
    }
    ...
    private static func waitForPortFree(_ port: UInt16, timeout: TimeInterval) {
        ...
        while Date() < deadline {
            ...
            NSLog("Noiro: 11470 still held by a dying instance, waiting")
            Thread.sleep(forTimeInterval: 0.25)
        }
        NSLog("Noiro: 11470 not freed within \(timeout)s; node may fall back to 11471+ (latch will follow it)")
    }
