...
/// fragmented MP4, writing the muxed bytes to an in-memory `NoiroRemuxBuffer` via a CUSTOM AVIO write callback
/// (no file, no disk). `NoiroRemuxHLSServer` serves that buffer to AVPlayer as local HLS (the b166 default
/// delivery; the legacy `NoiroRemuxResourceLoader` progressive path stays compiled for rollback), so AVPlayer
...
///     Vorbis, PCM variants), that ONE track is TRANSCODED in-flight by `NoiroAudioTranscoder` (EAC3-first,
...
final class NoiroMKVRemuxStream: @unchecked Sendable {

    let buffer = NoiroRemuxBuffer()
    ...
    /// produced bytes in place (NoiroRemuxBuffer.overwrite) instead of failing EPIPE and leaving a size-0 moov.
    ...
    /// the noiroremux:// asset as a regular file, cannot index an open-ended fMP4, and either fails the item
    ...
    /// cut by this class at keyframe boundaries), which `NoiroRemuxHLSServer` serves to AVPlayer as local
    /// HLS from 127.0.0.1. When false (the legacy noiroremux:// loader path, kept for rollback) none of the
    ...
    private func run() {
        ...
        let avio = avio_alloc_context(
            ...
            { (opaque, buf, size) -> Int32 in
                ...
                let me = Unmanaged<NoiroMKVRemuxStream>.fromOpaque(opaque).takeUnretainedValue()
        ...
        var transcoder: NoiroAudioTranscoder? = nil
        for i in 0..<nb where mappable.contains(i) {
            ...
            if transcodeActive, i == transcodeAudioIn {
                ...
                guard let par,
                      let t = NoiroAudioTranscoder(sourcePar: par, outStream: outStream,
    ...
    private static func hevcCodecString(_ par: UnsafeMutablePointer<AVCodecParameters>?) -> String? {
        ...
        // NoiroRemuxHLSServer.serveMaster (b170), which always survives the filter. Forced-L is retained only
    ...
    /// `NoiroAudioTranscoder`; only a source with no decodable and no transcodable audio fails to libmpv.
