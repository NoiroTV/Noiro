...
// Keep Metal API validation enabled in Debug builds. The layer lifecycle below sizes the drawable
// before MoltenVK starts and tears down the active video output before any later surface resize.
...
final class MPVMetalViewController: PlatformViewController {
    ...
    private lazy var captureQueue = DispatchQueue(label: "com.elvissalihovic.noiro.trickplay.capture", qos: .utility)
    ...
    private let mpvLog = Logger(subsystem: "com.elvissalihovic.noiro.app", category: "mpv")
    ...
    override func viewDidLoad() {
        ...
        #if canImport(UIKit)
        // `UIScreen.main` is deprecated: prefer the screen behind this view's own scene, and fall back
        // to the trait-collection scale (which is what the deprecation note points at) while the view
        // is not yet in a window, as it is here in viewDidLoad.
    ...
    /// `UIViewControllerRepresentable` loads a controller before SwiftUI assigns its final container bounds.
    /// Starting MoltenVK in `viewDidLoad` therefore creates a full-screen 3840x2160 swapchain for an ambient
    /// hero player that is about to become a 1280x720-point panel. The first layout then hands that still-live
    /// 4K render pass a 2560x1440 drawable and Metal validation aborts. Start only after a valid layout has
    /// sized the layer, and never resurrect a controller SwiftUI has already dismantled.
    ...
    private func layoutDrawable() {
        ...
        let targetDrawableSize = CGSize(width: size.width * metalLayer.contentsScale,
                                        height: size.height * metalLayer.contentsScale)
        let drawableNeedsResize = Int(metalLayer.drawableSize.width.rounded()) != Int(targetDrawableSize.width.rounded())
            || Int(metalLayer.drawableSize.height.rounded()) != Int(targetDrawableSize.height.rounded())

        // Tear the existing VO down BEFORE changing CAMetalLayer.drawableSize. Doing this after the layer
        // mutation leaves MoltenVK's old renderTargetWidth/Height live long enough to acquire a smaller new
        // attachment, which is the 3840x2160 <= 2560x1440 validation crash seen on physical Apple TV.
        let shouldResumeVideoOutput = drawableNeedsResize && mpv != nil
        if shouldResumeVideoOutput {
        ...
        if !didSetupMpv, !isStopped {
            didSetupMpv = true
            setupMpv()
            if let url = playUrl {
                loadFile(url, headers: playHeaders, live: playUrlLive, audioSidecar: playAudioSidecarURL)
            }
        } else if shouldResumeVideoOutput {
            resumeVideoOutputAfterDrawableResize()
        }
    }

    private func resumeVideoOutputAfterDrawableResize() {
        guard mpv != nil else { return }
        // Runtime rebuild after a live resize/rotation: `vid` must be set as a PROPERTY. The old VO was
        // synchronously disabled before the drawable changed above; resume on the next run-loop turn so
        // Core Animation has committed the new attachment size before MoltenVK creates its replacement swapchain.
    ...
    func setupMpv() {
...
#endif
        // mpv reads an int64 holding the layer pointer. Taking `&metalLayer` handed it the address of
        // the variable *storing* the reference and relied on that slot's bit pattern being the object
        // pointer; passing the pointer value explicitly says the same thing without the punning.
        ...
        if !startMuted {
            ...
            // Keep steady film playback locked to the Apple TV display clock. A typical 23.976 fps
            // stream on the 24 Hz output otherwise accumulates a small timing error and periodically
            // repeats/drops a frame, perceived as continuous judder even though VideoToolbox decoding
            // is keeping up. display-resample absorbs that sub-percent mismatch in the decoded audio;
            // custom mpv options are applied later and can still override this diagnostic-safe default.
...
//        checkError(mpv_set_option_string(mpv, "profile", "fast"))   // can fix frame drop in poor device when play 4k
        ...
        // Power-user custom mpv options. Applied LAST, after every Noiro baseline option above, so an
        ...
        // The current subtitle cue's plain text — fed to the embedded-subtitle Gemini translation
        // (sub-text fires on every cue boundary). Observe as STRING so empty/unset is handled cleanly.
    ...
    func loadFile(
        ...
        let isLocalStream = playURL.host == "127.0.0.1" || playURL.host == "localhost"
            ...
            // already proxied to 127.0.0.1 (small), but a worker-fallback trailer (vortexo.app/api/noiro/v1/edge/trailer, a remote
            ...
            || (playURL.host?.contains("vortexo.app/api/noiro/v1/edge/trailer") ?? false)
    ...
    private func updateCapturePipeline() {
        ...
        guard size.width > 1, size.height > 1 else { return }
        #if os(tvOS)
        // A trickplay grab from a 4K Apple TV surface copies the entire 10-bit drawable and asks Core Image
        // to convert it while gpu-next is presenting the next frame. On A15 this repeatedly produced gain-map
        // conversion failures and visible playback hitches. Community/server thumbnails remain available;
        // skip only opportunistic local capture when the live drawable exceeds 1080p.
        guard size.width * size.height <= 1920 * 1080 else {
            metalLayer.updateCaptureTexture(nil)
            capturePipelineSize = .zero
            capturePipelineFormat = .invalid
            return
        }
        #endif
    ...
    private func syncDisplayDynamicRange(sigPeak: Double) {
        ...
        if range != .sdr {
            let mode = UserDefaults.standard.string(forKey: "noiro.hdrToneMapMode")
                ?? (UserDefaults.standard.bool(forKey: "noiro.forceSDRTonemap") ? "on" : "auto")
            let forceSDR: Bool
            switch mode {
    ...
    func addExternalSubtitle(url: String, title: String, lang: String,
        ...
        // Re-check an async auto-selection on the main thread immediately before it reaches mpv. This lets
        // a newer manual subtitle choice cancel a slow add-on download instead of the late completion
        // selecting itself and forcing another demux refresh-seek under active playback.
        let loadLocal: (URL) -> Void = { [weak self] localFile in
            DispatchQueue.main.async {
                guard shouldLoad?() ?? true else { completion?(false); return }
                guard let self else { completion?(false); return }
                self.queue.async {
                    self.command("sub-add", args: [localFile.path, "select", title, lang])
                    finish(true)
                }
            }
        }
        ...
        Self.downloadSubtitle(remote, timeout: timeout, retriesLeft: Self.subtitleDownloadRetries) { localFile in
            guard let localFile else { finish(false); return }
            // Load the original immediately. The shared `sub-text` overlay translates the current cue for
            // embedded AND external mpv tracks, so pre-translating the entire file here duplicated Gemini
            // work, delayed selection, and could translate already-translated text a second time.
            loadLocal(localFile)
        }
    }
    ...
    func setSubVisibility(_ visible: Bool) { setString("sub-visibility", visible ? "yes" : "no") }
    func setSubtitleVerticalOffset(_ points: Double) {
        let range = PlaybackSettings.subtitleVerticalOffsetRange
        let clamped = min(max(points, range.lowerBound), range.upperBound)
        // `sub-margin-y` only raises text. For a downward adjustment use mpv's signed-position
        // equivalent: values above 100 move subtitles below their original baseline.
        setString("sub-margin-y", String(Int(max(0, clamped).rounded())))
        if clamped < 0 {
            let canvasHeight = max(Double(view.bounds.height), 720)
            let position = min(150, 100 + abs(clamped) / canvasHeight * 100)
            setString("sub-pos", String(format: "%.2f", position))
        } else {
            setString("sub-pos", "100")
        }
    }
    ...
    private(set) var videoSizeMode = UserDefaults.standard.string(forKey: "noiro.videoSize") ?? MPVMetalViewController.defaultVideoSizeMode
    ...
    func applySubtitleStyle() {
        ...
        setSubtitleVerticalOffset(PlaybackSettings.subtitleVerticalOffset)
    }
    ...
    func captureFrameJPEGData(maxWidth: CGFloat, completion: @escaping (Data?) -> Void) {
        ...
        guard drawableSize.width * drawableSize.height <= 1920 * 1080 else {
            completion(nil)
            return
        }
        #endif
            ...
            while true {
                ...
                case MPV_EVENT_PROPERTY_CHANGE:
                    ...
                    if let property = UnsafePointer<mpv_event_property>(dataOpaquePtr)?.pointee {
                        ...
                        case "sub-text":
                            // The current embedded subtitle cue's plain text — read via getString (same as
                            // the gamma observer, which delegates rather than parsing property.data directly).
                            let text = self.getString("sub-text") ?? ""
                            self.emit("sub-text", text)
                        default: break
                        }
                    }
