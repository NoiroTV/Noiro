...
/// thread (`NoiroMKVRemuxStream`) appends muxed fragmented-MP4 bytes as they are produced; the local HLS
/// server (`NoiroRemuxHLSServer`, the default delivery) reads closed-segment byte ranges out of it, and the
/// legacy progressive loader (`NoiroRemuxResourceLoader`, the rollback path) reads it sequentially.
///
/// Design notes:
...
/// about in isolation.
final class NoiroRemuxBuffer: @unchecked Sendable {
    ...
    /// NoiroMKVRemuxStream.hlsMaxSegmentBytes (2 x 32 MiB = 64). Keep in lockstep with that constant. This is the
    ...
    /// window.
    private let windowFloorBytes: Int = max(NoiroRemuxBuffer.windowFloorMinMiB, RemoteConfig.snapshot.dvRemuxWindowMiB) * 1024 * 1024
