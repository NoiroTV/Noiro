...
///
/// #76 Phase 1: on iOS the surface is an `AVPlayer` + `AVPlayerLayer` (NOT `AVPlayerViewController`) so Noiro
...
/// this phase delivers the Noiro chrome + the existing ABR / resume / progress / PiP behaviour, nothing more.
struct HLSPlayerView: View {
    ...
    // the Noiro chrome rebuild is iOS/Mac-only in this phase.
    var body: some View {
        ZStack {
...
#if os(iOS)
// MARK: - iOS: AVPlayerLayer host + Noiro chrome overlay

extension HLSPlayerView {
    /// The whole iOS player: the AVPlayerLayer-backed video surface with the Noiro controls overlay on top.
    fileprivate struct ChromeBody: View {
        ...
        @AppStorage("noiro.seekStep") private var seekStep = "10"
...
/// Hosts an `AVPlayerLayer` in a plain UIView so Noiro owns the video surface (no Apple chrome). The layer is
