...
protocol PlayerEngine: AnyObject {
    ...
    /// Show/hide mpv's native subtitle rendering (used to suppress the embedded sub while a Gemini
    /// translation overlay is showing). No-op on AVPlayer (its embedded subs render natively and this
    /// path isn't used for translation on that engine).
...
extension PlayerEngine {
    ...
    func addExternalSubtitle(url: String, title: String, lang: String) {
        addExternalSubtitle(url: url, title: title, lang: lang, timeout: 20, shouldLoad: nil, completion: nil)
    }
    ...
    func addExternalSubtitle(url: String, title: String, lang: String, completion: ((Bool) -> Void)?) {
        addExternalSubtitle(url: url, title: title, lang: lang, timeout: 20, shouldLoad: nil, completion: completion)
    }

    /// Auto-selection form: re-check the request immediately before installing a downloaded track.
    func addExternalSubtitle(url: String, title: String, lang: String,
                             shouldLoad: (() -> Bool)?, completion: ((Bool) -> Void)?) {
        addExternalSubtitle(url: url, title: title, lang: lang, timeout: 20,
                            shouldLoad: shouldLoad, completion: completion)
    }
    ...
    /// Engines without a custom subtitle overlay can safely ignore vertical placement.
    /// The two playback engines used by Noiro override this with their live renderer setting.
    func setSubtitleVerticalOffset(_ points: Double) {}
