...
/// Serves the DV-for-MKV streaming remux (`NoiroMKVRemuxStream`) to AVPlayer as a `noiroremux://` custom-scheme
/// asset. AVPlayer refuses to load a custom scheme itself, so it hands every content-information + data request
/// to this `AVAssetResourceLoaderDelegate`, which answers them out of the growing `NoiroRemuxBuffer`:
...
///
/// LEGACY delivery (b166): the default DV-remux delivery is now local HLS (`NoiroRemuxHLSServer`), the one
/// way AVFoundation reliably consumes a growing fMP4. This progressive loader stays compiled as the instant
/// rollback path behind `NoiroRemuxHLSServer.deliveryEnabled`.
final class NoiroRemuxResourceLoader: NSObject, AVAssetResourceLoaderDelegate {

    static let scheme = "noiroremux"
    ...
    private let stream: NoiroMKVRemuxStream
    private let queue = DispatchQueue(label: "noiro.dvremux.loader")
    ...
    /// first request) and mounts an `AVURLAsset(url:)` whose scheme is `noiroremux`.
    init(stream: NoiroMKVRemuxStream) {
        self.stream = stream
        super.init()
    }

    /// Convenience: wrap a debrid URL + headers into a stream + loader + the `noiroremux://` asset URL.
    /// Returns nil if the URL can't be rewritten to the custom scheme.
    static func make(input: URL, headers: [String: String]?) -> (loader: NoiroRemuxResourceLoader, assetURL: URL)? {
        ...
        let stream = NoiroMKVRemuxStream(input: input.absoluteString, headers: headers)
        return (NoiroRemuxResourceLoader(stream: stream), assetURL)
    }
    ...
    private func serve(_ request: AVAssetResourceLoadingRequest) {
        ...
        while remaining > 0 {
            if request.isCancelled || isInvalidated {
                request.finishLoading(with: NSError(domain: "noiro.dvremux", code: -999,
            ...
            if let failure = result.failure {
                request.finishLoading(with: NSError(domain: "noiro.dvremux", code: -1,
