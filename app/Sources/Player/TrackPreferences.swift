...
enum PlaybackSettings {
    enum Key {
        static let directLinksOnly = "noiro.directLinksOnly"
        static let keepPlayingInBackground = "noiro.keepPlayingInBackground"
        static let customMpvOptions = "noiro.customMpvOptions"
        static let videoUpscaling = "noiro.videoUpscaling"
        static let subtitleVerticalOffset = "noiro.subtitleVerticalOffset"
    }

    // Positive lifts subtitles above the normal baseline; negative lowers them. The shared range is
    // used by native mpv subtitles and every app-rendered subtitle overlay.
    static let subtitleVerticalOffsetRange: ClosedRange<Double> = -100...240

    /// Signed distance in points from the normal subtitle baseline (positive = up, negative = down).
    static var subtitleVerticalOffset: Double {
        get {
            let value = UserDefaults.standard.object(forKey: Key.subtitleVerticalOffset) as? Double ?? 0
            return min(max(value, subtitleVerticalOffsetRange.lowerBound), subtitleVerticalOffsetRange.upperBound)
        }
        set {
            let clamped = min(max(newValue, subtitleVerticalOffsetRange.lowerBound), subtitleVerticalOffsetRange.upperBound)
            UserDefaults.standard.set(clamped, forKey: Key.subtitleVerticalOffset)
        }
    }
    ...
    /// Applied verbatim during player setup after Noiro's own baseline options, so an advanced viewer
...
/// still wins). `.standard` is intentionally a no-op: it keeps Noiro's existing default, which is already
...
enum VideoUpscaling: String, CaseIterable {
    ...
    case standard      // Noiro default: libplacebo's sharp lanczos + debanding (current behavior)
    ...
    /// empty list so Noiro's existing baseline is left untouched. `.anime4k` returns ONLY the scaler
...
struct TrackPreferences: Equatable {
    ...
    enum Key {
        static let audio = "noiro.tracks.audioLangs"
        static let subtitle = "noiro.tracks.subLangs"
        static let forced = "noiro.tracks.forced"
        static let reject = "noiro.tracks.reject"
    }
    ...
    static let commonLanguages: [(id: String, label: String)] = [
        ...
        ("hr", "Croatian"), ("sr", "Serbian"), ("bs", "Bosnian"),
    ]
