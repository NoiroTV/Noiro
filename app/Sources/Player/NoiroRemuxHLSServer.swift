...
/// Serves the DV-for-MKV streaming remux (`NoiroMKVRemuxStream`) to AVPlayer as LOCAL HLS from 127.0.0.1
/// (b166). AVFoundation does not support a growing fragmented MP4 as a plain progressive asset: on the old
/// `noiroremux://` delivery every DV play on device either failed "Cannot Open" or scanned hundreds of MB
...
final class NoiroRemuxHLSServer: @unchecked Sendable {
    ...
    /// explicit UserDefaults value wins (instant local rollback to the legacy `noiroremux://` loader path,
    /// which stays compiled), else the RemoteConfig `dvRemuxHLS` feature acts as a fleet kill-switch.
    static let deliveryKey = "noiro.dvRemuxHLS"
    ...
    private let queue = DispatchQueue(label: "noiro.dvremux.hls")
    ...
    private let serveQueue = DispatchQueue(label: "noiro.dvremux.hls.serve", attributes: .concurrent)
    ...
    static func make(input: URL, headers: [String: String]?) -> (server: NoiroRemuxHLSServer, playlistURL: URL)? {
        let stream = NoiroMKVRemuxStream(input: input.absoluteString, headers: headers, indexForHLS: true)
        let server = NoiroRemuxHLSServer(stream: stream)
    ...
    private init(stream: NoiroMKVRemuxStream) {
        self.stream = stream
    }
    ...
    private func serveMaster(_ connection: NWConnection) {
        ...
        // Noiro forces the Apple TV panel itself via HDRDisplayMode. BANDWIDTH-1 keeps the DV variant
    ...
    private func serveMedia(_ connection: NWConnection) {
        struct Ready { let segments: [NoiroMKVRemuxStream.HLSSegment]; let ended: Bool }
        ...
        var lines = [
            "#EXTM3U",
            "#EXT-X-VERSION:7",
            "#EXT-X-TARGETDURATION:\(NoiroMKVRemuxStream.hlsTargetDuration)",
