...
/// picks the cue that should be on screen. This is how Noiro renders EXTERNAL (add-on / community-pooled) srt/vtt
...
final class SubtitleCueRenderer {
    ...
    /// Gemini translation overlay: maps a cue's start time (seconds, unique per cue) → translated text.
    /// `activeText` prefers the translation when present, so translations fill in progressively without
    /// rebuilding the cue array. Cleared on `load(cues:)`/`clear()`.
    private var translatedByText: [TimeInterval: String] = [:]

    /// Replace the loaded cues (e.g. after parsing a freshly downloaded file). Resets the search cursor
    /// AND any pending translation overlay (the new track starts untranslated).
    ...
    /// Apply a batch of translations (start time → translated text), merged onto whatever is already there.
    /// Called by `SubtitleTranslator` after each Gemini chunk so the overlay refreshes progressively.
    func applyTranslations(_ byStartTime: [TimeInterval: String]) {
        for (start, text) in byStartTime where !text.isEmpty {
            translatedByText[start] = text
        }
    }
    ...
    func activeText(atClock clock: TimeInterval) -> String? {
        ...
        while i < cues.count {
            ...
            if t <= cue.end {
                // Prefer the Gemini translation when one has arrived for this cue; fall back to the
                // original text while a translation is still in flight (so the subtitle never blanks).
                if let translated = translatedByText[cue.start],
                   !SubtitleTranslationPresentationPolicy.displayedText(originalText: cue.text, translatedText: translated).isEmpty {
                    return SubtitleTranslationPresentationPolicy.displayedText(originalText: cue.text, translatedText: translated)
                }
                return cue.text
            }
            i += 1
...
final class SubtitleOverlayView: UIView {
    ...
    /// User-controlled lift above the normal picture-relative subtitle baseline.
    ...
    func setVerticalOffset(_ points: Double) {
        ...
        guard abs(value - verticalOffset) > 0.5 else { return }
        verticalOffset = value
        updateBottomConstraint()
    }

    private func updateBottomConstraint() {
        labelBottom.constant = -(bottomInset + letterboxBottom + verticalOffset)
    }
    ...
    func applyStyle() {
        ...
        setVerticalOffset(PlaybackSettings.subtitleVerticalOffset)
    }
...
final class SubtitleOverlayView: NSView {
    ...
    private func updateBottomConstraint() {
        labelBottom.constant = -(Self.baseInset + letterboxBottom + verticalOffset)
    }
