...
enum PlayerEngineRouter {
    enum Engine: String { case mpv, avfoundation, ksplayer }
    ...
    static let overrideKey = "noiro.playerEngine"
    ...
    static let dvRemuxKey = "noiro.dvRemux"
    ...
    static func engine(for url: URL,
        ...
        // and mounts an in-process MKV -> fMP4 streaming remux behind a `noiroremux://` resource loader, so
        ...
        // (5) On Apple TV hardware, direct/debrid media uses KSPlayer's FFmpeg demuxer, VideoToolbox
        // decoder and direct Metal presentation path. This avoids libmpv's Vulkan -> MoltenVK shader
        // translation during playback while retaining libmpv as the fallback for torrents/loopback URLs.
        // The Simulator mirrors Vortexo and remains AVPlayer/libmpv-only.
        #if os(tvOS) && targetEnvironment(simulator)
        // Vortexo is AVPlayer-only in Simulator. Unsupported containers fail back to libmpv through the
        // existing in-place demotion, while normal MP4/direct HTTP playback gets Apple's native timing.
        return .avfoundation
        #elseif os(tvOS)
        return .ksplayer
        #else
