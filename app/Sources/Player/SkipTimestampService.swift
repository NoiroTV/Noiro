...
enum SkipTimestampService {
    ...
    static let providerKey = "noiro.skipProvider"

    private static let log = Logger(subsystem: "com.elvissalihovic.noiro.app", category: "skiptimes")

    private static var provider: String {
        UserDefaults.standard.string(forKey: providerKey) ?? "both"
    }

    /// All skip candidates for a title, merging our own vortexo.app/api/noiro/v1/edge/skip worker (primary, keyless) with
    /// the chosen third-party crowd source(s), the user's optional custom SkipDB-compatible provider,
    /// and AniSkip (anime). The Noiro worker already aggregates SkipDB plus capture-on-miss server-side,
    ...
    static func candidates(imdbId: String, season: Int?, episode: Int?,
        ...
        async let noiro   = noiroSkip(imdbId: imdbId, season: season, episode: episode, durationSeconds: durationSeconds)
        ...
        return (await noiro) + crowd + (await custom) + (await aniskip)
    }

    /// Cache key for the Noiro worker, bucketed by runtime like the other providers so a different
    /// rip of the same title re-derives clamped segments instead of reusing one file's duration.
    static func noiroCacheKey(imdbId: String, season: Int?, episode: Int?, durationSeconds: Double) -> String {
        let durationBucket = Int(durationSeconds / 10) * 10
        return "noiro:\(imdbId):\(season ?? 0):\(episode ?? 0):\(durationBucket)"
    }

    /// Layer 2 (primary): our self-hosted, keyless vortexo.app/api/noiro/v1/edge/skip worker. It aggregates SkipDB plus a
    ...
    private static func noiroSkip(imdbId: String, season: Int?, episode: Int?,
                                  durationSeconds: Double) async -> [SegmentCandidate] {
        guard imdbId.range(of: #"^tt\d{7,8}$"#, options: .regularExpression) != nil else { return [] }
        let key = noiroCacheKey(imdbId: imdbId, season: season, episode: episode, durationSeconds: durationSeconds)
        ...
        guard var components = URLComponents(string: "https://vortexo.app/api/noiro/v1/edge/skip/skip") else { return [] }
        components.queryItems = [URLQueryItem(name: "key", value: lookup)]
        guard let url = components.url else { return [] }

        var request = URLRequest(url: url)
        request.timeoutInterval = 5
        NoiroEdgeAuth.sign(&request)   // gated host (vortexo.app/api/noiro/v1/edge/skip /skip read): stamp X-Noiro-Ts / X-Noiro-Sig
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
                log.info("\(key, privacy: .public): Noiro skip non-200")
                return []
            }
            let media = try JSONDecoder().decode(NoiroSkipResponse.self, from: data)
            log.info("\(key, privacy: .public): \(media.spans.count, privacy: .public) spans from Noiro")
            ...
            log.info("\(key, privacy: .public): Noiro skip failed, \(String(describing: error), privacy: .public)")
            return []
        }
    }
    ...
    /// Noiro at). Reuses the exact SkipDB read path + decoder, just parameterized by base URL and key,
    ...
    /// NOT Noiro edge-signed: third-party host.
    ...
    private static func skipDBCompatible(base: String, apiKey: String?, cachePrefix: String, label: String,
        ...
        // No NoiroEdgeAuth.sign here: skipdb.tv and the custom mirror are third-party hosts.
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let http = response as? HTTPURLResponse else { return [] }
    ...
    /// vortexo.app/api/noiro/v1/edge/skip `/skip` shape: `{ "segments": { "<type>": { "start_ms", "end_ms", "votes",
    ...
    private struct NoiroSkipResponse: Decodable {
        struct Span: Decodable {
            let start_ms: Int?
            let end_ms: Int?
        }
