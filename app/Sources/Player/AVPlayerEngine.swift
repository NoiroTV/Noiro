...
/// engine as `coordinator.player`, an `any PlayerEngine`). This is the engine Noiro routes Dolby Vision and
...
final class AVPlayerEngineController: NSObject, PlayerEngine {
    ...
    private(set) var videoSizeMode = UserDefaults.standard.string(forKey: "noiro.videoSize") ?? "original"
    ...
    // time-shift an external SRT, so Noiro owns it: parse the file into cues and draw the active cue in
    ...
    // in-process to fragmented MP4 and served to AVPlayer over the `noiroremux://` scheme. Held for the whole
    // session so its resource-loader delegate + remux thread stay alive; torn down in stop()/loadFile().
    // LEGACY delivery: kept compiled as the rollback path behind NoiroRemuxHLSServer.deliveryEnabled.
    private var remuxLoader: NoiroRemuxResourceLoader?
    ...
    private var remuxHLSServer: NoiroRemuxHLSServer?
    ...
    private let remuxLoaderQueue = DispatchQueue(label: "noiro.dvremux.delegate")
    ...
    func loadFile(_ url: URL, headers: [String: String]?, live: Bool) {
        ...
        // (and the lane Apple documents for Dolby Vision 8.1). The legacy `noiroremux://` progressive loader
        // stays compiled behind NoiroRemuxHLSServer.deliveryEnabled for instant rollback. Everything below
        ...
        if wantsRemux, NoiroRemuxHLSServer.deliveryEnabled,
           let mounted = NoiroRemuxHLSServer.make(input: url, headers: headers) {
            ...
            // fail-soft fires next (see NoiroMKVRemuxStream), the item .failed demotion below ties the reason
        ...
        } else if wantsRemux, !NoiroRemuxHLSServer.deliveryEnabled,
                  let built = NoiroRemuxResourceLoader.make(input: url, headers: headers) {
    ...
    func addExternalSubtitle(url: String, title: String, lang: String,
                             timeout: TimeInterval, shouldLoad: (() -> Bool)?, completion: ((Bool) -> Void)?) {
        ...
        SubtitleFileFetcher.fetch(remote, timeout: timeout) { [weak self] data in
            ...
            Task { @MainActor in
                ...
                // Gemini AI translation (opt-in): translate the cue track to the target language and
                // overlay each translated cue on the renderer as it arrives. The original cue shows until
                // its translation lands, so the subtitle never blanks. No-op when translation is off / no key.
                let resume = self.player.currentTime().seconds
                if SubtitleTranslator.shared.shouldTranslate(sourceLanguage: lang, force: false) {
                    // Map cue INDEX (the translator's key) → the cue's START time (the renderer overlay key).
                    let indexToStart = Dictionary(cues.enumerated().map { ($0.offset, $0.element.start) },
                                                  uniquingKeysWith: { a, _ in a })
                    SubtitleTranslator.shared.translateTrack(
                        sourceData: data, sourceLanguage: lang, cues: cues, resumeSeconds: resume
                    ) { [weak self] byIndex in
                        // Re-key the batch by start time so the renderer can match each cue.
                        let byStart = Dictionary(
                            byIndex.compactMap { idx, text -> (TimeInterval, String)? in
                                guard let start = indexToStart[idx] else { return nil }
                                return (start, text)
                            },
                            uniquingKeysWith: { _, new in new }
                        )
                        self?.subtitleRenderer.applyTranslations(byStart)
                        if self?.externalSubActive == true {
                            self?.updateSubtitleOverlay(atClock: self?.player.currentTime().seconds ?? 0)
                        }
                    }
                }
    ...
    func setSubVisibility(_ visible: Bool) { /* no-op: AVPlayer embedded subs render natively; translation uses the cue overlay path */ }
    func setAudioDelay(_ seconds: Double) {}
    func setSubtitleVerticalOffset(_ points: Double) { subtitleOverlay?.setVerticalOffset(points) }
    ...
    func captureFrameJPEGData(maxWidth: CGFloat, completion: @escaping (Data?) -> Void) {
        ...
        // CVPixelBuffer is not Sendable, but this buffer was just copied out of the video output and is
        // never read again on this thread, so the hand-off is sole-ownership. `nonisolated(unsafe)`
        // records that as a deliberate, checked decision rather than leaving the warning standing.
        nonisolated(unsafe) let buffer = pixelBuffer
        DispatchQueue.global(qos: .utility).async {
            let data = Self.encodeJPEG(from: buffer, maxWidth: maxWidth, context: ctx)
            DispatchQueue.main.async { completion(data) }
        }
    }

    /// CVPixelBuffer (BGRA) -> downscaled JPEG via ImageIO (cross-platform; no UIKit/AppKit dependency).
    /// `nonisolated`: a pure function over its arguments, called from the utility queue above.
    nonisolated private static func encodeJPEG(from pixelBuffer: CVPixelBuffer, maxWidth: CGFloat, context: CIContext) -> Data? {
        ...
        case .readyToPlay:
            ...
            if !didStart {
                ...
                if isRemuxMounted || contentIsDolbyVision {
                    ...
                    // `nominalFrameRate` is deprecated in favour of the async `load(.nominalFrameRate)`.
                    // The track is already loaded by the time the item reports ready, so this resolves
                    // from cache and the mode request still lands on the same run loop turn in practice.
