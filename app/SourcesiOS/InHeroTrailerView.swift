...
struct InHeroTrailerView: View {
    ...
    /// `vortexo.app/api/noiro/v1/edge/trailer/yt` resolver or a direct stream) does not, so it can mount immediately.
    ...
    var body: some View {
        ...
        .task(id: url) {
            ...
            failed = false
            startedDelay = false
            // A REMOTE url (the public vortexo.app/api/noiro/v1/edge/trailer/yt resolver or a direct CDN stream) does NOT need the
