...
struct FeaturedHeroItem: Identifiable, Equatable, Hashable {
    ...
    /// The Noiro `/clip` URL for this title's ambient hero trailer (libmpv plays the muted mp4; the still
    ...
    var clipURL: URL? {
        var c = URLComponents(string: "https://vortexo.app/api/noiro/v1/edge/trailer/clip")
    ...
    /// `/yt/{id}` route (vortexo.app/api/noiro/v1/edge/trailer -> a direct stream libmpv/AVPlayer plays natively), the SAME path our
