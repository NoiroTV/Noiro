...
/// `WKURLSchemeHandler` (custom `noiro-yt://` scheme) and `webView.load(URLRequest)`, with a
...
/// custom `noiro-yt://` scheme serves the player HTML as a real loaded document (so cross-origin
...
private enum YouTubeEmbedConfig {
    static let handlerName = "noiroYT"
...
/// Serves the single player HTML document for the custom `noiro-yt://` scheme. Responding from a real
...
/// the custom `noiro-yt://` scheme and loaded via `webView.load(documentRequest)`; the `<base href>` +
