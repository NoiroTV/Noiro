...
/// Probes whether a `vortexo.app/api/noiro/v1/edge/trailer/clip` mp4 is actually warmed in R2, shared by every iOS/Mac
/// Trailer button (hero + detail). A cold clip returns an INSTANT 404 (`x-noiro-reason: clip_warming`,
...
enum TrailerClipProbe {
    static func isReady(_ url: URL) async -> Bool {
        ...
        req.httpMethod = "HEAD"
        req.setValue("Noiro", forHTTPHeaderField: "User-Agent")
        do {
