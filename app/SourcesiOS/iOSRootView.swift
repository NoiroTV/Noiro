...
struct iOSRootView: View {
    ...
    /// Settings is an overlay action, not a content tab. Keeping its state separate preserves the
    /// current screen, navigation stack, and scroll position underneath the trailing panel.
    ...
    #endif
    @AppStorage("noiro.update.dismissedVersion") private var dismissedUpdateVersion = ""
    ...
    @AppStorage("noiro.mergeDiscoverSearch") private var mergeDiscoverSearch = false
    ...
    /// `launch.isContentInteractive && needsPicker && presenter.request == nil`; the shared launch
    /// coordinator owns both the returning reveal and onboarding, so no profile modal can race them.
    private var pickerOwed: Bool {
        launch.isContentInteractive && profiles.needsPicker && !playbackGate.playerActive && !settings.isPresented
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ZStack {
                ...
                iOSHomeView(isActive: tab == .home && !settings.isPresented).opacity(tab == .home ? 1 : 0)
                if isMounted(.discover) { iOSDiscoverView(isActive: tab == .discover && !settings.isPresented).opacity(tab == .discover ? 1 : 0) }
                if !hideLiveTab, isMounted(.live) { iOSLiveView().opacity(tab == .live ? 1 : 0) }
                if isMounted(.library) { iOSLibraryView(isActive: tab == .library && !settings.isPresented).opacity(tab == .library ? 1 : 0) }
                if !mergeDiscoverSearch, isMounted(.search) { iOSSearchView(isActive: tab == .search && !settings.isPresented).opacity(tab == .search ? 1 : 0) }
        ...
        #else
        .onChange(of: launch.isContentInteractive) { interactive in
            if interactive {
                settings.endTaskPresentation()
                startUpdateMonitoringIfReady()
            }
    ...
    /// Open the customer's entitlement-protected Vortexo Library.
    private func openReleasesPage() {
        guard let url = URL(string: "https://vortexo.app/library") else { return }
        openURL(url)
    }
    ...
    private func tabButton(_ item: Tab) -> some View {
        let selected = item == .settings ? settings.isPresented : tab == item
        let base = Button {
            if item == .settings {
                settings.open()
                return
...
struct iOSHomeView: View {
    ...
    @AppStorage("noiro.home.showCuratedRails") private var showCuratedRails = true   // owner-toggleable: hide the built-in editorial rails
    ...
    private var visibleContinueWatching: [CoreCWItem] {
    ...
    private var continueWatchingItems: [RailItem] {
        visibleContinueWatching.map {
            RailItem(id: $0.id, type: $0.type, name: $0.name, poster: $0.poster, progress: $0.progress,
                     ...
                     suppressResumeProgress: $0.isMirroredCompletedSeries)
        }
    }
    ...
    private var heroCandidates: [FeaturedHeroItem] {
        ...
        var items: [FeaturedHeroItem] = visibleContinueWatching.prefix(3).map { cw in
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            // macOS sign-in lives in Settings -> Account ("Noiro account & sync").
            #if os(iOS)
            .toolbar {
        ...
        .onAppear {
            ...
            refreshTopPicks()
            refreshContinueWatchingAvailability()
        ...
        .onChange(of: profiles.activeID) { _ in refreshContinueWatchingAvailability(); if isActive { hero.seed(heroCandidates, reduceMotion: reduceMotion) }; refreshTopPicks() }
        .onChange(of: continueWatchingAvailabilitySignature) { _ in refreshContinueWatchingAvailability() }
        .onChange(of: continueWatchingAvailability.playableCompletedSeriesIDs) { _ in
            if isActive { hero.seed(heroCandidates, reduceMotion: reduceMotion) }
            refreshTopPicks()
        }
        ...
        .onChange(of: account.addons.count) { _ in refreshContinueWatchingAvailability(); refreshReleaseCalendar() }
    ...
    private func refreshTopPicks() {
        ...
        topPicks.refresh(profileID: profiles.activeID, cw: visibleContinueWatching, library: library)
    }
...
/// Library: the user's saved titles from the engine, as one filterable poster rail under the interactive featured
...
struct iOSLibraryView: View {
    ...
    private var libraryItems: [RailItem] {
        ...
        return source.map {
            RailItem(id: $0.id, type: $0.type, name: $0.name, poster: $0.poster,
                     progress: $0.progress, resumeSeconds: $0.resumeSeconds)
        }
    }
        ...
        NavigationStack(path: $path) {
            ScrollView {
                ...
                if !libraryItems.isEmpty {
                    // Hero is an ambient billboard scroll-header above the rail (shown only when there
                    // are saved titles), so its Play / Trailer buttons stay tappable. Type + sort
                    // compact filter menus sit between the hero and the single catalog rail, mirroring
                    // Discover; long-press on a card offers the engine's
                    ...
                    LazyVStack(alignment: .leading, spacing: Theme.Space.lg) {
                        ...
                        VStack(alignment: .leading, spacing: Theme.Space.xs) {
                            if profiles.activeUsesEngineHistory, let lib = core.library {
                                filterMenus(lib.selectable)
                            }
                            PosterRail(
                                title: libraryRailTitle,
                                items: libraryItems,
                                onTap: handleTap,
                                menu: .library
                            )
                    ...
                    .padding(.bottom, Theme.Space.md)
                    // Pin the column to the viewport width so the horizontal rail cannot make the stack
                    // adopt an over-wide ideal and shift the column left.
    ...
    /// Type + sort dropdowns, mirroring tvOS Library: each option carries the
    /// engine's own `request` and dispatches it back via `core.selectLibrary` on tap. The library
    /// re-emits and the rail + hero refresh on their own.
    private func filterMenus(_ selectable: CoreLibrarySelectable) -> some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: Theme.Space.sm) {
                Menu {
                    ForEach(selectable.types) { type in
                        Button { core.selectLibrary(type.request) } label: {
                            libraryMenuOption(AddonTerms.localize(type.label), selected: type.selected)
                        }
                    }
                } label: {
                    iOSDiscoverDropdownLabel(
                        title: "Type",
                ...
                Menu {
                    ForEach(selectable.sorts) { sort in
                        Button { core.selectLibrary(sort.request) } label: {
                            libraryMenuOption(AddonTerms.localize(sort.label), selected: sort.selected)
                        }
                    }
                } label: {
                    iOSDiscoverDropdownLabel(
    ...
    private func libraryMenuOption(_ title: String, selected: Bool) -> some View {
        HStack {
    ...
    private var libraryRailTitle: String {
        guard profiles.activeUsesEngineHistory, let selectable = core.library?.selectable else {
            return String(localized: "Watched")
        }
        let type = AddonTerms.localize(selectable.types.first(where: \.selected)?.label ?? "All")
        let sort = AddonTerms.localize(selectable.sorts.first(where: \.selected)?.label ?? "Recent")
        return "\(sort) · \(type)"
    }
}
...
/// Discover, driven by the stremio-core engine (CatalogWithFilters): compact type, catalog, and
/// genre menus carrying the engine's own request, followed by one Home-style poster rail — under
/// the interactive featured hero (shown once a catalog has loaded).
struct iOSDiscoverView: View {
    ...
    @AppStorage("noiro.discover.showCollectionsHub") private var showCollectionsHub = true   // toggle the hub on Discover (needs a TMDB key)
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ScrollView {
                ...
                // plain VStack sizes to its WIDEST child — and nested horizontal selector/rail ScrollViews
                ...
                LazyVStack(alignment: .leading, spacing: Theme.Space.md) {
                    ...
                    // an empty pool and re-seeds on addons/revision) and scrolls with the menus/rail. Not a
                    ...
                    if let discover = core.discover {
                        discoverFilterMenus(discover.selectable)
                        // One horizontal catalog at a time, matching Home. De-dup by id because
                        // paginated add-on catalogs can repeat a title across page boundaries.
                        PosterRail(
                            title: discoverRailTitle(discover.selectable),
                            items: dedupedMetasById(discover.items).map {
                                RailItem(id: $0.id, type: $0.type, name: $0.name, poster: $0.poster, progress: 0,
                                         background: $0.background, description: $0.description,
                                         releaseInfo: $0.releaseInfo, imdbRating: $0.imdbRating, genres: $0.genres)
                            },
                            onTap: handleTap,
                            onReachEnd: { core.loadDiscoverNextPage() }
                        )
                ...
                .padding(.bottom, Theme.Space.md)
                // Pin the column to the viewport width so long selector or catalog names cannot make the
                // LazyVStack adopt an over-wide ideal and shift the hero/menus/rail off the left edge.
                .frame(maxWidth: .infinity, alignment: .leading)
            }
            .scrollDismissesHeroRotation(model: hero)
        ...
        .onChange(of: showCollectionsHub) { show in if show { collectionsHub.load() } }   // no clear() on toggle-off: the render is already gated on showCollectionsHub, and clear() blanked the shared hub for the OTHER surface (Home vs Discover)
        // The catalog changes whenever a different type/catalog/genre is selected, which bumps revision —
    ...
    /// One compact selector band on iPhone, iPad, and Mac. It scrolls horizontally on narrow phones;
    /// opening a menu reveals the full option list and marks the current engine selection.
    private func discoverFilterMenus(_ selectable: CoreDiscoverSelectable) -> some View {
                ...
                Menu {
                    ForEach(shownTypes) { type in
                        Button { core.selectDiscover(type.request) } label: {
                            discoverMenuOption(type.type.capitalized, selected: type.selected)
                        }
                    }
                } label: {
                    iOSDiscoverDropdownLabel(
                ...
                Menu {
                    ForEach(selectable.catalogs) { catalog in
                        Button { core.selectDiscover(catalog.request) } label: {
                            discoverMenuOption(catalog.catalog, selected: catalog.selected)
                        }
                    }
                } label: {
                    iOSDiscoverDropdownLabel(
                    ...
                    Menu {
                        ForEach(genre.options) { option in
                            Button { core.selectDiscover(option.request) } label: {
                                discoverMenuOption(AddonTerms.localize(option.label), selected: option.selected)
                            }
                        }
                    } label: {
                        iOSDiscoverDropdownLabel(
    ...
    private func discoverMenuOption(_ title: String, selected: Bool) -> some View {
        HStack {
            Text(title).lineLimit(1)
            if selected { Image(systemName: "checkmark") }
        }
    ...
    private func discoverRailTitle(_ selectable: CoreDiscoverSelectable) -> String {
        let catalog = selectable.catalogs.first(where: \.selected)?.catalog ?? String(localized: "Discover")
        let type = (hideLiveTab
                    ? selectable.types.first { $0.selected && !LiveTypes.contains($0.type) }
                    : selectable.types.first(where: \.selected))?.type.capitalized
        guard let type, !type.isEmpty else { return catalog }
        return "\(catalog) · \(type)"
    }
}

/// Compact menu label shared by the iPhone/iPad and Mac Discover layouts. Keeping the field name
/// visible makes unusual add-on catalog names understandable without reopening the other menus.
private struct iOSDiscoverDropdownLabel: View {
    let title: String
...
struct RailItem: Identifiable {
    ...
    /// A mirrored completed series keeps its raw progress for next-episode routing, while the card itself
    /// represents the unstarted next episode and must not paint the old full stripe or resume time.
    var suppressResumeProgress: Bool = false

    var displayProgress: Double { suppressResumeProgress ? 0 : progress }
    var displayResumeSeconds: Double? { suppressResumeProgress ? nil : resumeSeconds }
}
...
private func iOSDirectResume(for item: RailItem, core: CoreBridge,
    ...
    // Stremio can leave a mirrored series at 90-100% on the completed episode. Falling back to Details
    // lets the shared watched-episode selection choose the next unwatched episode instead of replaying it.
    if MirrorSettings.mirrorContinueWatching, item.type == "series", item.progress >= 0.9 {
...
private struct iOSOpenLinkView: View {
    ...
    /// the remote resolver (vortexo.app/api/noiro/v1/edge/trailer/yt) EXACTLY as before when the direct resolve misses.
            ...
            ForEach(items) { item in
                Button { onTap(item) } label: {
                    PosterCardiOS(id: item.id, type: item.type, name: item.name, poster: item.poster, fallbackArt: item.background, imdbRating: item.imdbRating,
                                  progress: item.displayProgress, resumeSeconds: item.displayResumeSeconds, menu: menu)
                }
                .buttonStyle(.plain)
                .accessibilityElement(children: .ignore)
                .accessibilityLabel(item.name)
                .accessibilityHint("Opens details")
                .accessibilityValue(item.displayProgress > 0 ? "\(Int(item.displayProgress * 100)) percent watched" : "")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Space.sm) {
            ...
            ScrollViewReader { proxy in
                ...
                // Vortexo parity on newer systems: cards render to the rail edge, with an iOS 16-safe
                // fallback because NoiroiOSNative still supports iOS 16.
                .modifier(RailScrollClipModifier())
    ...
    @ViewBuilder private func railCard(_ item: RailItem, proxy: ScrollViewProxy) -> some View {
        let base = Button { onTap(item) } label: {
            PosterCardiOS(id: item.id, type: item.type, name: item.name, poster: item.poster, fallbackArt: item.background, caption: item.caption, imdbRating: item.imdbRating,
                          year: item.releaseInfo, genres: item.genres,
                          progress: item.displayProgress, resumeSeconds: item.displayResumeSeconds, menu: menu,
...
private struct RailScrollClipModifier: ViewModifier {
    @ViewBuilder
...
struct PosterCardiOS: View {
    ...
    /// rating/year/genres is present, the card shows `CardMetaOverlay` (title + footer over a scrim) and
    /// hides the legacy below-card title. `imdbRating` doubles as the overlay's rating input.
    var year: String? = nil
    var genres: [String]? = nil
    let progress: Double
    ...
    /// Whether the Vortexo-parity on-card overlay should render. Requires `onCardMeta` AND at least one
    /// piece of metadata (rating/year/genres) so a bare-title card never shows an empty scrim.
    private var showsOnCardMeta: Bool {
        catalogPrefs.onCardMeta && (imdbRating != nil || year != nil || (genres?.isEmpty == false))
    }
        ...
        VStack(alignment: .leading, spacing: 6) {
            ZStack(alignment: .bottom) {
                ...
                Group {
                    ...
                    // Vortexo-parity on-card metadata: title + footer over a bottom scrim (see CardMetaOverlay).
                    // Hidden unless `onCardMeta` is on and the card has at least rating/year/genre. When shown,
                    // the legacy below-card title is suppressed below to avoid duplication. Sits under the
                    // rating badge and progress stripe so those stay legible over the scrim.
                    .overlay(alignment: .bottom) {
                        if showsOnCardMeta {
                            CardMetaOverlay(title: displayName, rating: imdbRating, year: year, genres: genres, landscape: landscape)
                        }
                    }
                    .overlay(alignment: .topTrailing) {
                        // When a poster service bakes the rating into the image (Noiro/XRDB or ERDB), skip
                        // the native overlay to avoid a double badge. Also hidden when the on-card overlay is
                        // showing, since it already carries the rating in its footer.
                        if let rating = imdbRating, !rating.isEmpty, !PosterArtwork.bakesRatings, !showsOnCardMeta {
                            HStack(spacing: 2) {
                    ...
                    // Overlays added after the artwork clip are not constrained by it. Reapply the active
                    // Poster Style radius so the metadata scrim follows the user's corner setting too.
                    .clipShape(RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
                if progress > 0.01 {
            ...
            // shown), AND when the on-card overlay is showing (avoids duplication). The caption (Upcoming
            // Episodes "S2E5 · Jun 30") is a functional date, not a title, so it stays visible either way.
        ...
        navigationTitle(pageTitle)
            ...
            .toolbar {
                ToolbarItem(placement: .principal) {
                    if isActive {
                        // Brand lockup: the Noiro wordmark and mark (follows the theme
...
private struct MacBackAffordance: ViewModifier {
    @Environment(\.dismiss) private var dismiss
    @Environment(\.noiroSettingsSurface) private var settingsSurface

    @ViewBuilder
    func body(content: Content) -> some View {
        if settingsSurface == .panel {
            content
        } else {
            content
