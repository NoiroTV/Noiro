...
struct iOSReorderServicesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            NoiroSettingsPageTitle("Streaming services")
...
struct iOSDiscoverSettingsView: View {
    ...
    var body: some View {
        NoiroSettingsPanelPage {
            NoiroSettingsPageTitle("Discover & Region")

            NoiroSettingsGroupCard(
                "Region",
                footer: String(localized: "Orders streaming services and scopes catalogs to this region. Device region follows your system setting.")
            ) {
            ...
            NoiroSettingsGroupCard(
                "Sections",
                footer: String(localized: "Hide a whole section from Home and Discover.")
            ) {
            ...
            if !prefs.isCategoryHidden(HubCategoryKey.discoverSection) {
                NoiroSettingsGroupCard("Discover cards") {
            ...
            if !prefs.isCategoryHidden(HubCategoryKey.decadesSection) {
                NoiroSettingsGroupCard("Decades") {
...
struct iOSPosterStyleView: View {
    ...
    var body: some View {
        NoiroSettingsPanelPage {
            NoiroSettingsPageTitle("Poster Style")

            NoiroSettingsGroupCard("Preview") {
                PosterStylePreview()
            ...
            NoiroSettingsGroupCard(
                "Poster Cards",
                footer: String(localized: "Width sets how large posters are and how many fit per row. Balanced and Rounded match the default look.")
            ) {
            ...
            NoiroSettingsGroupCard(
                "Layout",
                footer: String(localized: "Text on cards shows the title and available details over the artwork. Poster labels control the separate title underneath.")
            ) {
                // Landscape 16:9 art works keyless (bundled TMDB key + vortexo.app/api/noiro/v1/edge/catalogs edge); no personal key required.
                Toggle("Landscape (16:9) art", isOn: $prefs.landscapeCards)
                Toggle("Show text on cards", isOn: $prefs.onCardMeta)
...
private struct PosterStylePreview: View {
    ...
    private var landscape: Bool { prefs.landscapeCards && apiKeys.hasTMDBArtwork }
    ...
    var body: some View {
        VStack(spacing: Theme.Space.md) {
            VStack(alignment: .leading, spacing: 6) {
                ZStack(alignment: .bottom) {
                    LinearGradient(colors: [Theme.Palette.accent.opacity(0.85), Theme.Palette.surface2],
                        ...
                        .overlay(alignment: .bottom) {
                            if prefs.onCardMeta {
                                CardMetaOverlay(title: "Sample Title", rating: "7.8", genres: ["Drama"], landscape: landscape)
                            }
                        }
                ...
                .frame(width: width, height: height)
                if !prefs.hidePosterLabels && !prefs.onCardMeta {
                    Text("Sample Title")
        ...
        .animation(.easeOut(duration: 0.2), value: prefs.onCardMeta)
    }
}
