...
struct iOSDetailView: View {
    ...
    @AppStorage("noiro.spoilerBlur") private var spoilerBlur = true   // blur unwatched episode thumbnails to avoid spoilers
    ...
    var body: some View {
        ...
        .onChange(of: core.metaDetails?.meta?.id) { _ in
            ...
            if creditsKey != ratingsImdbID {
                castMembers = []; crewMembers = []; creditsKey = nil
            }
        ...
        .onChange(of: showWhereToWatch) { enabled in
            if enabled { loadWatchProviders() }
        }
    ...
    /// SAME path our YouTube/Twitch URL playback already uses. There is NO vortexo.app/api/noiro/v1/edge/trailer full-trailer route
    ...
    /// Compact informational dropdown listing every language this title is available in. It deliberately
    /// does not alter playback preferences or filter sources; the language index is availability metadata.
    @ViewBuilder private var languageChips: some View {
        if !langChips.isEmpty {
            VStack(alignment: .leading, spacing: Theme.Space.xs) {
                ...
                .buttonStyle(ChipButtonStyle())
                .accessibilityLabel("Available in \(langChips.count) languages")
            }
    ...
    @ViewBuilder private var creditsRows: some View {
        if !railCastMembers.isEmpty || !railCrewMembers.isEmpty {
            VStack(alignment: .leading, spacing: Theme.Space.sm) {
                Button {
                ...
                if castExpanded {
                    VStack(alignment: .leading, spacing: Theme.Space.md) {
                        if !railCastMembers.isEmpty {
                            Text("Cast").font(Theme.Typography.eyebrow).foregroundStyle(Theme.Palette.textTertiary)
                            castRail
                        }
                        if !railCrewMembers.isEmpty {
                            Text("Crew").font(Theme.Typography.eyebrow).foregroundStyle(Theme.Palette.textTertiary)
                            crewRail
                        }
                    }
                    ...
                    ForEach(railCastMembers) { member in
                        if member.id > 0 {
                            NavigationLink {
                                ...
                                iOSPersonCreditCard(name: member.name, role: member.character, profileURL: member.profileURL)
                            }
                            .buttonStyle(.plain)
    ...
    @ViewBuilder private var crewRail: some View {
        if !railCrewMembers.isEmpty {
            ScrollView(.horizontal, showsIndicators: false) {
                LazyHStack(alignment: .top, spacing: Theme.Space.md) {
                    ForEach(railCrewMembers) { member in
                        if member.id > 0 {
                            NavigationLink {
                                PersonDetailView(route: PersonRoute(id: member.id, name: member.name,
                                                                    role: member.role, profileURL: member.profileURL))
                            } label: {
                                iOSPersonCreditCard(name: member.name, role: member.role, profileURL: member.profileURL)
                            }
                            .buttonStyle(.plain)
                            .accessibilityHint("Opens person details")
                        } else {
    ...
    private func refreshSourceIndex(torboxMerged: [CoreStreamSourceGroup]? = nil) {
        ...
        sourceIndex.refresh(contentID: contentID, isSignedIn: NoiroDocumentSyncManager.shared.isSignedIn)
    ...
    /// Fetch cross-provider ratings for this title. Prefers the Noiro ratings service (no user key
    /// needed: IMDb keyless, RT/Metacritic via Noiro's server-side key), then fills any gap from the
    ...
    private func loadRatings() {
        ...
        Task {
            ...
            // Only reach for the user's MDBList key to fill what Noiro did not return (e.g. RT / Metacritic
    ...
    /// Compact cross-provider ratings row ("IMDb 8.5  ·  RT 92%  ·  TMDB 78%"), fed by the Noiro ratings
    ...
    @ViewBuilder private var episodeList: some View {
        if let videos = meta?.videos, !sortedEpisodes(videos).isEmpty {
            let seasons = Array(Set(sortedEpisodes(videos).compactMap { $0.season })).sorted()
            let watched = watchedSet
            VStack(alignment: .leading, spacing: Theme.Space.md) {
            ...
            .onChange(of: videos.count) { _ in
                applyEpisodeSeason(Array(Set(sortedEpisodes(videos).compactMap { $0.season })).sorted())
            }
            .onChange(of: hideSeriesSpecials) { _ in
                applyEpisodeSeason(Array(Set(sortedEpisodes(videos).compactMap { $0.season })).sorted())
            }
    ...
    private func episodes(_ videos: [CoreVideo]) -> [CoreVideo] {
        sortedEpisodes(videos).filter { ($0.season ?? 1) == season }
    ...
    private func episodesInSeason(_ s: Int) -> [CoreVideo] {
        sortedEpisodes(meta?.videos ?? []).filter { ($0.season ?? 1) == s }
            .sorted { $0.episodeNumber < $1.episodeNumber }
    }
    ...
    private func loadWatchProviders() {
        guard showWhereToWatch, !LiveTypes.contains(type), let imdb = ratingsImdbID else { return }
        Task {
...
/// Shared touch/Mac Cast and Crew card. A real TMDB entry is used as a NavigationLink label; a synthetic
/// fallback is the same static card. VoiceOver combines the person name with the character/job.
private struct iOSPersonCreditCard: View {
    let name: String
    let role: String?
        ...
        VStack(spacing: Theme.Space.xs) {
            Group {
                if let profileURL, !profileURL.isEmpty {
                    CachedPosterImage(url: profileURL)
                } else {
                    ZStack {
            ...
            .clipShape(Circle())
            .overlay(Circle().strokeBorder(focused ? Theme.Palette.accent : Theme.Palette.textPrimary.opacity(0.08),
