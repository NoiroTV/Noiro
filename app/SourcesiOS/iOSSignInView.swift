...
struct iOSSignInView: View {
    ...
    var body: some View {
        ...
        .onReceive(account.$isSignedIn) { signedIn in
            ...
            // Stremio account's add-ons, snapshot the full descriptor set into the Noiro account doc so
            ...
            // No-op (never-zero guarded) if the engine is still empty or the Noiro account is unreachable.
            Task { @MainActor in
