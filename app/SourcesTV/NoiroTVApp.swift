    ...
    var body: some Scene {
        WindowGroup {
            ...
            .onChange(of: scenePhase) { _, phase in
                ...
                if phase == .active {
                    ...
                    Task {
                        ...
                        // Noiro account's owned add-ons + library so the lists never read zero on foreground.
                        ...
                        NoiroDocumentSyncManager.shared.requestSyncSoon()     // then push THIS device's state (incl. the library + add-ons mirror) so the web dashboard repopulates on open
                    }
            ...
            .onAppear {
                ...
                // Noiro account's owned add-ons when it boots degraded (no stream add-on) so a logged-out /
