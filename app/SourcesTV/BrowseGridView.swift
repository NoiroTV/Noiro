...
struct TVCollectionsHub: View {
    ...
    @ViewBuilder private func section<Content: View>(title: String, eyebrow: String, @ViewBuilder _ tiles: @escaping () -> Content) -> some View {
        ...
        // now built on the shared `PosterRailShell` so the hub reads at the same Vortexo-parity rhythm
        // (48pt side inset, 40pt between tiles, scrollClipDisabled) as every other catalog rail.
        //
...
/// Hub tile dimensions match the landscape catalog CARDS in the rows so the Discover / Streaming /
/// Genre / Decade tiles read as one consistent grid with the media cards. Width = `kLandscapeCardWidth`
/// (390), height = the same 16:9 a landscape card uses. The logo frames below derive from these so they
/// scale together.
private let kHubCardWidth: CGFloat = kLandscapeCardWidth
private let kHubTileHeight: CGFloat = kLandscapeCardWidth * 9.0 / 16.0
private let kHubTileWidth: CGFloat = 240   // legacy, retained for any external reference
...
struct DiscoverCardTile: View {
    ...
    var body: some View {
        ...
        .frame(width: kHubCardWidth, height: kHubTileHeight)
            ...
            if let style = ProviderBrandLogo.brandStyle(for: provider.providerID),
               ...
               let bundled = BundledLogo.rawImage(named: slug) {
                ...
                bundled
                    ...
                    .frame(width: kHubCardWidth * 0.62, height: kHubTileHeight * 0.62)
                ...
                bundled
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .frame(width: kHubCardWidth * 0.82, height: kHubTileHeight * 0.72)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                ...
                RemoteLogo(url: provider.logoURL, brandName: provider.name)
                    .frame(width: kHubCardWidth * 0.86, height: kHubTileHeight * 0.76)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
            } else {
...
// MARK: - Category browse (compact catalog menu + one fixed rail)
...
struct TVCategoryBrowse: View {
    ...
    private var selectedSub: SubCatalog? { subs.first(where: { $0.id == selectedID }) }
    private var usesFixedGenreLayout: Bool {
        if case .genre = target { return true }
        return false
    }

    /// Non-genre collection destinations keep their established multi-row grid. The new compact layout is
    /// intentionally scoped to the entered Genre screen requested here.
    ...
    private var legacyColumns: [GridItem] {
        catalogPrefs.landscapeCards && apiKeys.hasTMDBArtwork
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .topLeading) {
                BrowseHeroBackdrop(
                    model: focusModel,
                    // The Genre destination has its own title + catalog menu above the fixed rail. Using
                    // Discover's taller 180pt hero reserve pushes wide clearlogos beyond the top edge.
                    // Lower only the hero block; the independently pinned browse chrome stays untouched.
                    detailsBottom: usesFixedGenreLayout ? kHomeRailStripHeight + 120 : 520
                )
                .frame(width: geometry.size.width, height: geometry.size.height)

                if usesFixedGenreLayout { categoryContent }
                else { legacyCategoryContent }
            }
            // A pushed NavigationStack destination otherwise derives its ZStack height from the living
            // hero's current detail payload. When the focused title's enrichment arrives, that intrinsic
            // height changes by a few points and tvOS re-centres the focused horizontal rail. Give the hero
            // and browse chrome one immutable viewport so metadata/logo changes can only redraw in place.
    ...
    private var legacyCategoryContent: some View {
        ScrollView {
    ...
    private var legacyPills: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: Theme.Space.sm) {
                ForEach(subs) { sub in
                    Button { select(sub.id) } label: {
                        Text(LocalizedStringKey(sub.title)).lineLimit(1)
                    }
    ...
    @ViewBuilder private var legacyGrid: some View {
        if items.isEmpty {
            if done {
            ...
            LazyVGrid(columns: legacyColumns, spacing: Theme.Space.xl) {
    ...
    /// No outer vertical ScrollView: tvOS otherwise recenters the title, menu, and rail by a few points as
    /// focus moves between them. Only the catalog menu and the media rail are independently focusable.
                ...
                Menu {
                    ForEach(subs) { sub in
                        Button { select(sub.id) } label: {
                            HStack {
                                Text(LocalizedStringKey(sub.title)).lineLimit(1)
                                if sub.id == selectedID { Image(systemName: "checkmark") }
                            }
                        }
                    }
                } label: {
                    DiscoverDropdownLabel(
                        title: "Catalog",
                        selection: selectedSub?.title ?? "Choose"
                    )
                }
    ...
    @ViewBuilder private var rail: some View {
        if items.isEmpty {
            ...
            .frame(height: fixedRailHeight, alignment: .top)
        }
    }
    ...
    private func loadNext() async {
        ...
        if page == 2, let first = items.first {
            // A menu selection should immediately refresh the living hero even before focus reaches the rail.
            if focusModel.hero == nil { focusModel.seedIfEmpty(hero(for: first)) }
            else { focusModel.focus(hero(for: first)) }
        }
    }
...
struct TVReorderServicesView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Theme.Space.md) {
                NoiroSettingsPageTitle("Streaming Services").padding(.horizontal, Theme.Space.screenEdge)
...
struct TVDiscoverSettingsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Theme.Space.md) {
                NoiroSettingsPageTitle("Discover & Region").padding(.horizontal, Theme.Space.screenEdge)
