    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Theme.Space.lg) {
                NoiroSettingsPageTitle("Poster Style").padding(.horizontal, Theme.Space.screenEdge)
                ...
                VStack(alignment: .leading, spacing: Theme.Space.md) {
                    ...
                    // Landscape 16:9 art (`stremiox.catalog.landscapeCards`). Works keyless via the bundled
                    // TMDB key + the vortexo.app/api/noiro/v1/edge/catalogs edge (a real clean backdrop per title); a personal
                    // TMDB key is NOT required. This is the SAME toggle surfaced inline in Appearance as
                    ...
                    choiceRow(String(localized: "Text on cards"),
                              [("1", String(localized: "Show")), ("0", String(localized: "Hide"))],
                              selection: Binding(get: { prefs.onCardMeta ? "1" : "0" },
                                                 set: { prefs.onCardMeta = ($0 == "1") }))
                    ...
                    Text("Text on cards shows the title and available details over the artwork. Poster labels control the separate title underneath.")
                        .font(Theme.Typography.label).foregroundStyle(Theme.Palette.textSecondary)
                        .padding(.horizontal, Theme.Space.screenEdge)
                }
            }
