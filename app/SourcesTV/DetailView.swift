...
private enum DetailInfoTab: String, Identifiable {
    case episodes, cast, crew, languages, watch, similar
    ...
    var title: String {
        ...
        case .watch: return "Where to Watch"
...
struct DetailView: View {
    ...
    /// Drives the Home-style hero backdrop (top-right panel + bottom-left details block). The detail page
    /// shows ONE fixed title, so this is seeded once with that title's hero data (not focus-driven like the
    /// browse pages) and never changes. BrowseHeroBackdrop reads it for the artwork + details block.
    @StateObject private var heroModel = FocusedItemModel()
    ...
    @State private var selectedInfoTab: DetailInfoTab?
    @State private var movieShowsAllSources = false
    @FocusState private var seriesPlayFocused: Bool
    ...
    @State private var crewMembers: [TMDBClient.CrewMember] = []
    @State private var creditsKey: String?
    @AppStorage("noiro.detail.showFinancials") private var showFinancials = true   // budget + box office on movie detail (movies only, needs a TMDB key)
    ...
    var body: some View {
        ...
        .onDisappear { langChipsDebounce?.cancel() }
        ...
        .onChange(of: core.metaDetails?.libraryItem?.state.timeOffset) {
            captureHero()
        }
        .onChange(of: profiles.watch[id]?.timeOffsetMs) {
            captureHero()
        }
        .onChange(of: core.streamLoadProgress().loaded) {
            refreshLanguageChips()   // recompute the "Also available in" chips as more sources answer
        }
        .onChange(of: showWhereToWatch) {
            if showWhereToWatch { loadWatchProviders() }
        }
    }
    ...
    /// Focusable circular language badges, matching the Cast and Crew portrait rails. A representative
    /// country flag is the icon and the localized language name sits beneath it; these remain informational and do
    /// not change playback-language preferences.
    private var uniqueLanguageChips: [(code: String, label: String)] {
        var seen = Set<String>()
        return langChips.filter { chip in
            let key = chip.label.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            return seen.insert(key).inserted
        }
    }

    @ViewBuilder private var languageChips: some View {
        if !uniqueLanguageChips.isEmpty {
            ScrollView(.horizontal, showsIndicators: false) {
                LazyHStack(alignment: .top, spacing: Theme.Space.lg) {
                    ForEach(uniqueLanguageChips, id: \.code) { chip in
                        LanguageBadgeCard(code: chip.code, label: chip.label)
                    }
                }
    ...
    private func loadCredits() {
        ...
        Task {
            ...
            await MainActor.run {
                ...
                if !result.crew.isEmpty { crewMembers = result.crew }
            }
        }
    }
    ...
    private func loadRatings() {
        ...
        Task {
            // Noiro's keyless ratings service first (IMDb / RT / Metacritic / TMDB, no user key needed), then
            // reach for the user's own MDBList key ONLY to fill what Noiro did not return. Mirrors iOS so both
            ...
            let vx = await NoiroRatingsClient.ratings(imdbID: imdb, type: type)
    ...
    private var availableInfoTabs: [DetailInfoTab] {
        var tabs: [DetailInfoTab] = []
        if effectiveType == "series", let videos = core.metaDetails?.meta?.videos,
           !sortedEpisodes(videos).isEmpty { tabs.append(.episodes) }
        if !railCastMembers.isEmpty { tabs.append(.cast) }
        if !railCrewMembers.isEmpty { tabs.append(.crew) }
        if !langChips.isEmpty { tabs.append(.languages) }
        if showWhereToWatch, watchAvail?.providers.isEmpty == false { tabs.append(.watch) }
        if !similarItems.isEmpty { tabs.append(.similar) }
        return tabs
    }

    private var activeInfoTab: DetailInfoTab {
        if let selectedInfoTab, availableInfoTabs.contains(selectedInfoTab) { return selectedInfoTab }
        return availableInfoTabs.first ?? .cast
    }

    /// Moonfin-style lower detail surface: one steady tab strip and exactly one fixed-height content window.
    /// On Apple TV merely focusing a tab selects it, so moving left/right previews each section without an
    /// extra click. The panel height never changes between Cast, Crew, Languages, Watch, and Similar.
    @ViewBuilder private var detailInfoTabs: some View {
        if !availableInfoTabs.isEmpty {
            VStack(alignment: .leading, spacing: Theme.Space.md) {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: Theme.Space.sm) {
                        ForEach(availableInfoTabs) { tab in
                            DetailInfoTabButton(tab: tab, selected: activeInfoTab == tab) {
                                selectedInfoTab = tab
                            }
                        }
                    }
                ...
                Group {
                    ...
                    case .episodes: seriesEpisodesTabContent
                    case .cast: castTabContent
                    case .crew: crewTabContent
                    case .languages: languageChips
                    case .watch: whereToWatchTabContent
                    case .similar: similarTabContent
                    }
                }
    ...
    @ViewBuilder private var seriesEpisodesTabContent: some View {
        if let meta = core.metaDetails?.meta, let videos = meta.videos, !videos.isEmpty {
            ...
            CoreSeasonedEpisodes(meta: meta, videos: videos,
                                 watched: watched,
                                 initialSeason: resumeSeasonHint(videos, metaID: meta.id),
                                 hideSpecials: hideSeriesSpecials)
        }
    }

    private var castTabContent: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHStack(alignment: .top, spacing: Theme.Space.lg) {
                ForEach(railCastMembers.prefix(30)) { member in
                    if member.id > 0 {
                        NavigationLink {
                            PersonDetailView(route: PersonRoute(id: member.id, name: member.name,
                                                                role: member.character, profileURL: member.profileURL))
                        } label: {
                            PersonCreditCard(name: member.name, role: member.character,
                                             profileURL: member.profileURL, interactive: true)
                        }
                        .buttonStyle(CardFocusStyle(scale: 1))
                        .accessibilityLabel([member.name, member.character].compactMap { $0 }.joined(separator: ", "))
                        .accessibilityHint("Opens person details")
                    } else {
                        PersonCreditCard(name: member.name, role: member.character, profileURL: member.profileURL)
                    }
                }
            }
            .padding(.horizontal, Theme.Space.screenEdge)
    ...
    private var railCrewMembers: [TMDBClient.CrewMember] {
        if !crewMembers.isEmpty { return crewMembers }
        guard let meta = core.metaDetails?.meta else { return [] }
        var fallback: [TMDBClient.CrewMember] = []
        for (index, name) in meta.directors.enumerated() {
            fallback.append(.init(id: -10_000 - index, name: name, role: "Director", profileURL: nil))
        }
        for (index, name) in meta.writers.enumerated() {
            fallback.append(.init(id: -20_000 - index, name: name, role: "Writer", profileURL: nil))
        }
        return fallback
    }
    ...
    private var crewTabContent: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHStack(alignment: .top, spacing: Theme.Space.lg) {
                ForEach(railCrewMembers.prefix(30)) { member in
                    if member.id > 0 {
                        NavigationLink {
                            PersonDetailView(route: PersonRoute(id: member.id, name: member.name,
                                                                role: member.role, profileURL: member.profileURL))
                        } label: {
                            PersonCreditCard(name: member.name, role: member.role,
                                             profileURL: member.profileURL, interactive: true)
                        }
                        .buttonStyle(CardFocusStyle(scale: 1))
                        .accessibilityLabel([member.name, member.role].compactMap { $0 }.joined(separator: ", "))
                        .accessibilityHint("Opens person details")
                    } else {
                        PersonCreditCard(name: member.name, role: member.role, profileURL: member.profileURL)
                    }
                }
            }
            .padding(.horizontal, Theme.Space.screenEdge)
    ...
    @ViewBuilder private var whereToWatchTabContent: some View {
        if showWhereToWatch, let avail = watchAvail, !avail.providers.isEmpty {
            ScrollView(.horizontal, showsIndicators: false) {
    ...
    private func loadWatchProviders() {
        guard showWhereToWatch, !LiveTypes.contains(type), id.hasPrefix("tt") else { return }
        Task {
    ...
    /// through the add-on system, so it works for every id scheme (tt, tmdb:, tvdb:, anything). Also
    /// seeds this page's own `heroModel` so the Home-style `BrowseHeroBackdrop` shows the title's art +
    /// details (the detail page shows one fixed title, so this is seeded once, not focus-driven).
    private func captureHero() {
        ...
        var parts: [String] = []
        if let r = m.releaseInfo, !r.isEmpty { parts.append(r) }
        if let imdb = m.imdbRating, !imdb.isEmpty { parts.append("★ \(imdb)") }
        if let rt = m.runtime, !rt.isEmpty { parts.append(rt) }
        parts.append(type.capitalized)
        if let progress = heroWatchProgress(metaID: m.id) {
            parts.append("\(Int((progress * 100).rounded()))% watched")
        }
        let genreLine = (m.genres.isEmpty == false) ? m.genres.prefix(3).joined(separator: " · ") : nil
        let hero = FocusedHero(id: m.id, type: type, title: m.name,
                               backdrop: m.background ?? m.poster,
                               metaLine: parts.joined(separator: "  ·  "),
                               overview: m.description, genreLine: genreLine, logo: m.logo)
        heroModel.setFixed(hero)
    }

    /// Use the same current-video progress represented by Home's `CoreCWItem.focusedHero`. This is the
    /// episode progress for a series and the movie progress for a movie, matching the source hero exactly.
    private func heroWatchProgress(metaID: String) -> Double? {
        let progress = profiles.activeUsesEngineHistory
            ? core.metaDetails?.libraryItem?.progress
            : profiles.watch[metaID]?.progress
    ...
    private func seriesPage(_ meta: CoreMetaItem, videos: [CoreVideo]) -> some View {
        ...
        return ZStack {
            // Series use the exact same fixed composition as movies: hero details above a pinned action
            // band and one lower tab window. With no vertical ScrollView, focus cannot move the page.
            BrowseHeroBackdrop(model: heroModel, detailsBottom: 660, artworkVerticalOffset: 137)
            VStack(alignment: .leading, spacing: Theme.Space.lg) {
                // Series actions align to the movie action baseline. The series row has no source-status
                // column above it, so its local offset is slightly smaller for the same screen position.
                Spacer().frame(height: 568)
                HStack(spacing: Theme.Space.md) {
                    if let primaryEpisode = primary?.video {
                        NavigationLink {
                            CoreEpisodeStreams(meta: meta, video: primaryEpisode,
                                               season: primaryEpisode.season ?? 0,
                                               episodes: sortedEpisodes(meta.videos ?? []))
                        } label: {
                            HStack(spacing: Theme.Space.sm) {
                                Image(systemName: "play.fill")
                                if seriesPlayFocused {
                                    Text(primaryEpisodeLabel(primaryEpisode,
                                                             isResume: primary?.isResume == true,
                                                             resumeSeconds: (primary?.isResume == true)
                                                             ? primaryEpisodeResumeSeconds(primaryEpisode, metaID: meta.id) : nil))
                                        .lineLimit(1)
                                }
                            }
                        }
                        .buttonStyle(ExpandableWatchButtonStyle())
                        .focused($seriesPlayFocused)
                    }
                    trailerChip(meta)
                    LibraryChip()
                    Spacer(minLength: 0)
                ...
                .overlay(alignment: .bottomLeading) {
                    if (primary?.isResume == true), primaryProgress > 0.01 {
    ...
    private func moviePage(_ m: CoreMetaItem) -> some View {
        // Home-style hero (Vortexo parity): the backdrop is the top-right artwork panel + a bottom-left
        // details block (title/logo, meta, synopsis) driven by `heroModel`, NOT full-bleed. The title is
        // fixed (the detail page shows one title), so `heroModel` is seeded once on appear. The interactive
        // surface — language chips, trailer chip, Watch/Sources — moves into the scroll content as the first
        // row, below the hero; cast / Where to Watch / More Like This follow as rows.
        ZStack {
            // Keep the fixed lower controls untouched; lift only the hero information so the reserved
            // five-line synopsis clears the action row in the non-scrolling composition.
            BrowseHeroBackdrop(model: heroModel, detailsBottom: 660, artworkVerticalOffset: 137)
            // Do not put this surface in a vertical ScrollView. tvOS automatically scrolls the nearest
            // vertical container to center whichever descendant receives focus; that made the action band
            // and tabs jump when focus crossed between Play, the tab strip, and a person card. The upper
            // controls and the one-window tab surface are deliberately pinned to screen coordinates. Only
            // their horizontal rails scroll.
            VStack(alignment: .leading, spacing: Theme.Space.lg) {
                // Root detail content begins beneath the persistent tvOS tab bar. This offset places the
                // action band immediately below the five-line synopsis (the approved 14:06 composition)
                // without relying on focus-driven scrolling.
                Spacer().frame(height: 590)
                CoreStreamList(title: m.name,
                               ...
                               trailerAction: hasFullTrailer(m) ? { playTrailer(m) } : nil,
                               showAllSourcesBinding: $movieShowsAllSources)
                    .frame(height: movieShowsAllSources ? 430 : 112, alignment: .topLeading)
                    .clipped()
                    .padding(.horizontal, Theme.Space.screenEdge)
                if !movieShowsAllSources { detailInfoTabs }
                Spacer(minLength: 0)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
            .clipped()
                ...
                VStack(alignment: .leading, spacing: Theme.Space.xs) {
                    HStack(spacing: Theme.Space.sm) {
                        ...
                        if primaryEpisode == nil {
                            ...
                            Button(action: scrollToContent) {
                                ExpandableDetailActionLabel(title: "Episodes", systemImage: "list.bullet")
                            }
                            .buttonStyle(ExpandableDetailActionStyle())
                        }
                        LibraryChip()
                        trailerChip(m)
                        Spacer(minLength: 0)
           ...
           let url = TrailerRequest.from(meta: m)?.playableURL {
            ...
            Group {
                if let attempt = heroDirectTrailer, attempt.metaID == m.id {
                    TVInHeroTrailerView(url: attempt.url ?? url, window: nil, showsMoreAtBottom: true)
                }
            }
    ...
    private func resolveHeroDirectTrailer(_ m: CoreMetaItem) async {
        ...
        var direct: URL? = nil
        // Prefer IMDb's stable muxed 1080p official trailer when this meta has an IMDb id. YouTube's
        // proof-gated HD URLs can stop partway through; its complete 360p representation stays the reserve.
        if directTrailerURL(m) == nil {
            if let imdb = await IMDbTrailerResolver.resolve(
                imdbID: m.id, maxHeight: 1080, languageCode: TMDBClient.trailerLanguageBaseCode
            ) {
                direct = imdb.videoURL
                NSLog("[imdb-trailer] tvOS detail ambient: h=%d", imdb.height)
            } else if let yt = m.trailerYouTubeID, !yt.isEmpty {
    ...
    /// via the public `vortexo.app/api/noiro/v1/edge/trailer/yt` remote resolver (needs no local server), so the chip no longer
    ...
    ///      `vortexo.app/api/noiro/v1/edge/trailer/yt` remote resolver on Lite, `StremioServer.trailerResolverBase` picks). The id
    ...
    /// on Lite the YouTube path resolves through the remote `vortexo.app/api/noiro/v1/edge/trailer/yt`, so the chip is no longer
    ...
    @ViewBuilder private func trailerChip(_ m: CoreMetaItem) -> some View {
        if hasFullTrailer(m) {
            Button { playTrailer(m) } label: {
                ExpandableDetailActionLabel(title: "Trailer", systemImage: "film")
            }
            .buttonStyle(ExpandableDetailActionStyle())
        }
    }

    private func playTrailer(_ m: CoreMetaItem) {
        Task { @MainActor in
            // A direct stream carried by the meta remains authoritative.
            if let direct = directTrailerURL(m) {
                presenter.request = PlaybackRequest(url: direct, title: "\(m.name) Trailer", isTrailer: true)
                return
            }

            // IMDb exposes the title's official trailer as one complete muxed MP4 (normally 1080p). Prefer it
            // over YouTube's proof-gated HD URLs; the complete YouTube representation remains the fail-soft
            // reserve for titles IMDb does not carry.
            if let imdb = await IMDbTrailerResolver.resolve(
                imdbID: m.id, maxHeight: 1080, languageCode: TMDBClient.trailerLanguageBaseCode
            ) {
                NSLog("[imdb-trailer] tvOS trailer button: h=%d", imdb.height)
                presenter.request = PlaybackRequest(url: imdb.videoURL, title: "\(m.name) Trailer", isTrailer: true)
                return
            }

            if let yt = await preferredTrailerYouTubeID(m),
               let resolved = await YouTubeDirectResolver.resolve(videoID: yt, maxHeight: 1080) {
    ...
    /// Compact cross-provider ratings row ("IMDb 8.5  ·  RT 92%  ·  MC 81  ·  TMDB 78%"), fed by the Noiro
    ...
    private func sortedEpisodes(_ videos: [CoreVideo]) -> [CoreVideo] {
        SeriesDetailSettings.visibleEpisodes(videos, hideSpecials: hideSeriesSpecials).sorted {
...
struct CoreSeasonedEpisodes: View {
    ...
    @AppStorage("noiro.spoilerBlur") private var spoilerBlur = true   // observed so a Settings toggle redraws; effective value via SpoilerBlurSetting (user wins over the RemoteConfig fleet default)
    ...
    @FocusState private var focusedSeason: Int?   // browsing season chips previews/selects immediately; OK remains available for context-menu behavior
    ...
    private var visibleVideos: [CoreVideo] {
        SeriesDetailSettings.visibleEpisodes(videos, hideSpecials: hideSpecials)
    }
    private var playbackEpisodes: [CoreVideo] { visibleVideos.orderedBySeasonEpisode }
    private var resumeSeason: Int? { resumeVideoId.flatMap { id in visibleVideos.first { $0.id == id }?.season } }

    private func recomputeSeasons() { seasons = Array(Set(visibleVideos.map { $0.season ?? 0 })).sorted() }
    private func recomputeEpisodes() {
        episodes = visibleVideos.filter { ($0.season ?? 0) == season }.sorted { ($0.episode ?? 0) < ($1.episode ?? 0) }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Space.xs) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                LazyHStack(alignment: .top, spacing: Theme.Space.lg) {
                    ForEach(episodes) { v in episodeCard(v).focused($focusedEpisode, equals: v.id) }
                }
                .padding(.horizontal, Theme.Space.screenEdge)
        ...
        .onChange(of: focusedSeason) { _, focused in
            guard let focused, seasons.contains(focused), season != focused else { return }
            season = focused
        }
        ...
        .onChange(of: presenter.request == nil) {
            guard presenter.request == nil, let id = resumeVideoId, visibleVideos.contains(where: { $0.id == id }) else { return }
        ...
        .onChange(of: hideSpecials) { applyPreferredSeason() }
    }
    ...
    /// Fixed-window episode card: the season stays above and episodes browse as one horizontal row, so
    /// moving focus between episodes never asks tvOS to scroll the entire detail page vertically.
    private func episodeCard(_ v: CoreVideo) -> some View {
        let isWatched = watched.contains(v.id)
        let progress = episodeProgress(v)
        return NavigationLink {
            CoreEpisodeStreams(meta: meta, video: v, season: v.season ?? season, episodes: playbackEpisodes)
        } label: {
    ...
    private func thumbnail(_ v: CoreVideo, isWatched: Bool, progress: Double) -> some View {
        ...
        .frame(width: kLandscapeCardWidth, height: (kLandscapeCardWidth * 9 / 16).rounded())
...
struct CoreStreamList: View {
    ...
    /// Optional movie-detail trailer action. Supplying it places Trailer immediately after the primary
    /// Watch/Resume control in every source-loading state; episode/live call sites leave it nil.
    var trailerAction: (() -> Void)? = nil
    /// Movie detail lifts this state so its fixed lower window can swap between information tabs and the
    /// ranked source list without moving the hero or action row. Other call sites keep local behavior.
    var showAllSourcesBinding: Binding<Bool>? = nil
    @EnvironmentObject private var core: CoreBridge
    @EnvironmentObject private var theme: ThemeManager
    ...
    @State private var localShowAllSources = false   // fallback for episode/live call sites
    ...
    // Stremio account (api.strem.io). NOTE: the source-index SERVE read is gated on the NOIRO-SYNC account
    // (NoiroDocumentSyncManager, the moat-token identity), NOT this one -- see refreshSourceIndex().
    ...
    @AppStorage("noiro.downloadEvictionAck") private var downloadEvictionAck = false
    ...
    private var showAllSources: Bool { showAllSourcesBinding?.wrappedValue ?? localShowAllSources }

    private func toggleAllSources() {
        if let showAllSourcesBinding {
            showAllSourcesBinding.wrappedValue.toggle()
        } else {
            localShowAllSources.toggle()
        }
    }
    ...
    var body: some View {
        ...
        return AnyView(VStack(alignment: .leading, spacing: Theme.Space.md) {
            ...
            if let best {
                ...
                HStack(spacing: Theme.Space.md) {
                        ...
                        HStack(spacing: Theme.Space.sm) {
                            ...
                            if watchFocused {
                                if watchReady {
                                    ...
                                    Text(verbatim: "\(lead)\(StreamRanking.watchLabel(best))")
                                } else {
                                    Text(verbatim: String(localized: "Finding best…  \(addons.loaded)/\(addons.total)"))
                                }
                            }
                        }
                        .animation(.easeOut(duration: 0.2), value: watchFocused)
                        .accessibilityLabel(
                            watchReady
                            ? Text(resumeSeconds == nil ? "Watch" : "Resume")
                    ...
                    if let trailerAction {
                        Button(action: trailerAction) {
                            ExpandableDetailActionLabel(title: "Trailer", systemImage: "film")
                        }
                        .buttonStyle(ExpandableDetailActionStyle())
                    }
                    ...
                    Button { showQualityPicker = true } label: {
                        ExpandableDetailActionLabel(title: "Quality", systemImage: "chevron.up.chevron.down")
                    }
                    .buttonStyle(ExpandableDetailActionStyle())
                    ...
                    Button { withAnimation { toggleAllSources() } } label: {
                        ExpandableDetailActionLabel(
                            title: showAllSources ? "Hide sources" : "All sources · \(streamCount)",
                            systemImage: showAllSources ? "chevron.up" : "list.bullet"
                        )
                    }
                    .buttonStyle(ExpandableDetailActionStyle(selected: showAllSources))
                ...
                if showAllSources {
                    // The full list owns the same bounded lower window normally occupied by the info tabs.
                    // Its own vertical scroller keeps the fixed hero/action geometry untouched.
                    ScrollView {
                        VStack(alignment: .leading, spacing: Theme.Space.md) {
        ...
        .onChange(of: core.streamLoadProgress().loaded) { scheduleSourceRefresh() }
        ...
        .onChange(of: sourceIndex.streams.count) { scheduleSourceRefresh() }
        ...
        .onChange(of: showAllSources) { if !showAllSources { sourceRenderLimit = Self.sourceWindowInitial } }
        .onChange(of: meta?.libraryId) { sourceRenderLimit = Self.sourceWindowInitial }
    ...
    @ViewBuilder private func downloadChip(ready: Bool, action: @escaping () -> Void) -> some View {
        ...
        Button {
            ...
            switch state {
            case .done:
                ExpandableDetailActionLabel(title: "Downloaded", systemImage: "checkmark.circle.fill")
            case .inProgress:
                ExpandableDetailActionLabel(title: "Downloading", systemImage: "arrow.down.circle", loading: true)
            case .none:
                ExpandableDetailActionLabel(title: "Download", systemImage: "arrow.down.circle")
            }
        }
        .buttonStyle(ExpandableDetailActionStyle(selected: state == .done))
    ...
    ///
    /// SIGN-IN IDENTITY: the SERVE read is gated on the NOIRO-SYNC account, not the Stremio account. The moat
    /// token that un-gates `vortexo.app/api/noiro/v1/edge/sources` is minted from the Noiro session bearer (`NoiroDocumentSyncManager`), so
    /// a Stremio-only sign-in mints no token and the worker returns an empty `login_required` list. Gate on the
    /// same identity that mints the token so a signed-in Noiro user actually sees pooled sources.
    private func refreshSourceIndex(torboxMerged: [CoreStreamSourceGroup]? = nil) {
        guard let contentID = sourceContentID else { return }
        let noiroSignedIn = NoiroDocumentSyncManager.shared.isSignedIn
        sourceIndex.refresh(contentID: contentID, isSignedIn: noiroSignedIn)
...
struct LibraryChip: View {
    ...
    var body: some View {
        ...
        Button {
            ...
            ExpandableDetailActionLabel(title: saved ? "In Library" : "Add to Library",
                                        systemImage: saved ? "bookmark.fill" : "bookmark")
        }
        .buttonStyle(ExpandableDetailActionStyle(selected: saved))
    }
}

/// Icon-only while idle, then reveals its label when the containing tvOS Button receives focus. Used by
/// every secondary action that follows Play so the row stays compact without hiding meaning during use.
private struct ExpandableDetailActionLabel: View {
    let title: String
...
/// Compact circular secondary detail action that expands into a capsule on focus. Selected states such as
/// In Library / Hide Sources keep a soft accent fill while retaining the same focus behavior.
private struct ExpandableDetailActionStyle: ButtonStyle {
    var selected = false
    ...
    private struct Content: View {
        ...
        var body: some View {
            configuration.label
                .font(Theme.Typography.label)
                .foregroundStyle(focused || selected ? Theme.Palette.textPrimary : Theme.Palette.textSecondary)
                .frame(minWidth: 56, minHeight: 56)
                .padding(.horizontal, focused ? Theme.Space.md : 0)
                .background(selected ? Theme.Palette.accent.opacity(0.22) : Theme.Palette.surface2,
...
/// The detail page's primary Watch/Resume control: a circular play icon while idle, expanding into the
/// full quality-aware label only while focused. Kept separate from `PrimaryActionStyle` so login, setup,
/// add-on, and other primary buttons elsewhere in the app retain their normal always-labeled shape.
private struct ExpandableWatchButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
...
/// One focus-driven tab in the fixed lower detail strip. Focus and press both select it; the explicit
/// focus callback gives Apple TV the instant Moonfin-style panel switching requested for remote browsing.
private struct DetailInfoTabButton: View {
    let tab: DetailInfoTab
    let selected: Bool
    let select: () -> Void
...
/// One tvOS person credit shared by Cast and Crew: circular TMDB headshot (initials fallback), name, and
/// character/job beneath. It is directly focusable rather than an actionless Button, avoiding tvOS's large
/// white focus plate; only the theme ring marks focus.
private struct PersonCreditCard: View {
    let name: String
    let role: String?
    ...
    private var photo: some View {
        ...
        .clipShape(Circle())
        .overlay(
            Circle().strokeBorder(focused ? Theme.Palette.accent : Theme.Palette.textPrimary.opacity(0.08),
                                  lineWidth: focused ? 5 : 1)
        )
        .animation(.easeOut(duration: 0.18), value: focused)
    }
}

/// Circular language icon in the same visual rhythm as Cast/Crew. Each language uses a representative
/// country flag; languages without a stable mapping fall back to the international globe.
private struct LanguageBadgeCard: View {
    let code: String
    let label: String
    @FocusState private var focused: Bool
    ...
    var body: some View {
        VStack(spacing: Theme.Space.sm) {
            ZStack {
                ...
                AsyncImage(url: Self.flagURL(for: code)) { phase in
                    switch phase {
                    ...
                    case .failure:
                        Image(systemName: "globe")
                            ...
                            .foregroundStyle(Theme.Palette.textSecondary)
                    default:
                        ProgressView().tint(Theme.Palette.textTertiary)
                    }
                }
            }
            .frame(width: 140, height: 140)
    ...
    private static func flagURL(for rawCode: String) -> URL? {
        let language = rawCode.lowercased()
            .replacingOccurrences(of: "_", with: "-")
            .split(separator: "-").first.map(String.init) ?? ""
        // Use the owner's chosen round English-language artwork exactly; the remaining languages use
        // equivalent high-resolution flag PNGs and receive the same circular crop in the view above.
        if language == "en" || language == "eng" {
            return URL(string: "https://toppng.com/uploads/preview/english-language-flag-round-11562903960vbmoq86rb5.png")
        }
        let country: String?
        switch language {
        case "ar", "ara": country = "sa"
        case "hi", "hin", "ta", "tam", "te", "tel", "mr", "mar", "pa", "pan": country = "in"
        case "fr", "fra", "fre": country = "fr"
        case "de", "deu", "ger": country = "de"
        case "es", "spa", "ca", "cat", "eu", "eus", "baq": country = "es"
        case "it", "ita": country = "it"
        case "pt", "por": country = "pt"
        case "zh", "zho", "chi": country = "cn"
        case "ja", "jpn": country = "jp"
        case "ko", "kor": country = "kr"
        case "ru", "rus": country = "ru"
        case "uk", "ukr": country = "ua"
        case "pl", "pol": country = "pl"
        case "nl", "nld", "dut": country = "nl"
        case "sv", "swe": country = "se"
        case "no", "nor": country = "no"
        case "da", "dan": country = "dk"
        case "fi", "fin": country = "fi"
        case "is", "isl", "ice": country = "is"
        case "el", "ell", "gre": country = "gr"
        case "tr", "tur": country = "tr"
        case "he", "heb": country = "il"
        case "fa", "fas", "per": country = "ir"
        case "ur", "urd": country = "pk"
        case "bn", "ben": country = "bd"
        case "th", "tha": country = "th"
        case "vi", "vie": country = "vn"
        case "id", "ind": country = "id"
        case "ms", "msa", "may": country = "my"
        case "cs", "ces", "cze": country = "cz"
        case "sk", "slk", "slo": country = "sk"
        case "hu", "hun": country = "hu"
        case "ro", "ron", "rum": country = "ro"
        case "bg", "bul": country = "bg"
        case "hr", "hrv": country = "hr"
        case "sr", "srp": country = "rs"
        case "bs", "bos": country = "ba"
        case "sl", "slv": country = "si"
        case "mk", "mkd", "mac": country = "mk"
        case "sq", "sqi", "alb": country = "al"
        default: country = nil
        }
        return country.flatMap { URL(string: "https://flagcdn.com/w320/\($0).png") }
    }
}
