...
/// URL (the SAME path the Trailer button plays; the retired R2 `vortexo.app/api/noiro/v1/edge/trailer/clip` snippet is gone,
...
struct TVInHeroTrailerView: View {
    ...
    /// Shared hero surfaces already apply their own narrow bottom blend. Avoid stacking a second near-black
    /// fade so moving trailers remain visible down to the adjacent content row.
    var showsMoreAtBottom = true
    ...
    /// Gate for mounting the libmpv surface. For a REMOTE url (the `vortexo.app/api/noiro/v1/edge/trailer` resolver or a direct
    ...
    /// in-process node server to be up. A remote host (the `vortexo.app/api/noiro/v1/edge/trailer` resolver, a direct CDN
    ...
    var body: some View {
        ...
        .task(id: url) {
            ...
            // REMOTE url (the public `vortexo.app/api/noiro/v1/edge/trailer` resolver or a direct CDN trailer stream) needs no
    ...
    private var scrim: some View {
        ZStack {
            if showsMoreAtBottom {
                LinearGradient(stops: [
                    .init(color: .clear, location: 0.0),
                    .init(color: .clear, location: 0.62),
                    .init(color: Theme.Palette.canvas.opacity(0.26), location: 0.84),
                    .init(color: Theme.Palette.canvas.opacity(0.58), location: 1.0),
