...
/// tvOS sign-in. Noiro-account sign-in (QR pairing) is the PRIMARY path, so you sign into the account that
...
struct LoginView: View {
    enum Mode { case noiro, stremioLink, stremioPassword }
    ...
    init(account: StremioAccount, initialMode: Mode = .noiro) {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: Theme.Space.lg) {
                ...
                switch mode {
                case .noiro:           NoiroAccountJoinerView(onSignedIn: { dismiss() })
        ...
        // The Stremio paths finish through StremioAccount.isSignedIn; the Noiro path dismisses itself via
        ...
        .onReceive(account.$isSignedIn) { signedIn in
            ...
            // add-ons, snapshot the full descriptor set into the Noiro account doc so the account OWNS them
    ...
    private var headline: String {
        switch mode {
        case .noiro:           return "Approve this device from your passwordless Vortexo Studio account."
        ...
        case .stremioLink:
            HStack(spacing: Theme.Space.sm) {
                Button { mode = .stremioPassword } label: { Text("Use password").frame(width: 200) }
                    .buttonStyle(ChipButtonStyle())
                Button { mode = .noiro } label: { Text("Pair Noiro Sync").frame(width: 240) }
                    .buttonStyle(ChipButtonStyle())
            }
