...
enum ScrollToTopAnchor {
    static let id = "noiro.scrollToTop.anchor"
}
...
enum TabScrollKeys {
    static let home = "noiro.tab.home"
    static let discover = "noiro.tab.discover"
    static let live = "noiro.tab.live"
    static let library = "noiro.tab.library"
    static let search = "noiro.tab.search"
    static let addons = "noiro.tab.addons"
    static let settings = "noiro.tab.settings"
}
...
struct ScrollToTopOnBump: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ScrollViewReader { proxy in
            #if os(tvOS)
            content.onChange(of: signal.token(key)) { _, _ in
