...
/// Fleet WATCH SIGNAL (D9): the anonymized "this fleet watched X today" ping that powers Noiro's OWN
/// Trending / Popular / Most-Watched rows (a real data moat instead of TMDB-only rankings).
///
/// Contract (matches the `vortexo.app/api/noiro/v1/edge/watch` worker, which is created + deployed separately and is owner-gated,
/// so this client MUST fail soft until it exists):
///   POST https://vortexo.app/api/noiro/v1/edge/watch/ping   body: { "content_id": "tt1234567", "day": "2026-07-02" }
...
/// The worker adds a daily-salted IP-hash witness dedup of its own (like noiro-sources); NO user id / token /
...
/// pool switch) AND edge-auth signed (`NoiroEdgeAuth`, observe-safe). One ping per (content_id, day) PER DEVICE
...
enum WatchSignalClient {
    ...
    /// owner-gated and created separately; keep the host in sync with `NoiroEdgeAuth.gatedHosts`.
    private static let baseURL = "https://vortexo.app/api/noiro/v1/edge/watch"
    ...
    private static let sentKey = "noiro.watchSignalSent"
    private static let sentCap = 500

    /// UserDefaults key for this install's stable, non-PII dedupe id sent in the `X-Noiro-Watcher` header. Minted
    ...
    private static let watcherIdKey = "noiro.watchSignalWatcherId"
    ...
    static func ping(contentId: String, type: String) {
        ...
        Task.detached(priority: .background) {
            ...
            req.setValue(watcher, forHTTPHeaderField: "X-Noiro-Watcher")   // per-device dedupe (hashed worker-side; no PII)
            req.httpBody = body
            NoiroEdgeAuth.sign(&req)   // gated host (vortexo.app/api/noiro/v1/edge/watch): stamp X-Noiro-Ts / X-Noiro-Kid / X-Noiro-Sig
