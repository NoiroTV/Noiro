...
/// Talks to the Noiro sync service (a Cloudflare Worker at `vortexo.app/api/noiro/v1/edge/api`).
///
...
/// (see `BackupCrypto` / `NoiroAccount`). Conflict policy is last-writer-wins by `version`
...
///   PUT  /v1/backup              <- push the sealed blob   (header: X-Noiro-Account)
///   GET  /v1/backup              -> pull the latest sealed blob
struct NoiroSyncClient: Sendable {
    var baseURL: URL?
    ...
    private func baseRequest(_ path: String, method: String, account: String?) throws -> URLRequest {
        ...
        if let account { req.setValue(account, forHTTPHeaderField: "X-Noiro-Account") }
        return req
    }
