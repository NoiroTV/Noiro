...
/// ride SettingsBackup / Noiro-account sync to the user's other devices. Shared by the tvOS SearchView
...
enum SearchHistoryStore {
    ...
    private static func storageKey(_ profileID: UUID?) -> String {
        "noiro.searchHistory.\(profileID?.uuidString ?? "default")"
    }
    ...
    /// All recent terms for a given profile (mirrors `load`, used by Noiro-account sync to read every
