...
enum ProfileSync {
    ...
    private static let log = Logger(subsystem: "com.elvissalihovic.noiro.app", category: "profilesync")
    ...
    private static func repairPoisonedLibrary(authKey: String) async -> [String: String] {
        ...
        for item in items {
            ...
            await putLibraryItem(sanitizedDoc(id: id, name: item["name"] as? String ?? "Noiro"),
                                 authKey: authKey)
            repaired += 1
        }
        if repaired > 0 {
    ...
    private static func repairDoneKey(_ authKey: String) -> String {
        ...
        return "noiro.libraryRepairComplete.\(digest)"
    }
