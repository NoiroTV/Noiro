...
/// call sites in NoiroApp / iOSSettingsView resolve to the right implementation per platform
...
enum NodeServer {
    ...
    private static let queue = DispatchQueue(label: "com.elvissalihovic.noiro.mac.nodeserver")
    ...
    private static let shareKey = "noiro.mac.shareServerOnLAN"
    ...
    static func startIfNeeded() {
        queue.async {
            ...
            guard let nodeBin = Bundle.main.path(forResource: "node-darwin-arm64", ofType: nil) else {
                NSLog("Noiro: node binary not found in bundle, streaming server disabled")
                return
            }
            guard let serverJs = Bundle.main.path(forResource: "server", ofType: "js") else {
                NSLog("Noiro: server.js not found in bundle, streaming server disabled")
                return
            }
    ...
    /// on app termination (see NoiroApp's macOS app-delegate hook) so the child never gets
    ...
    static func stop() {
        queue.sync {
            ...
            proc.waitUntilExit()
            NSLog("Noiro: node streaming server stopped on app termination")
        }
    }
    ...
    private static var serverHome: String {
        ...
        return support.appendingPathComponent("Noiro").path
    }
    ...
    private static func reclaimStalePort() {
        for pid in listeners(onPort: 11470) where isOurNodeServer(pid) {
            NSLog("Noiro: reclaiming port 11470 from a stale node server (pid \(pid))")
            kill(pid, SIGTERM)
            let deadline = Date().addingTimeInterval(terminateGrace)
    ...
    private static func spawn(nodeBin: String, scriptPath: String) {
        ...
        if let bins = ffmpegBinaries() {
            ...
            NSLog("Noiro: ffmpeg for transcoding: \(bins.ffmpeg) (VideoToolbox hw-accel)")
        } else {
            NSLog("Noiro: no ffmpeg found; HLS transcoding disabled (install via `brew install ffmpeg`)")
        }
        ...
        proc.terminationHandler = { p in
            queue.async {
                ...
                NSLog("Noiro: node server exited rc=\(p.terminationStatus)")
            }
        }

        do {
            NSLog("Noiro: starting node streaming server (bin=\(nodeBin), HOME=\(home))")
            try proc.run()
            process = proc
        } catch {
            started = false
            NSLog("Noiro: failed to launch node server: \(error)")
        }
    }
