...
///  - Launch with the environment variable NOIRO_PROBE=1 (Xcode scheme or a Terminal-launched run).
///  - Flip the "Diagnostic logging" toggle in Settings, which writes UserDefaults key
///    "noiro.probeLogging" and can enable it at runtime without a relaunch.
///
...
enum VXProbe {
    ...
    private static let envEnabled: Bool = ProcessInfo.processInfo.environment["NOIRO_PROBE"] == "1"

    /// UserDefaults key the Settings "Diagnostic logging" toggle binds to.
    static let defaultsKey = "noiro.probeLogging"
...
final class VXProbeFileLog {
    ...
    private let queue = DispatchQueue(label: "noiro.vxprobe.filelog", qos: .utility)

    /// caches/noiro-diag.log in the app container. Computed once; the caches dir always exists.
    let fileURL: URL
    ...
    private init() {
        ...
        fileURL = caches.appendingPathComponent("noiro-diag.log")
    }
...
enum VXProbeHeartbeat {
    private static let queue = DispatchQueue(label: "noiro.vxprobe.heartbeat", qos: .utility)
