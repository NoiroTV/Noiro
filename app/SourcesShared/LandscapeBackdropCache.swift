...
actor LandscapeBackdropCache {
    ...
    /// Read a logo that a visible cinematic card has already resolved, without starting another TMDB
    /// request. Person-page focus previews use this to match Home's clearlogo while remaining network-idle.
    static func cachedLogo(id: String) async -> String? {
        await shared.cachedLogoURL(id: id)
    }

    private func cachedLogoURL(id: String) -> String? { cache[id]?.logo }
