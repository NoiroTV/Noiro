...
enum Theme {
    ...
    enum Space {
        ...
        // Vortexo-parity rail metrics (see UIScale.swift in the Vortexo reference project).
        // Horizontal inset for catalog rails — tighter than the 60pt screenEdge used by chrome,
        // so rails read as a denser band of art (Vortexo rowHorizontalPadding = 48).
        static let railHorizontal: CGFloat = 48
        /// The compact overlay rail occupies the physical leading edge, so tvOS catalog artwork
        /// can sit nearer to it without changing the rail itself.
        static var catalogHorizontal: CGFloat {
            #if os(tvOS)
            24
            #else
            railHorizontal
        ...
        // Gap between cards in a horizontal rail (Vortexo rowItemSpacing = 40). Wider than
        // `lg` (32) so each card breathes; the focus halo still fits thanks to scrollClipDisabled.
        static var railItemGap: CGFloat {
            #if os(tvOS)
            // The compact left rail uses a tighter card rhythm so four complete landscape
            // cards leave a clearer preview of card five. Card dimensions remain unchanged.
            24
            #else
            40
            #endif
        ...
        static var catalogChromeHorizontal: CGFloat {
            #if os(tvOS)
            36
            #else
            screenEdge
    ...
    enum Typography {
        ...
        // Vortexo-parity rail header title (sectionTitleSize = 28, bold). Slightly smaller and
        // heavier than `sectionTitle` so a rail reads as a tighter band above its cards.
        static var railTitle: Font   { .system(size: scaled(28), weight: .bold) }
...
extension View {
    ...
    /// Vortexo-parity rail header title: the tighter, heavier 28pt bold a catalog rail sits under.
    func railTitleStyle() -> some View {
        font(Theme.Typography.railTitle).tracking(-0.3).foregroundStyle(Theme.Palette.textPrimary)
    }
...
private struct CardFocusContent: View {
    ...
    var body: some View {
        ...
        // A scale of exactly 1 is the opt-in used by pixel-fixed rails (for example the entered Genre
        // destination). Keep their depth shadow centred on the same coordinates too; otherwise the old
        // focused y-offset (7 -> 10) still makes a constant-size card look as if the row jumped vertically.
        let fixesFocusGeometry = abs(scale - 1) < 0.001
        let label = configuration.label
            ...
            .shadow(color: .black.opacity(active ? 0.45 : 0.32),
                    radius: fixesFocusGeometry ? 12 : (active ? 16 : 12),
                    x: 0,
                    y: fixesFocusGeometry ? 7 : (active ? 10 : 7))
...
struct CircleIconButtonStyle: ButtonStyle {
    ...
    private struct Content: View {
        ...
        #endif
        var body: some View {
            // `active` is hover state, which only exists on macOS. Deriving the hover-dependent
            // values inside the same #if keeps the other platforms free of branches the compiler
            // knows are unreachable.
            #if os(macOS)
            let active = isHovered
            let hoverScale: CGFloat = active && !reduceMotion ? 1.06 : 1
            let glowOpacity: Double = active ? 0.35 : 0
            #else
            let hoverScale: CGFloat = 1
            let glowOpacity: Double = 0
            #endif
            let label = configuration.label
                .scaleEffect(configuration.isPressed ? 0.92 : hoverScale)
                .shadow(color: Theme.Palette.accent.opacity(glowOpacity), radius: 14, y: 0)
...
struct HeroPlayButtonStyle: ButtonStyle {
    ...
    private struct Content: View {
        ...
        #endif
        var body: some View {
            // See CircleIconButtonStyle: hover exists only on macOS, so the hover-dependent
            // values are resolved inside the #if rather than by a ternary on a known constant.
            #if os(macOS)
            let active = isHovered
            let fill = active ? Theme.Palette.accentBright : Theme.Palette.accent
            let glowOpacity: Double = active ? 0.55 : 0.30
            #else
            let fill = Theme.Palette.accent
            let glowOpacity: Double = 0.30
            #endif
            let label = configuration.label
                ...
                .shadow(color: Theme.Palette.accent.opacity(glowOpacity), radius: 22, y: 10)
                .animation(reduceMotion ? nil : Theme.Motion.state, value: configuration.isPressed)
            #if os(macOS)
            return label.onHover { isHovered = $0 }
            #else
...
// MARK: - On-card metadata overlay (Vortexo parity)

/// The title plus an optional "Genre · ★ rating" footer rendered OVER the poster's bottom edge on a
/// darkening scrim, instead of a separate title line beneath the card. This is the visible majority of
/// Vortexo's `MediaPosterCard` overlay layer (title + rating + genre); the Plex-only 4K/DV/HDR/audio
/// badge pills are out of scope here because Stremio-core does not expose stream quality at browse time.
///
/// Gated by `CatalogPreferences.onCardMeta`. When the overlay is shown, `PosterCard` / `PosterCardiOS`
/// suppress their legacy below-card title so the two never duplicate. All three metadata fields are
/// optional — a card with only a title shows just the title + scrim, matching how Vortexo degrades on
/// sparse metadata. Lives here (Theme.swift) so both tvOS and iOS render the same overlay.
struct CardMetaOverlay: View {
    let title: String
    var rating: String? = nil      // IMDb rating, e.g. "7.6"
    var year: String? = nil        // releaseInfo / year
    var genres: [String]? = nil    // shown as a " · "-joined footer (prefix 2)
    var landscape: Bool = false    // landscape cards get a slightly larger title

    /// tvOS is a 10-foot screen; iOS/Mac is arm's length. Mirror Theme.Typography's platform factor so
    /// the overlay text matches the card title size on each platform instead of being tvOS-large on a phone.
    ...
    private var titleSize: CGFloat { (landscape ? 22 : 19) * sizeFactor }
    private var footerSize: CGFloat { (landscape ? 17 : 15) * sizeFactor }
    private var inset: CGFloat { 12 * sizeFactor }

    private var footerParts: [String] {
        var p: [String] = []
        if let genres, !genres.isEmpty { p.append(genres.prefix(2).joined(separator: " · ")) }
        if let rating, !rating.isEmpty { p.append("★ \(rating)") }
        return p
    }

    var body: some View {
        ZStack(alignment: .bottomLeading) {
            // Scrim: clear at top, opaque near the bottom so the text clears contrast over any art.
            LinearGradient(
                stops: [
            ...
            VStack(alignment: .leading, spacing: 4 * sizeFactor) {
                Text(title)
