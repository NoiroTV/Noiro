...
/// tiny HTTP server bound to the LAN on an ephemeral port that serves the current `noiro-diag.log` as a
...
final class VXDiagExport {
    ...
    private let queue = DispatchQueue(label: "com.elvissalihovic.noiro.vxdiagexport")
    ...
    private let listenerQueue = DispatchQueue(label: "com.elvissalihovic.noiro.vxdiagexport.listener")
    ...
    private func serve(_ connection: NWConnection) {
        ...
        Content-Disposition: attachment; filename="noiro-diag.log"\r
...
extension VXDiagExport {
    ...
    /// copy the current rolling `noiro-diag.log` into the user's Downloads folder and reveal it in Finder so
    ...
    @MainActor func revealInFinder() -> String? {
        ...
        let dest = destDir.appendingPathComponent("noiro-diag.log")
