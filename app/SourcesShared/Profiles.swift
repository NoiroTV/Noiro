...
struct UserProfile: Codable, Identifiable, Equatable {
    ...
    /// Family head (the account owner) may edit this profile from the vortexo.app dashboard WITHOUT its
    /// PIN. Per-secondary, set from the dashboard. Default false. Governs WEB edit permission only,
    /// not the on-device profile-switch PIN gate (see noiro-dashboard-profile-mgmt-design).
...
final class ProfileStore: ObservableObject {
    ...
    private static let listKey = "noiro.profiles"
    private static let activeKey = "noiro.profiles.active"
    private static let modifiedKey = "noiro.profiles.modified"
    /// Durable cross-device delete tombstones: profile ids the user has DELETED. The app owns this set
    /// (it lives in doc.noiro.deletedProfiles, the app's namespace) so a deleted profile can never be
    /// resurrected by a peer device's union-merge or a stale pre-delete cloud blob. The owner id is
    /// never tombstoned. See [[noiro-2026-06-25-rootcause-investigation]] section 2.
    private static let deletedKey = "noiro.profiles.deleted"
    private static func watchCacheKey(_ id: UUID) -> String { "noiro.profiles.watch." + id.uuidString }
    /// The pre-profiles single-account Keychain slot; shared profiles keep using it.
    static let primaryTokenAccount = "noiro.authKey"
    ...
    static let activeDisabledAddonsKey = "noiro.profile.disabledAddons"
    ...
    static let activeKidsKey = "noiro.profile.isKids"
    ...
    /// doc.noiro.deletedProfiles by NoiroDocumentSyncManager, and subtracted from every roster union so a
    ...
    /// Apply web-authored profile edits (the vortexo.app dashboard writes doc.profileEdits; the app reads it
    ...
    /// Union-safe: a profile absent from the edits is left untouched. See [[noiro-dashboard-profile-mgmt-design]].
        ...
        if let roster = edits["roster"] as? [[String: Any]] {
            for e in roster {
                ...
                if var p = existing {
                    ...
                    // the add-on list (doc.noiro.addons), so the dashboard only toggles which ones are off
                    // for this profile; that set rides the profileEdits channel, never doc.noiro.
    ...
    private func applyPlayback(_ profile: UserProfile) {
        ...
        if let p = profile.playback {
            ...
            if let order = p.sourceTypeOrder {
                d.set(order.joined(separator: ","), forKey: "noiro.streaming.sourceTypeOrder")
            }
            if let addon = p.useAddonOrder {
                d.set(addon, forKey: "noiro.streaming.useAddonOrder")
            }
            ...
            for key in [TrackPreferences.Key.audio, TrackPreferences.Key.subtitle,
                        ...
                        "noiro.streaming.sourceTypeOrder", "noiro.streaming.useAddonOrder"] {
    ...
    private func migrateFromSingleAccount() {
        let email = UserDefaults.standard.string(forKey: "noiro.email")
    ...
    private func persist(touch: Bool = true) {
        ...
        if touch {
            ...
            // global didChangeNotification observer in NoiroDocumentSyncManager, arming hasPendingPush even though this
            ...
            NoiroDocumentSyncManager.suppressHousekeeping(writeRosterAndActive)
        }
    }
    ...
    /// Fold incoming tombstones (from another device's doc.noiro.deletedProfiles) into the local set,
    ...
    /// Apply the LOCAL delete tombstones to the live roster. Called by NoiroDocumentSyncManager after EVERY sync pull,
    ...
    /// Apply a roster that a Noiro account sync just restored into UserDefaults (SettingsBackup.restore)
    ...
    private func normalizeOwner() {
        ...
        let signedInEmail = UserDefaults.standard.string(forKey: "noiro.email")
    ...
    /// local roster can never shrink the cloud (see NoiroDocumentSyncManager.syncUp).
    ///
    ...
    func refreshWatchFromServer() {
        ...
        Task { [weak self] in
            ...
            // Re-capture weakly here rather than reading the enclosing Task's `self`: referencing a
            // captured variable from a second concurrently-executing closure is an error under Swift 6.
    ...
    private func schedulePushWatch() {
        ...
        // so nudging the Noiro E2E sync on EACH write (the old immediate requestSyncSoon) stormed the
        ...
        // sync 3s after the last write. requestSyncSoon no-ops when not signed into a Noiro account.
        let profile = active
        let snapshot = watch
        pushWatchTask = Task { @MainActor in
            try? await Task.sleep(for: .seconds(3))
            guard !Task.isCancelled else { return }
            NoiroDocumentSyncManager.shared.requestSyncSoon()
    ...
    /// The stored watch overlay for ANY profile, read straight from its cache, so the Noiro sync can
