...
///      `vortexo.app/api/noiro/v1/edge/trickplay/tp/{key}`. On a hit, download the sprite-sheet + WEBVTT index ONCE and serve
...
enum CommunityTrickplay {
    ...
    /// *.vortexo.app host, else the baked default), so the owner can repoint it with no app update. Baked default
    /// `https://vortexo.app/api/noiro/v1/edge/trickplay` == the shipping value; a null/invalid remote endpoint keeps that default.
    ...
    /// the player already uses; the 0.4 rename seam (`stremiox.` -> `noiro.`) maps it via SettingsBackup.
    static let settingKey = "noiro.communityTrickplay"
    ...
    private static let tmdb2ttDefaultsKey = "noiro.trickplay.tmdb2tt"
    ...
    ///
    /// Resolution is our own keyless TMDB edge (`vortexo.app/api/noiro/v1/edge/catalogs/3/{movie|tv}/{id}/external_ids`, edge-signed,
    ...
    static func resolveIMDbID(rawId: String, seriesHint: Bool) async -> String? {
        ...
        // Scoped `withLock` rather than bare lock/unlock: the latter is unavailable from an async
        // context, because a suspension between the two would hold the lock across an await.
        let cached = tmdb2ttLock.withLock { tmdb2ttCache[cacheKey] }
        ...
        for media in order {
            if let tt = await fetchExternalIMDbID(media: media, tmdbID: numeric) {
                let snapshot = tmdb2ttLock.withLock { () -> [String: String] in
    ...
    static func fetch(key: String) async -> Sheet? {
        ...
        do {
            ...
            NoiroEdgeAuth.sign(&req)   // gated host (vortexo.app/api/noiro/v1/edge/trickplay /tp/<key>): stamp X-Noiro-Ts / X-Noiro-Sig
            let (data, resp) = try await URLSession.shared.data(for: req)
            guard let http = resp as? HTTPURLResponse, http.statusCode == 200 else { return nil }
            // NOTE: the sprite (meta.sprite) is on the R2 public asset host, NOT a gated *.vortexo.app
    ...
    private static func post(key: String, sprite: Data, vtt: String, meta: [String: Any]) async -> Bool {
        ...
        let boundary = "noiro-tp-\(UUID().uuidString)"
        var body = Data()
        ...
        req.httpBody = body
        NoiroEdgeAuth.sign(&req)   // gated host (vortexo.app/api/noiro/v1/edge/trickplay /tp/<key> POST): stamp X-Noiro-Ts / X-Noiro-Sig
        do {
            let (data, resp) = try await URLSession.shared.data(for: req)
