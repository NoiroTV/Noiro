...
enum TMDBClient {
    ...
    static var trailerLanguageOverride: String? {
        let v = UserDefaults.standard.string(forKey: "noiro.trailerLanguage")
    ...
    /// posters). Uses the keyless catalog edge when no personal TMDB key is configured; accepts an IMDb id
    /// (tt..., via /find) or a `tmdb:[type:]id`. Either URL
    ...
    /// (`get` -> `vortexo.app/api/noiro/v1/edge/catalogs/3` when the user has no key, TMDB direct when they do), so it needs no user
    ...
    // MARK: - Person details, combined credits, and photos

    /// A single localized person request with TMDB's combined credits and profile images appended. The
    /// result is cached by person + app language for this app session. TMDB often leaves translated
    /// biographies empty, so only that field falls back to en-US while localized title data stays intact.
    static func personProfile(personID: Int, language: String = LocalizedMetadataLanguage.current) async -> PersonProfile? {
        guard personID > 0 else { return nil }
        if let cached = await PersonProfileSessionCache.shared.profile(personID: personID, language: language) {
            return cached
        }

        func tmdbLanguage(_ value: String) -> String {
        ...
        let key = ApiKeys.effectiveTMDBKey()
        let requestedLanguage = tmdbLanguage(language)
        let encodedLanguage = requestedLanguage.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "en-US"
        let path = "/person/\(personID)?api_key=\(key)&language=\(encodedLanguage)&append_to_response=combined_credits,images"
        guard let payload = await get(path), let name = payload["name"] as? String, !name.isEmpty else { return nil }

        var biography = (payload["biography"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if biography.isEmpty, !requestedLanguage.lowercased().hasPrefix("en"),
           let english = await get("/person/\(personID)?api_key=\(key)&language=en-US"),
           let fallback = english["biography"] as? String {
            biography = fallback.trimmingCharacters(in: .whitespacesAndNewlines)
        }
        ...
        func image(_ path: String?, size: String) -> String? {
            guard let path, !path.isEmpty else { return nil }
            return "https://image.tmdb.org/t/p/\(size)\(path)"
        }
        func parseCredit(_ entry: [String: Any], kind: PersonCreditKind) -> PersonMediaCredit? {
            guard (entry["adult"] as? Bool) != true,
                  let tmdbID = integer(entry["id"]),
                  let mediaType = entry["media_type"] as? String,
                  mediaType == "movie" || mediaType == "tv" else { return nil }
            let title = ((mediaType == "tv" ? entry["name"] : entry["title"]) as? String)
                ?? ((mediaType == "tv" ? entry["original_name"] : entry["original_title"]) as? String)
            guard let title, !title.isEmpty else { return nil }
            let role: String?
            switch kind {
            case .cast: role = entry["character"] as? String
            case .crew: role = (entry["job"] as? String) ?? (entry["department"] as? String)
            }
            let overview = (entry["overview"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
            return PersonMediaCredit(
                tmdbID: tmdbID,
                mediaType: mediaType,
                title: title,
                posterURL: image(entry["poster_path"] as? String, size: "w500"),
                backdropURL: image(entry["backdrop_path"] as? String, size: "w1280"),
                overview: overview?.isEmpty == true ? nil : overview,
                releaseDate: (entry[mediaType == "tv" ? "first_air_date" : "release_date"] as? String).flatMap { $0.isEmpty ? nil : $0 },
                voteAverage: double(entry["vote_average"]),
                popularity: double(entry["popularity"]) ?? 0,
                role: role?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == true ? nil : role,
                department: entry["department"] as? String,
                job: entry["job"] as? String,
                kind: kind
            )
        }

        let combined = payload["combined_credits"] as? [String: Any]
        let cast = (combined?["cast"] as? [[String: Any]] ?? []).compactMap { parseCredit($0, kind: .cast) }
        let crew = (combined?["crew"] as? [[String: Any]] ?? []).compactMap { parseCredit($0, kind: .crew) }

        let rawProfiles = (payload["images"] as? [String: Any])?["profiles"] as? [[String: Any]] ?? []
        let photos: [PersonPhoto] = rawProfiles.compactMap { entry in
            guard let filePath = entry["file_path"] as? String,
                  let full = image(filePath, size: "original"),
                  let thumb = image(filePath, size: "w500") else { return nil }
            return PersonPhoto(path: filePath, url: full, thumbnailURL: thumb,
                               width: integer(entry["width"]), height: integer(entry["height"]),
                               aspectRatio: double(entry["aspect_ratio"]))
        }.sorted { lhs, rhs in
            let lh = lhs.height ?? 0, rh = rhs.height ?? 0
            return lh == rh ? (lhs.width ?? 0) > (rhs.width ?? 0) : lh > rh
        }

        let credits = cast + crew
        let heroBackdrop = credits.filter { $0.backdropURL != nil }
            .max { lhs, rhs in lhs.popularity < rhs.popularity }?.backdropURL
        let profile = PersonProfile(
            id: personID,
            name: name,
            biography: biography,
            birthday: (payload["birthday"] as? String).flatMap { $0.isEmpty ? nil : $0 },
            deathday: (payload["deathday"] as? String).flatMap { $0.isEmpty ? nil : $0 },
            placeOfBirth: (payload["place_of_birth"] as? String).flatMap { $0.isEmpty ? nil : $0 },
            knownForDepartment: (payload["known_for_department"] as? String).flatMap { $0.isEmpty ? nil : $0 },
            alsoKnownAs: (payload["also_known_as"] as? [String] ?? []).filter { !$0.isEmpty },
            profileURL: image(payload["profile_path"] as? String, size: "h632"),
            credits: credits,
            photos: photos,
            heroBackdropURL: heroBackdrop
        )
        await PersonProfileSessionCache.shared.store(profile, language: language)
        return profile
    }
    ...
    /// One behind-the-camera credit for the detail Crew rail. Movies expose `job`; aggregate TV credits
    /// expose `jobs[]`, so `role` is normalized before it reaches the UI.
    ...
    static func credits(imdbID: String, type: String) async -> CreditsResult? {
        ...
        guard let payload = await get(path) else {
            return overview == nil ? nil : CreditsResult(cast: [], crew: [], overview: overview)
        }
        ...
        let rawCrew = payload["crew"] as? [[String: Any]] ?? []
        var seenCrew = Set<Int>()
        let crew: [CrewMember] = rawCrew.compactMap { entry in
            guard let name = entry["name"] as? String, !name.isEmpty else { return nil }
            let personID = entry["id"] as? Int ?? name.hashValue
            guard seenCrew.insert(personID).inserted else { return nil }
            let aggregateJobs = (entry["jobs"] as? [[String: Any]])?
                .compactMap { $0["job"] as? String }
                .filter { !$0.isEmpty }
            let role = (entry["job"] as? String)
                ?? aggregateJobs?.prefix(2).joined(separator: " · ")
                ?? (entry["department"] as? String)
            let profile = (entry["profile_path"] as? String).map { "https://image.tmdb.org/t/p/w185\($0)" }
            return CrewMember(id: personID, name: name,
                              role: role.flatMap { $0.isEmpty ? nil : $0 },
                              profileURL: profile)
        }
        return CreditsResult(cast: members, crew: crew, overview: overview)
    }

    /// Noiro's keyless catalog edge: a cached, app-gated TMDB proxy that injects OUR key server-side, so
    /// users with no TMDB key still get the hub. Path here mirrors TMDB's /3 namespace. Sourced from the
    /// RemoteConfig `endpoints.catalogs` dial (validated https + *.vortexo.app host, else baked default), so the
    /// owner can repoint the catalog edge with no app update. Baked default `https://vortexo.app/api/noiro/v1/edge/catalogs/3` ==
    ...
    ///                          the gate attributes the call to Noiro. The bundled key is stripped from the
    ...
    /// GET + decode JSON. Signs the request via `NoiroEdgeAuth` when `sign` (only our edge host is gated;
    ...
    private static func fetchJSON(_ url: URL?, sign: Bool) async -> [String: Any]? {
        ...
        if sign { NoiroEdgeAuth.sign(&req) }
        do {
    ...
    /// other call here (vortexo.app/api/noiro/v1/edge/catalogs caches external_ids ~24h with SWR, so a warm title resolves in a few
