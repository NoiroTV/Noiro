...
final class CoreBridge: ObservableObject {
    ...
    func start() {
        ...
        // Bring up RemoteConfig once, on the single shared launch path every Apple target runs (NoiroTV,
        // NoiroTV, NoiroiOSNative, NoiroMac, Noiro). Synchronously loads last-good cached JSON into the
    ...
    /// dashboard's `doc.webAddonRemovals` + `doc.noiro.deletedAddons` on syncDown) and is NOT
    ...
    /// removal SYNCS: `noiroSummary` pushes the set into `doc.noiro.deletedAddons` and subtracts it from
    /// the `doc.noiro.addons` UNION, and `syncDown` re-applies it on peers (mirrors `deletedProfiles`).
    ...
    func uninstallAddon(_ descriptor: CoreDescriptor, tombstone: Bool = true) {
        ...
        if tombstone, !descriptor.isOfficial, !descriptor.isProtected {
            ...
            // the tombstone lands in doc.noiro.deletedAddons right away and peers pick it up on their next pull.
            let removedUrl = descriptor.transportUrl
            Task {
                let ok = await NoiroDocumentSyncManager.shared.pushThisDevice()
        ...
        // live, we keep the tombstone (the Noiro-view removal) and rely on refreshAddons to suppress the
        ...
        if let raw, !tombstone || pushDeletionToStremio {
            ...
            // add-on disappears from the Noiro view immediately, while the engine (and the user's Stremio
    ...
    /// descriptor set into the Noiro account doc for network-free re-hydration. Account/engine add-on
    ...
    /// the user's add-on PRIORITY: a reorder on one device reaches the others via doc.noiro.addons.
    func rawAddonDescriptorsOrdered() -> [[String: Any]] {
        addons.compactMap { rawAddonsByUrl[$0.transportUrl] }
    }

    /// Install the Noiro account's owned add-ons back INTO the engine, but ONLY descriptors the engine
    ...
    /// A lowercase-key mismatch silently no-ops in the engine, so `NoiroOwnedAddon.installDescriptor`
    ...
    func hydrateAddonsFromAccount(_ owned: [NoiroOwnedAddon]) {
    ...
    private func bootstrapAuth() {
        if isLoggedIn() {
            refreshFromAPI()
            // Noiro-first (account-owns-everything): hydrate the Noiro account's owned add-ons into the engine
            // on EVERY launch, not only when degraded, so doc.noiro.addons is the source of truth and a still-
        ...
        guard let key = Keychain.string(activeTokenAccount), !key.isEmpty else {
            ...
            // Account-owns-everything: with no Stremio session, hydrate the Noiro account's owned add-ons
    ...
    private func scheduleSessionRepair() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 14) { [weak self] in
            ...
            // Account-owns-everything: hydrate the Noiro account's owned add-ons + recover the owner
            ...
            Task { @MainActor in
                ...
                if hasStremioToken, let key {
                    ...
                    NSLog("[CoreBridge] degraded session with no Stremio token — recovered from the Noiro account doc")
                    self.loadBoard()
                }
            }
        }
    ...
    /// board here, see [[noiro-engine-needs]] #7 IPTV + the source-registry.)
    ...
    func removeFromLibrary(id: String) {
        ...
        // uninstallAddon's AddonTombstones.tombstone: noiroSummary pushes the set into
        // doc.noiro.deletedLibrary and SUBTRACTS it from the doc.noiro.library UNION, and syncDown re-folds
        ...
        // so the tombstone lands in doc.noiro.deletedLibrary right away, mirroring uninstallAddon.
        Task {
            let ok = await NoiroDocumentSyncManager.shared.pushThisDevice()
    ...
    /// When Stremio Continue Watching mirroring is enabled, however, a completed series episode must remain:
    /// Stremio commonly leaves the library state parked on that episode and derives the next unwatched episode
    /// from metadata. Removing it here made the whole show disappear instead of offering the next episode.
    static func pruneFinished(_ items: [CoreCWItem]) -> [CoreCWItem] {
        return items.filter { item in
            !item.isFinished || item.isMirroredCompletedSeries
        }
    }
    ...
    func finishedWatching(libraryId: String) {
        ...
        Task { _ = await NoiroDocumentSyncManager.shared.pushThisDevice() }
    }
