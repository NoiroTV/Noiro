...
//
//  Live-repro'd on Noiro 0.3.11 b154 / macOS 26.5.1 (crash 0629BAE4-...): play a
...
//  one window toolbar is hidden and unused — NoiroApp sets
...
//
//  WHY OBJECTIVE-C++ AND `catch (...)`, NOT `@catch` — the NoiroMac binary already
...
static VXInsertIMP noiro_orig_insert_public = nullptr;   // insertItemWithItemIdentifier:atIndex:
static VXInsertIMP noiro_orig_insert_user   = nullptr;   // _userInsertItemWithItemIdentifier:atIndex:
static VXCoreInsertIMP noiro_orig_insert_core = nullptr; // _insertItem:atIndex:notify…
static SEL noiro_sel_public = nullptr;
static SEL noiro_sel_user   = nullptr;
static std::atomic<long> noiro_swallow_count{0};
...
static thread_local int noiro_guard_depth = 0;
...
struct VXDepthGuard {
    VXDepthGuard() { ++noiro_guard_depth; }
    ~VXDepthGuard() { --noiro_guard_depth; }
};

static void noiro_log_swallow(id detail) {
    long n = noiro_swallow_count.fetch_add(1) + 1;
    if (n <= 5 || (n % 100) == 0) {
        NSLog(@"[Noiro] guarded NSToolbar insert throw #%ld (%@)", n, detail);
    }
}

// Wrapper for the two outer entries (dispatch to the right original by _cmd).
static void noiro_insert_guarded(id self, SEL _cmd, id identifier, NSInteger index) {
    VXInsertIMP orig = (_cmd == noiro_sel_user) ? noiro_orig_insert_user
                                                : noiro_orig_insert_public;
    if (noiro_guard_depth > 0) {                 // nested: let the outer frame catch
...
static void noiro_core_insert_guarded(id self, SEL _cmd, id item, NSInteger index,
                                      BOOL notifyDelegate, BOOL notifyView, BOOL notifyFamily) {
    if (noiro_guard_depth > 0) {
        if (noiro_orig_insert_core)
            noiro_orig_insert_core(self, _cmd, item, index, notifyDelegate, notifyView, notifyFamily);
        return;
    }
    VXDepthGuard depth;
    try {
        ...
        noiro_log_swallow(item);
    }
}

static bool noiro_install(SEL sel, IMP replacement, void *origSlot) {
...
extern "C" void NoiroInstallToolbarCrashGuard(void) {
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        noiro_sel_public = NSSelectorFromString(@"insertItemWithItemIdentifier:atIndex:");
        noiro_sel_user   = NSSelectorFromString(@"_userInsertItemWithItemIdentifier:atIndex:");
        SEL selCore = NSSelectorFromString(
            @"_insertItem:atIndex:notifyDelegate:notifyView:notifyFamilyAndUpdateDefaults:");
        bool pub  = noiro_install(noiro_sel_public, (IMP)noiro_insert_guarded,      &noiro_orig_insert_public);
        bool user = noiro_install(noiro_sel_user,   (IMP)noiro_insert_guarded,      &noiro_orig_insert_user);
        bool core = noiro_install(selCore,          (IMP)noiro_core_insert_guarded, &noiro_orig_insert_core);
        NSLog(@"[Noiro] toolbar crash guard installed (public=%d, user=%d, core=%d).", pub, user, core);
    });
}
