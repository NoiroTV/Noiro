...
final class ScrubThumbnailsStore: ObservableObject {
    ...
    private func pushUpload(key: String, imdb: String) {
        ...
        Task.detached(priority: .utility) { [weak self] in
            ...
            await MainActor.run { [weak self] in self?.uploadInFlight = false }
        }
    }
...
private final class LocalTrickplayFrameCache {
    ...
    private let ioQueue = DispatchQueue(label: "com.elvissalihovic.noiro.trickplay.localcache", qos: .utility)
