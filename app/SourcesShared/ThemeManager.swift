...
final class ThemeManager: ObservableObject {
    ...
    private static let accentKey = "noiro.theme.accent"
    private static let oledKey = "noiro.theme.oled"
    private static let textScaleKey = "noiro.theme.textScale"
    private static let previousProductAccentID = "vor" + "tx"
    private static let legacyGoldAccentID = "legacy-gold"

    private init() {
        let defaults = UserDefaults.standard
        // One-way local migration preserves the user's appearance while moving
        // the externally-visible configuration namespace to Noiro.
        if defaults.object(forKey: Self.accentKey) == nil,
           let legacy = defaults.string(forKey: "stremiox.theme.accent") {
            defaults.set(legacy == Self.previousProductAccentID ? Self.legacyGoldAccentID : legacy,
                         forKey: Self.accentKey)
        }
        if defaults.object(forKey: Self.oledKey) == nil,
           let legacy = defaults.object(forKey: "stremiox.theme.oled") as? Bool {
            defaults.set(legacy, forKey: Self.oledKey)
        }
        if defaults.object(forKey: Self.textScaleKey) == nil,
           let legacy = defaults.object(forKey: "stremiox.theme.textScale") as? Double {
            defaults.set(legacy, forKey: Self.textScaleKey)
        }
        let savedAccent = defaults.string(forKey: Self.accentKey)
        accentID = savedAccent == Self.previousProductAccentID ? Self.legacyGoldAccentID : (savedAccent ?? "noiro")
        if savedAccent == Self.previousProductAccentID {
            defaults.set(Self.legacyGoldAccentID, forKey: Self.accentKey)
        }
        oled = defaults.bool(forKey: Self.oledKey)
        let saved = defaults.object(forKey: Self.textScaleKey) as? Double
    ...
    /// Curated accents. Prismatic blue-violet on obsidian is Noiro's default.
    static let accents: [AccentOption] = [
        AccentOption(id: "noiro",   label: "Noiro",   base: themeRGB(0.250, 0.390, 0.980), bright: themeRGB(0.920, 0.180, 0.960)),
        AccentOption(id: legacyGoldAccentID, label: "Legacy Gold", base: themeRGB(0.851, 0.467, 0.024), bright: themeRGB(0.961, 0.620, 0.043)),
    ...
    var onAccent: Color {
        if accentID == Self.legacyGoldAccentID { return themeRGB(0.059, 0.051, 0.039) }
