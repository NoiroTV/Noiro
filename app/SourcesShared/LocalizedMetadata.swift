...
/// looked up by imdb/tmdb id + the user's language from the shared Noiro pool worker (vortexo.app/api/noiro/v1/edge/catalogs
...
final class LocalizedMetadataStore: ObservableObject {
    ...
    /// GET /meta/l10n?ids=<csv>&lang=<code> against the catalogs edge, signed via NoiroEdgeAuth. Fail-soft to
    ...
    /// https + *.vortexo.app (RemoteConfig already guarantees that for the catalogs endpoint).
    ...
    private var cacheURL: URL? {
        ...
        return dir?.appendingPathComponent("noiro-l10n-meta.json")
    }
