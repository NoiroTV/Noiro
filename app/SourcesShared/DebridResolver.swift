...
/// `wiki/projects/stremiox/noiro-debrid-implementation.md`.
...
actor TorBoxResolver: DebridResolving {
    ...
    private func postMultipart<T: Decodable>(_ urlString: String, fields: [String: String]) async throws -> T {
        ...
        let boundary = "noiro-\(UUID().uuidString)"
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("Bearer \(apiKey)", forHTTPHeaderField: "Authorization")
...
/// NOTE: the API flow follows the Brain spec (noiro-debrid-implementation.md); it is compile-verified but not
...
actor AllDebridResolver: DebridResolving {
    ...
    private func authed(_ path: String, _ extra: [URLQueryItem]) -> URL {
        ...
        c?.queryItems = [URLQueryItem(name: "agent", value: "noiro"), URLQueryItem(name: "apikey", value: apiKey)] + extra
