...
enum LibraryPortability {
    static let schema = 1
    static let formatTag = "noiro-library"
    ...
    enum RestoreError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notALibrary: return "This file is not a Noiro library export."
            }
        }
    }
    ...
    static func encode(items: [Item], profile: String, now: Date = Date()) throws -> Data {
        let env = Envelope(
            ...
            app: (Bundle.main.infoDictionary?["CFBundleDisplayName"] as? String) ?? "Noiro",
    ...
    static func defaultFilename(profile: String) -> String {
        ...
        return "Noiro-Library-\(tag)-\(df.string(from: Date()))"
    }
}
