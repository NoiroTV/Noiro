...
/// Client for Noiro's community SOURCE INDEX at `vortexo.app/api/noiro/v1/edge/sources` ("Singularity"): the pooled record of which
...
///
/// GATING (Noiro-only): `vortexo.app/api/noiro/v1/edge/sources` is in `NoiroEdgeAuth.gatedHosts`, so BOTH the POST and the GET are
...
enum SourceIndexClient {
    ...
    static func contribute(contentID: String, descriptors: [Descriptor]) async {
        ...
        for (i, chunk) in batches.enumerated() {
            ...
            req.httpBody = data
            NoiroEdgeAuth.sign(&req)   // gated host: stamp X-Noiro-Ts / X-Noiro-Sig / X-Noiro-Kid
    ...
    static func fetchPooled(contentID: String, isSignedIn: Bool) async -> [PooledSource] {
        ...
        // IS required (owner decision 2026-07-04: keep Singularity results a Noiro-user-only benefit; the worker
        ...
        let signed = req.value(forHTTPHeaderField: "X-Noiro-Sig") != nil
        // Moat token: the SERVE gate is login-only AND moat-token-gated (the worker's verifyMoatToken returns
        // an empty list with no token). Stamp X-Noiro-Moat after the edge signature. Fail-soft: no token -> no
    ...
    static let serveKey = "noiro.singularity.serve"
    ...
    static let groupID = "noiro.singularity.sources"
...
final class SourceIndexServeSource: ObservableObject {
    ...
    /// 2026-07-04: Singularity results are a Noiro-user-only benefit). Fail-soft + deduped by content id. Safe
