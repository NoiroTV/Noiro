...
enum NoiroCrashReporter {
    ...
    static func install() {
        ...
        action.__sigaction_u = __sigaction_u(__sa_handler: noiroSignalHandler)
        ...
        NSSetUncaughtExceptionHandler(noiroExceptionHandler)
    ...
    private static func foldPreviousCrash(_ marker: URL) -> Bool {
        ...
        // Tag as "[crash] <signal> at <ts>\n<backtrace>" via the shared exportable sink (Caches/noiro-diag.log).
    ...
    /// `Caches/noiro-lastcrash.txt`, the same container directory VXProbe's exportable log lives in.
    private static var markerURL: URL {
        let caches = (try? FileManager.default.url(for: .cachesDirectory, in: .userDomainMask,
                                                   appropriateFor: nil, create: true))
            ?? URL(fileURLWithPath: NSTemporaryDirectory())
        return caches.appendingPathComponent("noiro-lastcrash.txt")
    }
}
...
private func noiroSignalHandler(_ sig: Int32) {
    let fd = gMarkerFD
...
private func noiroExceptionHandler(_ exception: NSException) {
    let fd = gMarkerFD
