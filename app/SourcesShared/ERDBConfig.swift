...
/// query string. The base URL is configurable so Noiro can later point at a self-hosted instance (e.g. a
/// compatible renderer selected by the user.
///
...
enum ERDB {
    static let enabledKey = "noiro.erdb.enabled"   // absent = OFF (opt-in: it replaces every poster)
    static let tokenKey = "noiro.erdb.token"
    static let baseKey = "noiro.erdb.baseURL"
    static let fanartPostersKey = "noiro.erdb.fanartPosters"   // absent = off
    ...
    /// Opt-in only. No inherited hosted ERDB endpoint ships with Noiro.
    static var isActive: Bool {
        UserDefaults.standard.object(forKey: enabledKey) as? Bool ?? false
    }

    /// Opt-in: request fanart.tv posters from the image service (it appends ?art=fanart, using the user's own
    /// fanart key when set, else the Noiro service key). Posters only; backdrops and logos are unaffected.
    ...
    static func imageURL(_ type: String, id: String, fallback: String?) -> String? {
        guard isActive, let rid = renderableID(id), !normalizedBase().isEmpty else { return fallback }
        let tk = token
    ...
    /// user-configured instance when one is present.
...
enum Fanart {
    static let enabledKey = "noiro.fanart.enabled"   // absent = OFF (opt-in, independent of ERDB)
...
/// once. Precedence: ERDB (when the user set a token) wins, then the Noiro / XRDB poster service, then the
...
enum PosterArtwork {
    ...
    /// Poster image URL for a title id. ERDB token wins, then Noiro / XRDB, then the original poster.
