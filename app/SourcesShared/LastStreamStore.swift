...
enum LastStreamStore {
    ...
    private static func key(_ profileID: UUID) -> String { "noiro.lastStream.\(profileID.uuidString)" }
    ...
    /// Drop the in-memory cache so the next `entry()`/`load()` re-reads UserDefaults. A Noiro-account
    ...
    /// back to re-resolving (and could grab the wrong source). Called from NoiroDocumentSyncManager.syncDown.
