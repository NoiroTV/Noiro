...
/// `doc.noiro.deletedLibrary`, the app's namespace) so a title the user removed on one device can never be
...
/// on a cold/empty engine and `noiroSummary` re-unions the engine library into `doc.noiro.library`, so a
...
///  - PUSH the EFFECTIVE removed set into `doc.noiro.deletedLibrary` from `noiroSummary`, and SUBTRACT it from
///    the `doc.noiro.library` UNION so a removed title is never re-unioned back in.
///  - FOLD an incoming `doc.noiro.deletedLibrary` (plus its `doc.noiro.deletedLibraryTs` companion) into the
...
///
/// WIRE COMPATIBILITY. `doc.noiro.deletedLibrary` keeps its old shape (an array of ids), now computed as the
/// EFFECTIVE removed set, so the dashboard and older app builds keep reading it exactly as before. The new
/// companion `doc.noiro.deletedLibraryTs` (id -> {removedAt, addedAt}) carries the stamps; clients that do not
...
enum LibraryTombstones {
    ...
    private static let removedAtKey = "noiro.library.removedAt"
    private static let addedAtKey = "noiro.library.addedAt"
    ...
    private static let legacyDeletedKey = "noiro.library.deleted"
    ...
    /// UserDefaults so every surface (CoreBridge write, noiroSummary push, syncDown fold, recovery skip) sees
    /// the same authority.
    static func all() -> Set<String> {
        effectiveRemoved(load())
    }

    /// The per-id timestamp map for the wire (`doc.noiro.deletedLibraryTs`). Carries BOTH stamps for every
    ...
    ///
    /// Why this is safe against a stale-doc re-resurrection: `noiroSummary` rewrites `doc.noiro.library` from
    /// the live engine library (which now includes the re-added title), `doc.noiro.deletedLibrary` from the
    /// EFFECTIVE removed set (which no longer lists this id) and `doc.noiro.deletedLibraryTs` with the newer
    ...
    /// Fold an incoming peer's library tombstones. `legacyIDs` is the back-compat `doc.noiro.deletedLibrary`
    /// array (the effective removed set as written by any client); `stampsRaw` is the raw
    /// `doc.noiro.deletedLibraryTs` map ({removedAt, addedAt} per id) written by builds that know the field.
