...
/// Zero-knowledge device pairing for the Noiro account handoff (e.g. phone -> Apple TV, which has no
...
enum PairingCrypto {
    ...
    private static let salt = Data("noiro-pairing-salt-v1".utf8)
    private static let info = Data("noiro-pairing-v1".utf8)
    ...
    static func wrap(_ account: NoiroAccount, toJoinerPublicKey joinerBase64URL: String) -> (claimPublicKey: String, wrapped: String)? {
    ...
    static func unwrap(wrapped: String, holderPublicKey holderBase64URL: String, using ourPrivate: Curve25519.KeyAgreement.PrivateKey) -> NoiroAccount? {
        ...
        return NoiroAccount.from(key: SymmetricKey(data: keyBytes))
    }

    // MARK: - Data-key variant (the Noiro-account QR sign-in)
    //
    // The live Noiro account (NoiroDocumentSyncManager) is NOT the login-less `NoiroAccount` above: it is a
    ...
    // itself), so the handoff wraps a raw `Data`, not a `NoiroAccount`. Same ECDH + HKDF + AES-GCM
