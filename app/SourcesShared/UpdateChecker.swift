...
/// Checks the entitlement-protected Noiro release interface for a newer build.
/// Downloads always begin in the signed-in Vortexo Library; the app never claims
/// to install itself or an Apple TV directly.
///
...
/// build number, so the comparison is reliable.
@MainActor
final class UpdateChecker: ObservableObject {
    ...
    struct Release: Equatable, Identifiable {
        ...
        var id: String { key }
        /// The Library performs entitlement checks and creates a short-lived
        /// artifact grant. It never receives Apple credentials.
        var installURL: URL? {
            return URL(string: "https://vortexo.app/library")
        }
    }
    ...
    private static let lastCheckedKey = "noiro.update.lastChecked"
    private static let dismissedKey = "noiro.update.dismissedVersion"   // shared with the iOS banner
    private static let manifestURL = "https://vortexo.app/api/noiro/v1/releases"
    // Release metadata is entitlement-protected and no public Noiro product is
    // authorized yet. Keep legacy appcast polling off until this checker uses a
    // paired device token and verifies the signed release manifest.
    private static let releaseFeedEnabled = false
    ...
    @MainActor private func check() {
        // UI-iteration hook, alongside `-stremiox-fake-build`: the update popup covers Home, which makes
        // layout work in the simulator impossible to see. Launch-argument only, so shipping builds are
        // unaffected.
        guard Self.releaseFeedEnabled,
              !ProcessInfo.processInfo.arguments.contains("-stremiox-no-update-check") else { return }
        Task { [weak self] in
