...
/// Noiro RemoteConfig: tune / kill / upgrade shipped app behavior from a backend JSON with NO app update.
///
...
///
/// SIGNING: the config GET is signed with the SAME HMAC helper (`NoiroEdgeAuth.sign`) every other gated
/// `*.vortexo.app` worker uses (skip / trickplay / ratings / …). `vortexo.app/api/noiro/v1/edge/config` was added to that helper's
...
enum RemoteConfigDefaults {
    ...
    static let dvRemuxWindowMiB = 64         // Re-read floor, in MiB. MUST stay >= two full HLS segments
                                             ...
                                             // NoiroRemuxBuffer.windowFloorMinMiB agree. Widening (never lowering)
    ...
    // Endpoints.
    static let endpointTrickplay = "https://vortexo.app/api/noiro/v1/edge/trickplay"   // CommunityTrickplay.baseURL
    static let endpointCatalogs = "https://vortexo.app/api/noiro/v1/edge/catalogs/3"   // TMDBClient.edgeBase
    static let endpointSubtitles = "https://vortexo.app/api/noiro/v1/edge/subtitles"   // SubtitlePoolClient / LanguageIndexClient base
    static let endpointSources = "https://vortexo.app/api/noiro/v1/edge/sources"       // SourceIndexClient base (Singularity source index)
    ...
    static let featureSpoilerBlur = true        // noiro.spoilerBlur default (user setting wins)
}
...
final class ResolvedConfig: @unchecked Sendable {
    ...
    // Endpoints, validated (https + *.vortexo.app) or baked default.
...
actor RemoteConfig {
    ...
    // Networking.
    private static let configURL = URL(string: "https://vortexo.app/api/noiro/v1/edge/config/v1/config.json")!
    private static let fetchTimeout: TimeInterval = 8
    private static let etagKey = "noiro.remoteConfig.etag"
    private static let lastFetchKey = "noiro.remoteConfig.lastFetchEpoch"
    ...
    func refresh() async {
        ...
        // Sign with the shared edge-auth helper (vortexo.app/api/noiro/v1/edge/config is a gated host). No-op without a secret;
        // the worker's observe mode lets an empty-key / unsigned request through, so a fetch never bricks.
        NoiroEdgeAuth.sign(&req)
    ...
    static func validate(_ data: RemoteConfigData) -> ResolvedConfig {
        ...
        // NoiroRemuxBuffer.windowFloorMinMiB) and cap at 512 MiB. The lower bound is the design invariant, not a
        ...
        // --- Endpoints: https + host ends ".vortexo.app" or baked default. ---
    ...
    /// Accept `raw` only if it parses as an https URL whose host ends with ".vortexo.app"; otherwise the baked
    ...
    private static func validatedEndpoint(_ raw: String?, fallback: String) -> URL {
        ...
        guard let raw, let url = URL(string: raw), url.scheme?.lowercased() == "https",
              ...
              host == "vortexo.app" || host.hasSuffix(".vortexo.app") else { return bakedURL }
        return url
    }
...
/// `noiro.spoilerBlur` in Settings, that choice wins; otherwise the remote default; otherwise baked true.
/// The Settings `@AppStorage("noiro.spoilerBlur")` toggle stays the source of truth for the user's choice; the
...
enum SpoilerBlurSetting {
    static let key = "noiro.spoilerBlur"
