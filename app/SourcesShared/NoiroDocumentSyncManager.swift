...
/// Bridges the thread-agnostic `NoiroDocumentSyncManager.addonOrderChangedNote` to a `@Published` the add-on list
...
/// not also re-render on NoiroDocumentSyncManager's unrelated account/sign-in @Published churn.
final class AddonOrderObserver: ObservableObject {
    ...
    private init() {
        ...
        NotificationCenter.default.addObserver(forName: NoiroDocumentSyncManager.addonOrderChangedNote, object: nil, queue: .main) { [weak self] _ in
            self?.revision &+= 1
        }
    }
}

/// Noiro's local merge engine for the website-authoritative encrypted document.
/// Passwordless device identity and the recovery key are owned by
/// `NoiroSyncManager`; this type keeps the proven profile/library/add-on merge
/// semantics while transporting only opaque AES-GCM ciphertext through Vortexo.
/// Noiro still works fully offline when it is unpaired or the vault is locked.
@MainActor
final class NoiroDocumentSyncManager: ObservableObject {
    static let shared = NoiroDocumentSyncManager()
    private static let appDocumentKey = "noiro"
    private static let previousAppDocumentKey = "vor" + "tx"

    /// Read both document generations while the fleet migrates. New writes are
    /// mirrored to both keys so an older installed client is not stranded.
    private static func appDocument(in document: [String: Any]) -> [String: Any]? {
        (document[appDocumentKey] as? [String: Any])
            ?? (document[previousAppDocumentKey] as? [String: Any])
    }
    ...
    // The pre-Noiro password account protocol is intentionally retired. Keep
    // the implementation temporarily for local data migration, but point every
    // accidental call at a fail-closed compatibility path and never restore an
    // old session into an active signed-in state.
    private let base = "https://vortexo.app/api/noiro/v1/legacy-disabled"
    private let kcAccount = "noiro.sync.session.v1"
    private var token: String?
    private var dataKey: Data?
    private var currentServerRevision = 0
    ...
    /// MIGRATION flip for the version-bound sync-document format (see NoiroSyncCrypto.sealDocument). Stays
    ...
    ///   1. Dual-read shipped + adopted on EVERY sync client: this app, the noiro-site dashboard
    ///      (vortexo.app), AND the web client (webapp/, vortexo.app/api/noiro/v1/edge/web) - all three, each with WRITE_SYNC_DOC_V2
    ...
    ///      NOT DONE.
    /// Flip this AND WRITE_SYNC_DOC_V2 in noiro-site vault.ts AND webapp/src/lib/vault.ts together.
    ...
    private func versionKey(for accountId: String?) -> String { "noiro.sync.lastAppliedRevision." + (accountId ?? "") }
    ...
    private func editsAtKey(for accountId: String?) -> String { "noiro.sync.lastAppliedProfileEditsAt." + (accountId ?? "") }
    ...
    private static let kAddonOrderKey = "noiro.sync.appliedAddonOrder"
    ...
    // `nonisolated`: Notification.Name is Sendable and this is observed from any context.
    nonisolated static let addonOrderChangedNote = Notification.Name("noiro.addonOrderChanged")
    ...
    private init() {
        ...
        NotificationCenter.default.addObserver(
            forName: .noiroDeviceAuthorizationDidChange,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor in self?.adoptNoiroIdentity() }
        }
    ...
    private func restore() {
        // Do not delete the legacy Keychain item: retaining it makes a future
        // explicit, local-only migration possible. It must never authorize a
        // Noiro request or silently reactivate reusable-password cloud sync.
        token = nil
        account = nil
        dataKey = nil
        isSignedIn = false
    ...
    private func adoptNoiroIdentity() {
        let wasSignedIn = isSignedIn
        let hadRecoveryKey = dataKey != nil
        guard let identity = NoiroSyncManager.shared.syncIdentity else {
            stopRealtime()
            token = nil
            account = nil
            dataKey = nil
        ...
        isSignedIn = true
        if !wasSignedIn { startRealtime() }
        if !hadRecoveryKey, identity.recoveryKey != nil {
            Task { [weak self] in
                guard let self else { return }
                if !(await self.syncDown(force: true)) { _ = await self.syncUp() }
            }
        }
    }
    ...
    func signOut() {
        ...
        NoiroSyncManager.shared.disconnect()
    ...
    func register(email: String, username: String, password: String) async -> (result: AuthResult, recoveryCode: String?) {
        let kdfSalt = NoiroSyncCrypto.randomBytes(16)
        let iters = NoiroSyncCrypto.defaultIters
        let masterKey = NoiroSyncCrypto.masterKey(password: password, kdfSalt: kdfSalt, iters: iters)
        let dataKey = NoiroSyncCrypto.randomBytes(32)
        let recoveryCode = NoiroSyncCrypto.makeRecoveryCode()
        let recoveryKey = NoiroSyncCrypto.recoveryKey(recoveryCode: recoveryCode, kdfSalt: kdfSalt, iters: iters)
        guard let wrappedPw = NoiroSyncCrypto.seal(key: masterKey, dataKey),
              let wrappedRec = NoiroSyncCrypto.seal(key: recoveryKey, dataKey) else {
            return (.failed("Could not set up encryption."), nil)
        }
        let body: [String: Any] = [
            "email": email, "username": username,
            "kdfSalt": kdfSalt.base64EncodedString(), "kdfIters": iters,
            "authVerifier": NoiroSyncCrypto.authVerifier(masterKey: masterKey, password: password),
            "wrappedKeyPassword": wrappedPw, "wrappedKeyRecovery": wrappedRec,
            "recVerifier": NoiroSyncCrypto.recVerifier(recoveryKey: recoveryKey, recoveryCode: recoveryCode),
    ...
    func signIn(login: String, password: String, totp: String? = nil) async -> AuthResult {
        ...
        guard let saltStr = pre?["kdfSalt"] as? String, let salt = Data(base64Encoded: saltStr),
              let iters = pre?["kdfIters"] as? Int else { return .failed("Could not reach Noiro. Try again.") }
        // Reject a downgraded work factor from the UNAUTHENTICATED prelogin response before deriving the key.
        guard iters >= NoiroSyncCrypto.minIters else { return .failed("Could not verify Noiro security parameters. Try again.") }
        let masterKey = NoiroSyncCrypto.masterKey(password: password, kdfSalt: salt, iters: iters)
        var body: [String: Any] = ["login": login, "authVerifier": NoiroSyncCrypto.authVerifier(masterKey: masterKey, password: password)]
        ...
        guard code == 200, let token = json?["token"] as? String, let acct = json?["account"] as? [String: Any],
              ...
              let dk = NoiroSyncCrypto.open(key: masterKey, wrappedPw) else {
    ...
    func recover(email: String, recoveryCode: String, newPassword: String) async -> AuthResult {
        ...
        guard iters >= NoiroSyncCrypto.minIters else { return .failed("Could not verify Noiro security parameters. Try again.") }
        let recoveryKey = NoiroSyncCrypto.recoveryKey(recoveryCode: trimmed, kdfSalt: salt, iters: iters)
        guard let dk = NoiroSyncCrypto.open(key: recoveryKey, wrappedRec) else { return .failed("That recovery code is not correct.") }
        // Keep the existing kdfSalt (it also derives the recovery key); derive the new master from it.
        let newMaster = NoiroSyncCrypto.masterKey(password: newPassword, kdfSalt: salt, iters: iters)
        guard let wrappedPw = NoiroSyncCrypto.seal(key: newMaster, dk) else { return .failed("Could not re-encrypt.") }
        let body: [String: Any] = [
            "email": email,
            "recVerifier": NoiroSyncCrypto.recVerifier(recoveryKey: recoveryKey, recoveryCode: trimmed),
            "newAuthVerifier": NoiroSyncCrypto.authVerifier(masterKey: newMaster, password: newPassword),
    ...
    // Legacy document helpers remain for an explicit local migration only. New
    // writes use NoiroStudioClient's AES-GCM envelope and D1 CAS revision.
    ...
    private static func sawDocV2(_ accountId: String) -> Bool {
        !accountId.isEmpty && UserDefaults.standard.bool(forKey: "noiro.sync.sawDocV2." + accountId)
    }
    private static func markSawDocV2(_ accountId: String) {
        guard !accountId.isEmpty else { return }
        UserDefaults.standard.set(true, forKey: "noiro.sync.sawDocV2." + accountId)
    }
    ...
    private func openSyncDocument(_ stored: String, version: Int) -> Data? {
        ...
        let isV2 = stored.hasPrefix(NoiroSyncCrypto.docV2Prefix)
        if !isV2, Self.sawDocV2(acctId) { return nil }             // legacy after v2 seen -> tamper
        let pt = NoiroSyncCrypto.openDocument(dataKey: dataKey, stored: stored, accountId: acctId, version: version)
    ...
    func pullSyncDoc() async -> [String: Any]? {
        ...
        do {
            guard let pulled = try await NoiroSyncManager.shared.pullDashboardDocument() else {
                currentServerRevision = 0
                return nil
            }
            currentServerRevision = pulled.revision
            return pulled.value
        } catch {
            return nil
        }
    ...
    private func pullSyncDocResult() async -> SyncDocPull {
        ...
        do {
            ...
            if pulled.revision < lastSyncedVersion { return .failed }
            currentServerRevision = pulled.revision
            return .doc(pulled.value)
        } catch {
            return .failed
        }
    }
    ...
    private func pullDocVersioned() async -> (doc: [String: Any], version: Int)? {
        ...
        do {
            ...
            currentServerRevision = pulled.revision
            return (pulled.value, pulled.revision)
        } catch {
            return nil
        }
    }
    ...
    private func pushSyncDocAt(_ obj: [String: Any], version _: Int) async -> PushOutcome {
        guard dataKey != nil else { return .error }
        do {
            let stored = try await NoiroSyncManager.shared.pushDashboardDocument(
                obj,
                expectedRevision: currentServerRevision
            )
            currentServerRevision = stored
            lastSyncedVersion = max(lastSyncedVersion, stored)
            persistLastSyncedVersion()
            return .accepted(version: stored)
        } catch let error as NoiroStudioClient.ClientError where error == .conflict {
            if let stored = try? await NoiroSyncManager.shared.dashboardRevision() {
                currentServerRevision = stored
    ...
    /// `existingNoiro` is the `doc["noiro"]` just pulled from the account (nil on a fresh/empty doc). It
    ...
    /// read-merge guards.
    private func noiroSummary(existingNoiro: [String: Any]? = nil) -> [String: Any] {
        ...
        // noiro.library from the engine's account library so the dashboard's main-profile Library is
        ...
        // default) = UNION the account's already-owned `doc.noiro.library` with the engine library, so a
        // Stremio removal never removes from Noiro and an empty/degraded engine can never SHRINK it. The
        ...
        if !mirrorReplaceLibrary, let prior = (existingNoiro?["library"] as? [[String: Any]]) {
        ...
        // still briefly holds it OR a peer device's prior doc.noiro.library still carries it. Compared on the
        ...
        //   already-owned `doc.noiro.addons` by transportUrl, so a Stremio removal NEVER removes from Noiro
        ...
        // the spine; in UNION mode the Noiro-doc-only add-ons (in the prior sync but not the engine)
        ...
        // owned mirror, so a logout can never shrink doc.noiro.addons to defaults.
        ...
        // the engine still briefly holds it OR a peer device's prior doc.noiro.addons still carries it.
        ...
        if !mirrorReplaceAddons, let prior = (existingNoiro?["addons"] as? [[String: Any]]) {
            for entry in prior {
        ...
        if !addonList.isEmpty {
            ...
            if let priorOwnedAt = existingNoiro?["addonsOwnedAt"] {
                v["addonsOwnedAt"] = priorOwnedAt
            } else {
                v["addonsOwnedAt"] = Int(Date().timeIntervalSince1970 * 1000)
            }
        } else if let priorOwnedAt = existingNoiro?["addonsOwnedAt"] {
    ...
    private func mergeLocalIntoDoc(base: [String: Any]?) async -> [String: Any]? {
        ...
        // Read-merge the pulled doc's tombstone stamps into the local stores BEFORE noiroSummary rebuilds them
        ...
        doc["format"] = 1
        // Pass the PULLED noiro block so noiroSummary can union the account-owned add-on set (never
        // shrink it from a degraded engine) and preserve addonsOwnedAt.
        let appSummary = noiroSummary(existingNoiro: Self.appDocument(in: doc))
        doc[Self.appDocumentKey] = appSummary
        doc[Self.previousAppDocumentKey] = appSummary
    ...
    /// priority order (the same `rawAddonDescriptorsOrdered` spine `noiroSummary` uses). Removed add-ons
    ...
    func syncDown(force: Bool = false) async -> Bool {
        ...
        // profile's overlay into doc.noiro.byProfile (what the dashboard reads); this pulls it BACK into the
        ...
        // Cross-device delete tombstones: fold any incoming doc.noiro.deletedProfiles into the local set
        ...
        if let noiro = Self.appDocument(in: doc), let deleted = noiro["deletedProfiles"] as? [String] {
        ...
        // can NEVER drive an uninstall. Fold the app-authored doc.noiro.deletedAddons AND a (future)
        ...
        if let noiro = Self.appDocument(in: doc) {
            if let removed = noiro["deletedAddons"] as? [String] { incomingAddonRemovals += removed }
            if let ts = noiro["deletedAddonsTs"] as? [String: Any] { incomingAddonRemovalsTs = ts }
        }
        ...
        // made on device A stays durable HERE, so this device's noiroSummary keeps subtracting it from the
        ...
        if let noiro = Self.appDocument(in: doc) {
            let removedLib = (noiro["deletedLibrary"] as? [String]) ?? []
            let removedLibTs = (noiro["deletedLibraryTs"] as? [String: Any]) ?? [:]
        ...
        if let noiro = Self.appDocument(in: doc), let byProfile = noiro["byProfile"] as? [String: Any] {
        ...
        // Web-authored profile edits (vortexo.app dashboard writes doc.profileEdits, a SIBLING key the app
        // preserves via syncUp's read-merge-write, unlike doc.noiro which the app overwrites). Apply
    ...
    /// Hydrate the engine from the Noiro account's OWNED add-ons + recover the owner library, so a
    ...
    /// empty). Only a real `.doc` triggers hydration. Not gated by the mirror toggles — the Noiro-owned
    /// set always hydrates when the engine is empty/degraded; the toggles only control the snapshot
    /// DIRECTION (Stremio -> Noiro), not the floor.
    ///
    /// Owned add-ons = `doc.noiro.addons` UNION `doc.addons` (the website Stremio import) by transportUrl.
    ...
    /// Compute the account-owned add-on descriptors from a pulled doc: `doc.noiro.addons` (the app's
    /// full descriptors) UNIONed with `doc.addons` (the website's Stremio import) by transportUrl.
    /// noiro.addons wins on conflict (it carries the freshest app descriptor). Legacy `{transportUrl,
    /// name}`-only entries (no manifest) are dropped: without a manifest the engine cannot InstallAddon.
    static func ownedAddons(from doc: [String: Any]) -> [NoiroOwnedAddon] {
        var byUrl: [String: NoiroOwnedAddon] = [:]
        ...
        // doc.noiro.addons (the app/engine-owned set) FIRST, so it defines the order spine - matching the
        // write-side spine in noiroSummary - and its richer descriptor wins a URL present in both; then the
        // web-import-only URLs append after.
        if let noiro = appDocument(in: doc), let appAddons = noiro["addons"] as? [[String: Any]] {
            for raw in appAddons { if let a = NoiroOwnedAddon(json: raw) { add(a) } }
        }
        if let webAddons = doc["addons"] as? [[String: Any]] {
            for raw in webAddons { if let a = NoiroOwnedAddon(json: raw) { add(a) } }
        }
    ...
    private func recoverOwnerLibraryIfEmpty(from doc: [String: Any]) async {
        guard let noiro = Self.appDocument(in: doc) else { return }
        // doc.noiro.library is the owner library; fall back to doc.library (web Stremio import) if present.
        let ownedLibrary = (noiro["library"] as? [[String: Any]]) ?? (doc["library"] as? [[String: Any]]) ?? []
        ...
        if recovered > 0 {
            DiagnosticsLog.log("sync", "recovered \(recovered) owner-library title(s) from the legacy local account cache on a cold device")
        }
    }
    ...
    private func foldDocTombstones(_ doc: [String: Any]) {
        let noiro = Self.appDocument(in: doc)
        let libIDs = (noiro?["deletedLibrary"] as? [String]) ?? []
        let libTs = (noiro?["deletedLibraryTs"] as? [String: Any]) ?? [:]
        if !libIDs.isEmpty || !libTs.isEmpty {
            LibraryTombstones.merge(legacyIDs: libIDs, stampsRaw: libTs)
        }
        let addonIDs = (noiro?["deletedAddons"] as? [String]) ?? []
        let addonTs = (noiro?["deletedAddonsTs"] as? [String: Any]) ?? [:]
        if !addonIDs.isEmpty || !addonTs.isEmpty {
            AddonTombstones.merge(legacyIDs: addonIDs, stampsRaw: addonTs)
        }
    }

    private static let addonBaselineStampedKey = "noiro.sync.addonBaselineStampedV2"
    ...
    /// are handled by noiroSummary's read-side guard; this just forces a push so the snapshot lands.
    func snapshotOwnedFromEngine() async {
        ...
        await syncUp()   // noiroSummary unions the engine descriptors into doc.noiro.addons + sets addonsOwnedAt
    }
    ...
    func ownedAddonsNeverSnapshotted() async -> Bool {
        ...
        let noiro = Self.appDocument(in: doc)
        return noiro?["addonsOwnedAt"] == nil
    }
    ...
    func syncUpOnBackground() {
        ...
        bgTask = app.beginBackgroundTask(withName: "noiro.sync.background") {
    ...
    // MARK: - Noiro-account QR sign-in (device pairing)
    //
    // A device with no keyboard (Apple TV) signs into the Noiro account by showing a QR/code. A device
    // already signed into Noiro (a phone, or vortexo.app/api/noiro/v1/edge/web) approves it and hands over the sync data key,
    ...
    // Both the app holder (qrApprove) and the web holder (noiro-site vault.ts qrApprove) MUST emit this exact
    // envelope, and both joiners MUST parse it. Crypto contract: PairingCrypto (salt noiro-pairing-salt-v1,
    // info noiro-pairing-v1, base64url, iv‖ct‖tag) — identical across app + web.
    ...
    // MARK: - While-active encrypted sync polling

    /// Start the while-active poll. The retired Noiro WebSocket used the old
    /// reusable-password realm; Noiro's versioned sync endpoint is polled until
    /// a realm-isolated push channel is introduced.
