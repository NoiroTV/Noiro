...
/// Resolves the one special Continue-Watching case that cannot be decided from the engine preview alone:
/// Stremio mirroring may leave a series parked at 90-100% on the episode just completed so clients can offer
/// its successor. Noiro keeps that placeholder only when metadata proves the immediate successor has aired.
/// A future-dated successor, a season finale, missing metadata, or an unknown parked episode stays out of
/// Continue Watching. Ordinary in-progress movies and episodes never pass through this gate.
@MainActor
final class ContinueWatchingAvailabilityModel: ObservableObject {
    @Published private(set) var playableCompletedSeriesIDs: Set<String> = []
    ...
    func refresh(items: [CoreCWItem], metaBases: [String], reference: Date = Date()) {
        let candidates = Dictionary(
            items.compactMap { item -> (String, String)? in
                guard item.isMirroredCompletedSeries,
                      let videoID = item.state.videoId, !videoID.isEmpty else { return nil }
        ...
        var bases: [String] = []
        for base in metaBases where !bases.contains(base) { bases.append(base) }

        guard !candidates.isEmpty, !bases.isEmpty else {
            loadTask?.cancel()
            loadTask = nil
            lastSignature = nil
            playableCompletedSeriesIDs = []
            return
        }

        let dayBucket = Int(reference.timeIntervalSinceReferenceDate / 86_400)
        let candidateKey = candidates.map { "\($0.key)=\($0.value)" }.joined(separator: ",")
        let signature = "\(dayBucket)|\(bases.joined(separator: ","))|\(candidateKey)"
        guard signature != lastSignature else { return }

        lastSignature = signature
        loadTask?.cancel()
        // Fail closed while the new state resolves: a completed placeholder is never more important than
        // accidentally advertising an episode that has not aired yet.
        playableCompletedSeriesIDs = []
        loadTask = Task {
            var eligible: Set<String> = []
            var resolvedCount = 0
            for (seriesID, currentVideoID) in candidates {
                if Task.isCancelled { return }
                guard let meta = await SeriesMetaFetcher.fetch(id: seriesID, bases: bases) else { continue }
                resolvedCount += 1
                if Self.hasAiredSuccessor(in: meta.videos ?? [], after: currentVideoID, reference: reference) {
                    eligible.insert(seriesID)
                }
            }
            if Task.isCancelled { return }
            playableCompletedSeriesIDs = eligible
            // A missing add-on response is retryable on the next Home appearance/input refresh.
            if resolvedCount < candidates.count { lastSignature = nil }
        }
    }

    /// Episodes follow the same season/episode ordering used by the players. The successor must carry an
    /// explicit release date at or before `reference`; unknown dates fail closed so future episodes never leak.
    static func hasAiredSuccessor(in videos: [CoreVideo], after currentVideoID: String,
                                  reference: Date = Date()) -> Bool {
        let ordered = videos.orderedBySeasonEpisode
        guard let current = ordered.firstIndex(where: { $0.id == currentVideoID }) else { return false }
        let next = ordered.index(after: current)
        guard next < ordered.endIndex else { return false }
        guard let releaseDate = ordered[next].releasedDate else { return false }
        return releaseDate <= reference
    }
}
