...
/// Client for the "Install by QR / pair once, add many" add-on pairing relay at `vortexo.app/api/noiro/v1/edge/add`.
///
...
///
/// SIGNING: `vortexo.app/api/noiro/v1/edge/add` is a gated Noiro host (see `NoiroEdgeAuth.gatedHosts`), so both routes are
/// HMAC-signed with `NoiroEdgeAuth.sign(&req)`. Signing is a no-op without a provisioned secret, which
/// the worker's observe mode lets through, so the flow works in every build.
enum AddonPairingClient {
    /// The relay base. HTTPS only; the host must stay in `NoiroEdgeAuth.gatedHosts` for signing.
    private static let baseURL = URL(string: "https://vortexo.app/api/noiro/v1/edge/addons")!
    ...
    static func createSession() async -> Session? {
        ...
        NoiroEdgeAuth.sign(&req)   // gated host (vortexo.app/api/noiro/v1/edge/add /pair/new POST): stamp X-Noiro-Ts / X-Noiro-Sig
    ...
    static func poll(token: String) async -> PollResult {
        ...
        req.httpMethod = "GET"
        req.setValue("application/json", forHTTPHeaderField: "accept")
        NoiroEdgeAuth.sign(&req)   // gated host (vortexo.app/api/noiro/v1/edge/add /pair/<token> GET): stamp X-Noiro-Ts / X-Noiro-Sig
