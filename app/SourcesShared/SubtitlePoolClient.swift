...
/// Client for Noiro's community-subtitle pool at `vortexo.app/api/noiro/v1/edge/subtitles`.
///
/// The pool lets any Noiro user READ subtitles other users have contributed for the same title/episode, and
...
///
/// GATING (Noiro-only pool): the owner gates this pool to real Noiro builds, like trickplay and trailers, so
/// EVERY request to `vortexo.app/api/noiro/v1/edge/subtitles` is HMAC-signed with `NoiroEdgeAuth.sign` -- the GET reads and the
/// sub-text download as well as the POSTs. Reads carry no ACCOUNT key (still "keyless" that way) but do carry
/// the `X-Noiro-Ts` / `X-Noiro-Sig` headers so that when the worker flips to enforce mode only Noiro clients read the
...
enum SubtitlePoolClient {
    ...
    /// (verifyMoatToken => empty list with no token), so a signed-in device stamps `X-Noiro-Moat`. Default
    ...
    static func fetchPooled(contentKey: String,
        ...
        NoiroEdgeAuth.sign(&req)   // Noiro-only gate: sign reads too (no-op without a secret)
    ...
    /// signed too when its host is a gated Noiro host, so pool-hosted sub text stays Noiro-only. Timeout comes
    ...
    static func download(_ pooled: PooledSubtitle, isSignedIn: Bool = false) async -> URL? {
        ...
        req.httpMethod = "GET"
        NoiroEdgeAuth.sign(&req)   // host-gated inside sign(): no-op unless the URL host is a gated Noiro host
        // The pool-hosted sub TEXT is served from a moat-gated Noiro host too, so stamp the moat token; a
        ...
        let name = "noiro-poolsub-\(stableHash(pooled.url.absoluteString)).\(ext)"
