...
/// Client crypto for the Noiro end-to-end-encrypted account, byte-for-byte matching the Cloudflare
/// Worker contract (cloudflare/src/index.ts header) and the website (noiro-site/src/lib/vault.ts),
...
enum NoiroSyncCrypto {
    static let defaultIters = 210_000
    /// Hard floor for a server-supplied `kdfIters`. `/v1/auth/prelogin` and `/v1/auth/recover-start` are
    /// UNAUTHENTICATED, so a spoofed/compromised vortexo.app/api/noiro/v1/edge/api (or a MITM) could return `kdfIters: 1` and make
    ...
    // still open, so this is backward-compatible. The webapp (noiro-site/src/lib/vault.ts) mirrors this byte
