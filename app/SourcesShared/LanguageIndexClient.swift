...
/// Client for the crowd-sourced language-availability index at `vortexo.app/api/noiro/v1/edge/subtitles/lang`.
///
...
///
/// GATING (Noiro-only): like the sub pool, EVERY request to `vortexo.app/api/noiro/v1/edge/subtitles` is HMAC-signed with
/// `NoiroEdgeAuth.sign`, including the GET read, so the index can be gated to real Noiro builds when the worker
...
enum LanguageIndexClient {
    ...
    static func fetch(contentKey: String) async -> LanguageAvailability? {
        ...
        NoiroEdgeAuth.sign(&req)   // Noiro-only gate: sign the read (no-op without a secret)
