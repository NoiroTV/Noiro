...
enum PosterWidthPreset: String, CaseIterable, Identifiable {
    ...
    /// = 224); tvOS posters sit on the `kPosterWidth` = 230 baseline (Vortexo parity), so each preset scales
    /// `regularWidth` by 230/224. This keeps the SAME relative ladder while landing the 10-foot grid at the
    /// new larger baseline (`.balanced.tvWidth` == 230 == kPosterWidth).
    var tvWidth: CGFloat {
        (regularWidth * 230.0 / 224.0).rounded()
    }
...
enum CatalogPrefsStore {
    static let hiddenKey = "noiro.catalog.hidden"
    static let orderKey = "noiro.catalog.order"
    static let landscapeKey = "noiro.catalog.landscapeCards"
    static let widthKey = "noiro.catalog.posterWidthPreset"
    static let radiusKey = "noiro.catalog.posterRadiusPreset"
    static let hideLabelsKey = "noiro.catalog.hidePosterLabels"
    /// On-card metadata overlay (Vortexo parity): title + rating/genre footer over a bottom scrim on the
    /// poster, instead of the title below the card. Defaults to ON (the key unset reads true), matching the
    /// reference project's look; the Appearance toggle lets users revert to the legacy title-below layout.
    static let onCardMetaKey = "noiro.catalog.onCardMeta"
    static let hiddenCategoriesKey = "noiro.discover.hiddenCategories"
    static let regionKey = "noiro.discover.regionPreference"   // "" / absent = follow the device region
    ...
    static func widthPreset() -> PosterWidthPreset {
        ...
        // == 230 == kPosterWidth (Vortexo parity); the old `.large` fell back to 286pt/320pt.
    ...
    /// On-card metadata overlay (Vortexo parity). Defaults to ON (the key unset reads true), matching the
    /// reference project; the Appearance toggle lets users revert to title-below-card.
    static func onCardMeta() -> Bool {
        UserDefaults.standard.object(forKey: onCardMetaKey) == nil ? true : UserDefaults.standard.bool(forKey: onCardMetaKey)
    }
    static func setOnCardMeta(_ value: Bool) { UserDefaults.standard.set(value, forKey: onCardMetaKey) }
...
final class CatalogPreferences: ObservableObject {
    ...
    /// On-card metadata overlay (Vortexo parity): title + rating/genre over a bottom scrim on the poster,
    /// instead of the title below the card. Default ON; the Appearance toggle reverts to title-below.
    @Published var onCardMeta: Bool = CatalogPrefsStore.onCardMeta() {
        didSet { CatalogPrefsStore.setOnCardMeta(onCardMeta) }
    }
...
struct CatalogManagerView: View {
    ...
    #if os(tvOS)
    // The group shortcut is left-aligned while every actionable control in a catalog row sits at the far
    // right. tvOS therefore sees the nearby left rail as the geometric Down target. Keep one guaranteed-
    // enabled focus seat (the visibility button) per row so the header can hand focus into the list.
    @FocusState private var focusedCatalogVisibility: String?
    #endif
        ...
        VStack(alignment: .leading, spacing: Theme.Space.md) {
            ...
            if !ordered.isEmpty {
                ...
                .onMoveCommand { direction in
                    if direction == .down, let first = ordered.first {
                        focusedCatalogVisibility = first.key
                    }
                }
                #endif
            }
    ...
    private func row(_ info: CoreBridge.CatalogInfo, index: Int, total: Int, keys: [String]) -> some View {
        ...
        HStack(spacing: Theme.Space.md) {
            ...
            #if os(tvOS)
            .focused($focusedCatalogVisibility, equals: info.key)
            #endif
        }
