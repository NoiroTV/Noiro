...
enum DiscoverList: String, CaseIterable, Hashable {
    ...
    /// by hue, so the hub reads as Noiro's own dark-ember identity rather than the rainbow-card reference.
...
struct GenreSpec: Hashable {
    ...
    var tint: Color { Theme.Palette.accent }   // ember accent, not a per-genre rainbow hue (Noiro identity)
}
...
enum HubRefreshCadence: String, CaseIterable, Identifiable {
    ...
    static var current: HubRefreshCadence {
        HubRefreshCadence(rawValue: UserDefaults.standard.string(forKey: "noiro.collections.refreshCadence") ?? "daily") ?? .daily
    }
}
...
final class CollectionsHubModel: ObservableObject {
    ...
    /// Available now: the keyless vortexo.app/api/noiro/v1/edge/catalogs edge serves the hub (Discover/services/genres) even with
    ...
    private static func genreCacheKey(_ region: String) -> String { "noiro.collections.genreBackdrops.\(region)" }
    private static func genreCacheAtKey(_ region: String) -> String { "noiro.collections.genreBackdropsAt.\(region)" }
    ...
    private static func discoverCacheKey(_ region: String) -> String { "noiro.collections.discoverBackdrops.\(region)" }
    private static func discoverCacheAtKey(_ region: String) -> String { "noiro.collections.discoverBackdropsAt.\(region)" }
    ...
    private static func cacheKey(_ region: String) -> String { "noiro.collections.providers.v2.\(region)" }
    private static func cacheAtKey(_ region: String) -> String { "noiro.collections.providersAt.v2.\(region)" }
    ...
    private static let globalCacheKey = "noiro.collections.globalProviders.v2"
    private static let globalCacheAtKey = "noiro.collections.globalProvidersAt.v2"
    ...
    private static let orderKey = "noiro.collections.providerOrder"
    ...
    // `nonisolated`: an immutable defaults key, read off the main actor.
    nonisolated static let selectedProvidersKey = "noiro.collections.selectedProviders"
