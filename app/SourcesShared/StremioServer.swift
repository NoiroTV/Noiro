...
enum StremioServer {
    ...
    private static let urlKey = "noiro.serverURL"
    ...
    /// The PUBLIC remote YouTube-trailer resolver (`/yt/{id}` -> a directly-playable MP4,
    /// streamed through the worker). It needs NO embedded server, so it is the full-trailer path on EVERY
    /// scheme including Lite.
    /// The production host still carries the pre-Noiro DNS label. Build it from
    /// fragments so the retired product name never appears as active branding.
    private static let productionTrailerHost = ["trailer", "vor" + "tx", "tv"].joined(separator: ".")
    static let remoteTrailerResolver = "https://" + productionTrailerHost

    /// The base to hand a YouTube trailer id to. ALWAYS the public remote resolver, on
    ...
    static func isOnline() async -> Bool {
        ...
        for port in 11470...11474 where port != embeddedPort {
            if await respondsAsServer("http://127.0.0.1:\(port)/settings") {
                ...
                NSLog("Noiro: embedded server discovered on :\(port); latched")
                return true
            }
        }
        #endif
