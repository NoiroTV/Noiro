...
/// `doc.noiro.deletedAddons`, the app's namespace) so an add-on the user removed on one device can
...
/// owned descriptor and `noiroSummary` re-unions the engine set into `doc.noiro.addons`, so a removal
...
///  - PUSH the EFFECTIVE removed set into `doc.noiro.deletedAddons` from `noiroSummary`, and SUBTRACT it
///    from the `doc.noiro.addons` UNION so a removed add-on is never re-unioned back in.
///  - FOLD an incoming `doc.noiro.deletedAddons` (plus its `doc.noiro.deletedAddonsTs` companion, and a
...
///
/// WIRE COMPATIBILITY. `doc.noiro.deletedAddons` keeps its old shape (an array of URLs), now computed as the
/// EFFECTIVE removed set, so the dashboard and older app builds keep reading it exactly as before. The new
/// companion `doc.noiro.deletedAddonsTs` (url -> {removedAt, addedAt}) carries the stamps; clients that do not
...
enum AddonTombstones {
    ...
    private static let removedAtKey = "noiro.addons.removedAt"
    private static let addedAtKey = "noiro.addons.addedAt"
    ...
    private static let legacyDeletedKey = "noiro.addons.deleted"
    ...
    /// from UserDefaults so every surface (CoreBridge write, noiroSummary push, syncDown fold/apply) sees the
    /// same authority.
    static func all() -> Set<String> {
        effectiveRemoved(load())
    }

    /// The per-url timestamp map for the wire (`doc.noiro.deletedAddonsTs`). Carries BOTH stamps for every
    ...
    ///
    /// Why this is safe against a stale-doc re-resurrection: `noiroSummary` rewrites `doc.noiro.addons` from
    /// the live engine set (which now includes the re-installed add-on), `doc.noiro.deletedAddons` from the
    /// EFFECTIVE removed set (which no longer lists this url) and `doc.noiro.deletedAddonsTs` with the newer
    ...
    /// Fold an incoming peer's add-on tombstones. `legacyIDs` is the back-compat `doc.noiro.deletedAddons`
    /// array plus any web-authored `doc.webAddonRemovals` (both stamp-less effective removed sets); `stampsRaw`
    /// is the raw `doc.noiro.deletedAddonsTs` map ({removedAt, addedAt} per url) written by builds that know
