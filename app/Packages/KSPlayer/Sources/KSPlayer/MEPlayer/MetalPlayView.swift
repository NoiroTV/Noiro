...
public final class MetalPlayView: UIView, VideoOutput {
    ...
    private var fps = Float(60) {
        didSet {
            if fps != oldValue {
                if options.preferredFrame {
                    // Preserve fractional cinema rates (23.976/29.97/59.94). Rounding 23.976 up to
                    // 24.0 makes the display clock gain roughly one source frame every 42 seconds,
                    // forcing videoClockSync to discard a frame even when decode and buffering are healthy.
                    let preferredFramesPerSecond = max(fps, 1)
                    if #available(iOS 15.0, tvOS 15.0, macOS 14.0, *) {
                        ...
                        displayLink.preferredFramesPerSecond = Int(ceil(preferredFramesPerSecond)) << 1
                    }
                }
                options.updateVideo(refreshRate: fps, isDovi: isDovi, formatDescription: formatDescription)
            }
