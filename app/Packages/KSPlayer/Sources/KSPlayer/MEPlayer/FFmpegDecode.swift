...
class FFmpegDecode: DecodeProtocol {
    ...
    required init(assetTrack: FFmpegAssetTrack, options: KSOptions) {
        ...
        if assetTrack.mediaType == .video {
            ...
            guard let audioDescriptor = assetTrack.audioDescriptor else {
                // Provide default AudioDescriptor for tracks without audio descriptor
                let defaultDescriptor = AudioDescriptor(sampleFormat: AV_SAMPLE_FMT_FLT, sampleRate: 48000, channel: AVChannelLayout.defaultValue)
                frameChange = AudioSwresample(audioDescriptor: defaultDescriptor)
                KSLog("Audio track \(assetTrack.trackID) missing audioDescriptor, using default")
                return
            }
                    ...
                    for i in 0 ..< inputFrame.pointee.nb_side_data {
                        if let sideData = inputFrame.pointee.side_data[Int(i)]?.pointee {
                            if sideData.type == AV_FRAME_DATA_A53_CC {
                                if let closedCaptionsTrack = closedCaptionsTrack(for: packet.assetTrack),
...
//                                frame.corePixelBuffer?.transferFunction = kCVImageBufferTransferFunction_ITU_R_2020
    ...
    private func closedCaptionsTrack(for assetTrack: FFmpegAssetTrack) -> FFmpegAssetTrack? {
