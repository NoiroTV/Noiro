...
open class KSPlayerLayer: NSObject {
    ...
    private lazy var timer: Timer = .scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] _ in
        ...
        runOnMainThread { [weak self] in
            guard let self else { return }
            self.delegate?.player(layer: self, currentTime: currentTime, totalTime: totalTime)
        }
    ...
    /// Rebuilds the current media pipeline even when the URL has not changed.
    ///
    /// `set(url:options:)` intentionally treats an identical URL as a request
    /// to resume playback. Stall recovery needs different semantics because
    /// the existing decoder may already have been shut down or deadlocked.
    public func reopenCurrentURL(options: KSOptions) {
        self.options = options
        runOnMainThread {
            self.stop()
            self.player.replace(url: self.url, options: options)
        }
    }
