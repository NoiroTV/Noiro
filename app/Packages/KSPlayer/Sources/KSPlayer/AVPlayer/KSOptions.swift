...
open class KSOptions {
    ...
    public var preferredFrame = KSOptions.preferredFrame
    public var matchDisplayCriteria = true
...
//                }
    ...
    open func updateVideo(refreshRate: Float, isDovi: Bool, formatDescription: CMFormatDescription?) {
        #if os(tvOS) || os(xrOS)
        guard matchDisplayCriteria else {
            return
        }
        /**
         ...
         */
        guard let displayManager = UIApplication.shared.connectedScenes
            .compactMap({ $0 as? UIWindowScene })
            .flatMap({ $0.windows })
            .first?.avDisplayManager,
...
public extension KSOptions {
    #if !os(macOS)
    private static let audioSessionActivationQueue = DispatchQueue(
        label: "app.vortexo.ksplayer.audio-session",
        qos: .userInitiated
    )
    #endif

    static var firstPlayerType: MediaPlayerProtocol.Type = KSMEPlayer.self
    static var secondPlayerType: MediaPlayerProtocol.Type? = KSAVPlayer.self
    /// 最低缓存视频时间
...
//        try? AVAudioSession.sharedInstance().setRouteSharingPolicy(.longFormAudio)
    ...
    #if !os(macOS)
    private static func activateAudioSession(_ session: AVAudioSession) {
        audioSessionActivationQueue.async {
            do {
                try session.setActive(true)
            } catch {
                KSLog("AVAudioSession activation failed: \(error.localizedDescription)")
            }
        }
    }
    #endif
