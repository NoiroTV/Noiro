...
// FFmpeg 8 removed the legacy "native order" channel-layout mask. KSPlayer only exposes this value
// through its compatibility shim and never uses it to describe an actual speaker layout, so zero is the
// correct neutral value when building against MPVKit's newer Libavutil.
