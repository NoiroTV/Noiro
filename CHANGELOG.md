...
- **Catalog rows now match the Vortexo reference project.** Every tvOS catalog rail (Home, Continue Watching, Top Picks, Streaming, Upcoming, and the Collections Hub) is built on one shared `PosterRailShell`, tightened to Vortexo's rhythm (48pt side inset, 40pt between cards) with `scrollClipDisabled` so the focused card and its halo escape the rail band. Poster cards are larger (the 230pt Vortexo baseline, up from 200pt) and carry a rest shadow; when a card has rating/genre/year its title and a "Genre · ★ rating" footer render over the art on a scrim (`CardMetaOverlay`) instead of a title line beneath the card. iOS/Mac rails adopt the same overlay and `scrollClipDisabled`. The on-card overlay is on by default and can be turned off in Appearance (CatalogPreferences.onCardMeta). The Plex-only 4K/DV/HDR/audio badge pills Vortexo shows are out of scope: Stremio-core does not expose stream quality at browse time. Apple TV, iPhone, iPad, and Mac.
...
- **Singularity sources show up.** The toggle used to check the wrong sign-in, so community-corroborated sources never loaded. Sign in to Noiro, turn on Singularity sources in Settings, and they now appear in the source list alongside your own, on iPhone, iPad, Mac, and Apple TV.
...
- **Trailer clips use no server storage.** Trailers and the ambient hero clip resolve on demand from your own device, so nothing is pre-cut or stored on Noiro's side.
...
- **Titles, posters, and logos in your language.** Metadata now localizes across every catalog: a title's name, poster, and logo show in your language when the source has them, drawn from Noiro's own localized-metadata service so it works without your own key.
...
- **Hardened how Noiro fetches remote resources** so a malicious add-on or link cannot make the app reach an unintended internal address (SSRF protection).
...
- **Apple TV: an in-player skip-segment editor.** Mark and submit intro, recap, outro, and preview times right from the Apple TV player; submissions go to Noiro's open skip database and the community SkipDB. Thanks to OrigamiSpace, whose iPhone and Mac editor this is the Apple TV version of.
...
- **A Collections hub on Home and Discover.** A new band high on the screen with three rows: Discover cards (Trending, Popular, Latest, Upcoming), Streaming Services shown with their logos (Netflix, Disney+, Prime, and the services available in your region, including anime, K-drama, and regional ones), and Genres. Tap any tile to browse its catalogs, Movies, Shows, New Movies, New Shows, Top This Week, Month, and Year, and Trending, and every title plays through Noiro as usual. You can reorder the streaming services, turn the hub on or off for Home and Discover separately, and choose how often it refreshes. Needs a TMDB key (Settings, Metadata). It replaces the older grouped streaming and genre rails. On Apple TV, iPhone, iPad, and Mac.
...
- **Self-hosted skip database with an in-player editor.** Skip-intro, recap, and credits times are now fetched first from Noiro's own database, then the open community sources. You can mark and submit segments yourself from a built-in player editor, and a submission goes to Noiro and the open SkipDB at the same time. You can also add an API key for any other compatible provider to contribute there too. Thanks to OrigamiSpace, who created the open SkipDB and contributed the in-player editor this builds on (#98, #100).
...
- **Play straight from a cached debrid link.** If a source is already cached on your debrid account (Real-Debrid, AllDebrid, Premiumize, or TorBox), Noiro now plays it directly from the cached link instead of starting the torrent, and cached sources show a "cached" chip and rank higher in the list. Without a debrid key nothing changes.
...
- **Apple TV: Back returns to Home before it exits.** Pressing Menu on a non-Home tab used to drop you out to the tvOS home screen. It now returns to the Noiro Home tab first, and only exits from Home. Pushed pages still step back one level at a time.
...
- **ERDB posters with no token.** The optional Noiro poster, backdrop, and logo art is now a simple on or off toggle. Our service is keyless, so you no longer need to paste a token; a token is only for a self-hosted setup. It stays opt-in.
...
- **Posters from our own art service.** The optional poster, backdrop, and logo art now comes from Noiro's own service at vortexo.app/api/noiro/v1/edge/posters instead of a third party. It stays opt-in and you set it up the same way.
...
- **Trailers play again on iPhone, iPad, and Mac.** YouTube began requiring a real referrer in July 2025, which silently broke embedded trailers. Noiro now serves the embed so a proper referrer reaches YouTube, and trailers play again. Apple TV was never affected.
...
- **Mac no longer crashes.** The Mac app could quit the instant you switched tabs, resized the window, or changed the language. Several browse screens were each trying to place the Noiro wordmark into the single shared window title bar at once. The wordmark is now iPhone and iPad only; the Mac keeps its native title and the crash is gone. This also unsticks Mac language changes and keyboard navigation, which the crash had been swallowing.
- **Apple TV plays again.** Some streams opened to a black screen with no way out. When the native AVPlayer cannot open a stream, Noiro now hands it straight to the built-in player instead of dead-ending, so playback always starts. You can also force the built-in player from Settings (see Added).
...
- Bringing your add-ons, library, and sources fully into your Noiro account so the app works from your account alone, independent of a live Stremio session.
- More of the in-app player and discovery experience landing on the web and desktop apps.
...
- **Know why a source was picked.** When Noiro auto-picks the recommended source, a short line under Watch now explains the reason in plain terms, for example that it was picked for instant playback from cache or because it matches your preferred source type. (#16)
- **Ratings without setup.** IMDb, Rotten Tomatoes, Metacritic, and more now come straight from Noiro, so there is no key to find or paste. Ratings appear on the detail screen automatically, and they are also baked right onto your posters, on by default, so you can see a title's score at a glance while you browse.
...
- **Live TV no longer shows up empty.** When a live-TV add-on was installed and online, the Live tab could still look empty because its catalogs loaded outside the default Home window. Noiro now reaches for those catalogs so live channels appear as expected. Fixed across iPhone, iPad, Mac, and Apple TV.
...
- **Ratings now come from Noiro, no key needed.** Posters and the detail page show IMDb, Rotten Tomatoes, and Metacritic from Noiro's own ratings service, so you no longer need your own MDBList key. The rating is baked onto your posters out of the box (on by default); you can still point at your own instance or turn it off in Settings.
...
A fix-and-polish beta. The headline is that managing your account from [vortexo.app/studio](https://vortexo.app/studio) now works the way it should: your whole household, your profiles, and almost every per-profile setting are editable from the web and sync straight to your devices. In-place update, nothing resets. This is a beta, so please install it and report anything off.
...
- **One-tap sideload updates.** An AltStore / SideStore source so a sideloaded Noiro updates in place.
...
The big one: a free, end-to-end-encrypted **Noiro account**. Sign in and your profiles and settings follow you between devices, the server only ever holds ciphertext. This build fixes the headline problem from the first beta: your devices now actually sync to each other. Plus in-app add-on management, a catalog manager, optional TMDB-powered recommendations, and a batch of fixes. This is a pre-release for testing; QR sign-in on Apple TV and one-tap Stremio sign-in are coming in 0.3.9.

### Added

- **Noiro account (optional, end-to-end encrypted).** Create an account, sign in, or recover it from Settings; your password derives the encryption key on-device, so the server can never read your data. Your **profiles and settings sync** across devices, pulled automatically each time you open the app. Manage it (backup/restore, two-factor, change password, connect Stremio, library and add-ons) at [vortexo.app/studio](https://vortexo.app/studio).
...
A small release: a multi-file magnet picker, a macOS search fix, and the move to the NoiroTV GitHub organization.
...
- The project moved to the **NoiroTV** GitHub organization. Old links redirect; stars, forks, issues, and releases carried over.

## 0.3.6 - 2026-06-15

The curvy vortex X everywhere, the Noiro gold theme by default, and a macOS custom-server fix.
...
- **Noiro gold is the default accent** for new installs. If you already picked a theme, it stays.
...
- Remaining "StremioX" labels in Settings are now "Noiro".

## 0.3.5 - 2026-06-15

StremioX is now Noiro. This release puts on the new name, a new gold-on-obsidian icon, and an animated Noiro intro, and it adds Backup & Restore so your settings can travel with you. It is an in-place update: your library, add-ons, history, and settings stay exactly as they are. A handful of player and Apple TV fixes ride along too, including a smarter best-stream picker.

### Added

- **The app is now Noiro.** A new name, a new gold-on-obsidian app icon, and an animated Noiro launch screen on iPhone, iPad, Mac, and Apple TV. Same app and same account underneath, so nothing resets.
...
- Next up is Noiro in full: the repository and docs move to the new name, with a website, a subreddit, and a Discord to follow.
