# Noiro Design System (single source of truth)

One design, every platform. This is the canonical spec every Noiro surface builds to: Apple
(tvOS/iOS/iPad/Mac, the reference implementation), Web (`archived browser preview`), Desktop (Tauri), and Android.
...
### Palette (default "noiro" gold theme; the chrome is user-themeable, gold is the shipping default)
| Token | Hex | Use |
|---|---|---|
...
| `accent` | `#D97706` | focus / selection / primary action / progress (Noiro gold) |
...
Themeable accents (chrome only; never change the layout): noiro gold (default), ember `#F27849` (the
...
- **Mark:** two woven gradient ribbons forming an X + a cream center dot. Front ribbon bright gold
  ...
  #7C2D12`); cream dot `#FDF6E3`. Canonical geometry in `noiro-site/src/components/Mark.astro` +
  `SplashView.swift`. The in-app header mark may recolor to the live accent; the splash + app icon keep
  the fixed brand gold.
- **Wordmark:** "Noiro" in the serif face alongside the Noiro mark.
- **Splash:** warm radial obsidian (`#18130c -> #0c0a07`); ember glow blooms, ribbons draw on
  (front delayed), cream dot pops, then "Everything. / Noiroed." rises (Noiro in gold). ~2.8s, lifts
  itself via CSS, once per session, reduced-motion shows static/none. Canonical:
  `noiro-site/src/components/Splash.astro` (port verbatim; web already does).
...
- **Poster card:** 2:3 art (230pt portrait baseline, Vortexo parity), card radius, `rest` shadow
  (`.shadow(black 0.35, radius 8, y: 6)`), title below in label. Hover/focus: lift + scale(~1.03) +
  glow + title brightens to textPrimary. Watched: dim + check badge. In-progress: a 3px accent progress
  track under the art. **On-card metadata (Vortexo parity, default on, `CatalogPreferences.onCardMeta`
  toggle):** when a card has rating/year/genre, the title + a "Genre · ★ rating" footer render OVER the
  art's bottom edge on a darkening scrim (`CardMetaOverlay`) instead of a title line beneath the card;
  the legacy below-card title is suppressed while the overlay is showing. The Plex-only 4K/DV/HDR/audio
  badge pills Vortexo carries are out of scope (Stremio-core has no stream quality at browse time).
...
- **Rails:** each rail = `t-eyebrow` (add-on/source kind) + `t-section` title, then a horizontal poster
  scroll. Board section gaps = `xl`. **Catalog rail rhythm (Vortexo parity):** rails sit on a shared
  `PosterRailShell` — 48pt side inset (`Theme.Space.railHorizontal`), 40pt between cards
  (`Theme.Space.railItemGap`), and `scrollClipDisabled` so the focus halo and the scaled-up focused card
  render outside the band. tvOS Home's hero+strip layout (`kHomeRailStripHeight`) already constrains the
  viewport, so rails stay self-sizing (no fixed frame height). iOS rails keep tighter spacing (viewed at
  arm's length) but adopt `scrollClipDisabled`.
...
## 7. Anti-patterns (the "looks like a prototype / not Noiro" tells)
