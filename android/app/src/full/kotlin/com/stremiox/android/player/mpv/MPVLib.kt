package com.noiro.android.player.mpv

import android.content.Context
import android.view.Surface
import dev.jdtech.mpv.MPVLib as JdtechMPVLib

/// The Noiro libmpv CONTRACT. This is the single surface every Android player phase codes against
...
/// so this wrapper only adapts its instance API to the Noiro contract and keeps the option/config
/// surface in ONE place (see [MpvConfig]).
///
/// Why a wrapper instead of using the artifact class directly:
///   1. It pins the Noiro contract independent of the upstream artifact's exact method shape, so a
///      future artifact bump (or a swap to vendored `.so` + our own JNI) does not ripple into every
///      caller. Only this file changes.
///   2. The artifact's `MPVLib` is a per-instance class created via `MPVLib.create(context)`; Noiro
...
class MPVLib private constructor(private val delegate: JdtechMPVLib) {
    ...
    /// Bridges the artifact's `EventObserver` to the Noiro [EventObserver]. Registered once with the
    /// delegate; fans out to every Noiro observer added via [addObserver]. The five `eventProperty`
