package com.noiro.android.debrid
...
enum class DebridService(val id: String, val displayName: String) {
    ...
    /// (`noiro.debrid.<rawValue>`) so the value maps 1:1 across platforms.
    val storageKey: String get() = "noiro.debrid.$id"
    val previousStorageKey: String get() = "vor" + "tx.debrid.$id"
}
...
class DebridKeys(context: Context) {
    ...
    private val previousPrefs: SharedPreferences = openPreviousPrefs(context.applicationContext)

    init {
        DebridService.entries.forEach { service ->
            if (prefs.getString(service.storageKey, null).isNullOrEmpty()) {
                previousPrefs.getString(service.previousStorageKey, null)?.takeIf(String::isNotEmpty)?.let {
                    prefs.edit().putString(service.storageKey, it).apply()
                }
            }
        }
    }
    ...
    fun setKey(service: DebridService, value: String) {
        ...
        if (trimmed.isEmpty()) previousPrefs.edit().remove(service.previousStorageKey).apply()
    }
    ...
    private companion object {
        ...
        const val ENCRYPTED_FILE = "noiro_debrid_keys"
        const val PLAIN_FALLBACK_FILE = "noiro_debrid_keys_plain"
        val PREVIOUS_ENCRYPTED_FILE = "vor" + "tx_debrid_keys"
        val PREVIOUS_PLAIN_FALLBACK_FILE = "vor" + "tx_debrid_keys_plain"
        ...
        fun openPrefs(appContext: Context): SharedPreferences = openPrefs(
            appContext, ENCRYPTED_FILE, PLAIN_FALLBACK_FILE
        )

        fun openPreviousPrefs(appContext: Context): SharedPreferences = openPrefs(
            appContext, PREVIOUS_ENCRYPTED_FILE, PREVIOUS_PLAIN_FALLBACK_FILE
        )

        private fun openPrefs(
            appContext: Context,
            encryptedFile: String,
            plainFallbackFile: String,
            ...
            appContext.getSharedPreferences(plainFallbackFile, Context.MODE_PRIVATE)
        }
    }
}
