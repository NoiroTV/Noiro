...
android {
    ...
    defaultConfig {
        applicationId = "com.noiro.android"
    ...
    //   - `full`  = the sideloaded Noiro release. Carries libmpv (the GPLv3 mpv/ffmpeg native .so via
    ...
    // update continuity + account migration are unaffected (the com.noiro.android namespace is a
    ...
    productFlavors {
        create("full") {
            ...
            // com.noiro.android id so it updates existing sideloads in place.
        }
        create("play") {
            dimension = "distribution"
        }
...
dependencies {
    ...
    // our thin com.noiro.android.player.mpv.MPVLib wraps it to the Noiro contract, and MpvConfig
    ...
    // event->coroutine bridge in com.noiro.android.engine. org.json (the engine JSON parser used
...
val cargoNdkBuild by tasks.registering(Exec::class) {
    ...
    commandLine(
        buildList {
            ...
            add("ndk")
            addAll(targetFlags)
            // Use the long option: cargo-ndk 3.x accepted `-p`, while 4.x reserves that short
            // option for Cargo package selection. `--platform` is stable across both versions.
            add("--platform"); add(nativeApiLevel.toString())
