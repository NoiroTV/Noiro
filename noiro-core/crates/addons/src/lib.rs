//! # noiro-addons
//!
//! The installed-add-on COLLECTION and the capability ROUTER that sit on top of [`noiro-protocol`].
//!
//! Given the user's installed add-ons, the router answers "which add-ons can serve this request?"
//! by reusing each manifest's [`noiro_protocol::Manifest::supports`] check, in install (priority)
//! order. This is the routing the web client does today (resourceNames / supportsResource /
//! fetchStreams) lifted into pure, testable Rust that every Noiro surface can share.
//!
