...
//! parsed manifest and Noiro-side flags. Install order is priority order, so the first-installed
//! stream add-on is tried first when the router aggregates results.

use serde::{Deserialize, Serialize};
use noiro_protocol::Manifest;

/// Noiro-side flags tracked per installed add-on (mirrors Stremio's collection descriptor flags).
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct AddonFlags {
    /// Shipped or endorsed by Noiro (Cinemeta, the Singularity hub, ...). Surfaced differently and
