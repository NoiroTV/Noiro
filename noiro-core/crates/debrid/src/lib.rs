...
//! -core has no debrid concept at all (debrid only ever arrives through a third-party add-on); Noiro owns
//! it natively. This crate is pure (no HTTP, no async): the trait + normalized types + a per-profile
//! credential format + a resolve-order planner that reuses the [`noiro_hive`] CacheFact vault as a
...
pub use noiro_hive::DebridService;
