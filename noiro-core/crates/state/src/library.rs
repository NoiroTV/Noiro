//! Per-profile library buckets, and the structural fence that keeps Noiro-only data out of the Stremio
//! account library.
//!
...
//! Noiro-specific data; [`LibraryItem::NativeMagnet`] and [`LibraryItem::TorrentPlaylist`] (the native
...
/// `TorrentPlaylist` are Noiro-only and never touch the account library.
...
pub enum LibraryItem {
    ...
    /// A native magnet saved directly to the library (Noiro-only; the #81 wrong-meta fix lives here).
    NativeMagnet {
        id: String,
        name: String,
        infohash: String,
    ...
    /// A user-built playlist of torrent/source entries (Noiro-only).
    TorrentPlaylist {
        id: String,
        name: String,
        #[serde(default)]
...
/// The set of watched video ids for a meta (Noiro's own per-profile watched schema).
...
/// parse. There is intentionally no field for Noiro-specific data, so a projection can never leak it.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
mod tests {
    ...
    fn account_item_serializes_only_stremio_standard_keys() {
        ...
        // No Noiro-specific or per-profile keys can ride into the account library.
        for forbidden in [
            "infohash", "fileIdx", "trackers", "entries", "kind", "noiro", "profile",
        ] {
            assert!(
