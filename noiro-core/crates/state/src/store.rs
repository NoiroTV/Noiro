//! The top-level engine state. `NoiroStore` replaces stremio-core's single `Ctx` + `LibraryBucket`: every
...
mod tests {
    ...
    fn new_store_seeds_owner_active_with_empty_library() {
        let store = NoiroStore::new(owner());
    ...
    fn switch_to_a_second_profile_is_instant_and_creates_its_bucket() {
        let mut store = NoiroStore::new(owner());
        store
            .roster
    ...
    fn store_serde_round_trip() {
        ...
        let back: NoiroStore = serde_json::from_str(&json).unwrap();
