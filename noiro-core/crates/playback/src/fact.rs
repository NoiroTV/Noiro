...
//! A direct sibling of [`noiro_hive::CacheFact`]: it signs a fixed canonical byte string (never serialized
//! JSON) under its own domain prefix, merges by the same LWW+TTL total-order CRDT, and is gated by the
//! same [`noiro_hive::TrustStore`]. It carries only a public title key plus a content digest: no tokens,
//! no account ids, no library contents.

use std::collections::HashMap;

use serde::{Deserialize, Serialize};
use noiro_hive::hive_constants::{ASSETFACT_PREFIX, MAX_CLOCK_SKEW_SECS, PUBLIC_TTL_CAP_SECS};
use noiro_hive::{verify, HiveError, NodeIdentity};
...
mod tests {
    ...
    fn asset_canonical_bytes_use_sentinels_for_absent_season_episode() {
        ...
        let expected = b"noiro-assetfact-v1\ntt1375666|-1|-1|trickplay|0||10|20|K";
        assert_eq!(bytes, expected);
    }
