//! Cross-language conformance for the native `noiro-source/1` addon manifest. Every Noiro implementation
...
use noiro_protocol::parse_manifest;
use noiro_source::{ResourceKind, SourceKind, NoiroAddonManifest, NoiroTransport};

const NATIVE: &str = include_str!("../conformance/native_manifest.json");

#[test]
fn native_manifest_parses_and_round_trips() {
    let m: NoiroAddonManifest = serde_json::from_str(NATIVE).expect("parse native manifest");
    assert_eq!(m.schema, "noiro-source/1");
    assert_eq!(m.kind, SourceKind::NativeNoiro);
    assert!(m.streaming);
    assert!(m.capabilities.contains(&ResourceKind::Ratings));
    assert!(matches!(m.transport, NoiroTransport::StremioHttp { .. }));
    ...
    let back: NoiroAddonManifest = serde_json::from_str(&json).unwrap();
    assert_eq!(m, back);
}
...
fn stremio_addon_lifts_to_native_losslessly() {
    ...
    let m = NoiroAddonManifest::from(&addon);
    ...
    match &m.transport {
        NoiroTransport::StremioHttp { manifest_url } => {
            assert_eq!(
                manifest_url.as_str(),
                "https://torrentio.strem.fun/manifest.json"
            )
