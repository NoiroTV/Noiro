...
use noiro_source::{
    ...
    ResourceKind, ThemeDef, NoiroAddonManifest, NoiroTransport,
};

const THEME_FULL: &str = include_str!("../conformance/theme_full.json");
const TOKEN_KEYS: &str = include_str!("../conformance/token_keys.json");

fn theme_manifest() -> NoiroAddonManifest {
    let mut m = NoiroAddonManifest::native(
        "tv.noiro.theme.midnight",
        "1.0.0",
        "Midnight",
        NoiroTransport::Federated {
            endpoint: "noiro://local".into(),
        },
    );
...
fn theme_full_parses_round_trips_and_validates_clean() {
    let m: NoiroAddonManifest = serde_json::from_str(THEME_FULL).expect("parse theme_full");
...
fn theme_full_canonicalizes_idempotently() {
    let m: NoiroAddonManifest = serde_json::from_str(THEME_FULL).unwrap();
    let once = canonicalize(&m).unwrap();
    let back: NoiroAddonManifest = serde_json::from_slice(&once).unwrap();
