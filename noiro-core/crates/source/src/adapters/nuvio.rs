...
//! and mapping with `noiro_adapters::scraper_streams_to_protocol` (the engine phase); this wrapper exposes
//! the provider's capability + id-space gate.

use noiro_adapters::ScraperInfo;
use noiro_protocol::Stream;
...
impl Source for NuvioProviderSource {
    ...
    fn resolve(&self, _req: &ResourceRequest) -> Result<Vec<Stream>, SourceError> {
        ...
        // is noiro_adapters::scraper_streams_to_protocol.
