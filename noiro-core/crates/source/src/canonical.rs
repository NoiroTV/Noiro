...
use crate::manifest::NoiroAddonManifest;

/// Produce the canonical bytes for a manifest.
pub fn canonicalize(manifest: &NoiroAddonManifest) -> Result<Vec<u8>, serde_json::Error> {
...
mod tests {
    use super::*;
    use crate::NoiroTransport;

    fn manifest() -> NoiroAddonManifest {
        NoiroAddonManifest::native(
    ...
    fn canonical_is_idempotent_through_reparse() {
        ...
        let reparsed: NoiroAddonManifest = serde_json::from_slice(&once).unwrap();
