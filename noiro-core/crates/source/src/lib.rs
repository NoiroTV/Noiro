//! # noiro-source
//!
//! The universal addon system. Every source family, Stremio addons (via `noiro-addons`), Nuvio providers
//! and Eclipse music (via `noiro-adapters`), ratings overlays (XRDB), scrapers, debrid stores, and
//! federated peers, becomes a `kind` behind ONE [`Source`] trait, registered in one [`SourceRegistry`].
//! On top of that sits the NATIVE [`NoiroAddonManifest`] (`noiro-source/1`): a strict superset of the
...
pub use manifest::{
    ...
    NoiroAddonManifest, NoiroTransport, NATIVE_SCHEMA,
};
