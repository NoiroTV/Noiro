...
use noiro_protocol::ResourcePath;

/// The typed resources a source can serve. A superset of the Stremio resources (catalog/meta/stream/
/// subtitles/addon_catalog) plus Noiro-native ones (ratings/artwork/music).
...
impl From<&ResourceRequest> for ResourcePath {
    /// Reuse the byte-exact Stremio transport grammar from `noiro-protocol`.
