...
// Noiro web client entry point. Flow: load installed add-ons (Cinemeta + user stream add-ons) ->
...
function pageTitle(route: Route): string {
  switch (route.name) {
    case "discover":
      return "Discover · Noiro";
    case "search":
      return route.query ? `${route.query} · Search · Noiro` : "Search · Noiro";
    case "library":
      return "Library · Noiro";
    case "addons":
      return "Add-ons · Noiro";
    default:
      return "Noiro";
  }
}
...
async function start(): Promise<void> {
  ...
  // blocks first paint; applySyncDoc fires noiro:addons-changed to re-render when something merges.
  ...
  window.addEventListener("noiro:addons-changed", () => void reloadAddonsAndRender());
  // Switching profile changes which (scoped) library / Continue Watching the views read; re-render the route.
  window.addEventListener("noiro:profile-changed", () => void renderRoute(parseRoute()));
}
