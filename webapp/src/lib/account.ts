 ...
 *  read noiro.profiles, collapse duplicate "main" rows (the duplicate-Main bug), drop deletedProfiles
 *  tombstones unconditionally, and overlay doc.profileEdits ONLY when it is newer than the mirror
 *  (editedAt > noiro.updatedAt) so a stale web edit never masks a newer device change. Returns the slim
 *  roster the web store needs. */
function rosterFromDoc(noiro: Record<string, unknown>, doc: Record<string, unknown>): SyncedProfile[] {
  let roster: SyncedProfile[] = asObjArr(noiro.profiles)
    .filter((p) => p.id != null)
    .map((p, i) => {
  ...
  const tombIds = new Set((Array.isArray(noiro.deletedProfiles) ? noiro.deletedProfiles : []).map((x) => String(x)));
  ...
  if (edits && (Number(edits.editedAt) || 0) > (Number(noiro.updatedAt) || 0) && Array.isArray(edits.roster)) {
...
export function applySyncDoc(doc: Record<string, unknown> | null | undefined): void {
  ...
  const noiro = (doc.noiro && typeof doc.noiro === "object" ? doc.noiro : {}) as Record<string, unknown>;

  // Settings read-down: metadata API keys (doc.apiKeys) + the app/dashboard per-profile appearance,
  // playback and stream-filter settings (doc.noiro.profiles[main].settings / doc.profileEdits). Applied
  ...
  // Add-ons: the app summary (noiro.addons: [{transportUrl,name}]) + the web Stremio import (doc.addons).
  // Membership union first (never drops a local add-on), then apply the synced ORDER (noiro order wins,
  // Cinemeta stays pinned first).
  const urls: string[] = [];
  for (const a of Array.isArray(noiro.addons) ? noiro.addons : []) {
    const u = addonUrl(a);
    if (u) urls.push(u);
  }
  ...
  // this a deleted add-on still in noiro.addons (app channel), a Stremio re-import, or one removed on an
  ...
  // stack's tombstones) + doc.webAddonRemovals (the flat mirror) + doc.noiro.deletedAddons (removals done in
  ...
  if (Array.isArray(noiro.deletedAddons))
    for (const k of noiro.deletedAddons) if (typeof k === "string") removedNorm.add(normalizeAddonUrl(k));
  ...
  // Owner library (noiro.library: [{id,name,type,poster,t,d,lastWatched,v,...}]). The app emits t/d in
  // seconds (the dashboard + now the web app derive Continue Watching from each item's progress).
  const libItems = (Array.isArray(noiro.library) ? noiro.library : []) as Array<Record<string, unknown>>;
  ...
  // noiro.continueWatching the app emits. cwEntriesFrom is shared with the per-profile hydration below.
  let cwChanged = mergeContinueWatching([...cwEntriesFrom(noiro.continueWatching), ...cwEntriesFrom(libItems)]);
  ...
  const rosterChanged = mergeSyncedProfiles(rosterFromDoc(noiro, doc));
  let byProfileChanged = false;
  const byProfile = noiro.byProfile && typeof noiro.byProfile === "object" ? (noiro.byProfile as Record<string, unknown>) : null;
  if (byProfile) {
  ...
  // what you watched on one browser shows on another even before the apps round-trip it into noiro.library.
  ...
  if (typeof window !== "undefined") {
    if (addonsChanged || orderChanged || cwChanged || rosterChanged || byProfileChanged) {
      window.dispatchEvent(new Event("noiro:addons-changed"));
    }
    if (rosterChanged) window.dispatchEvent(new Event("noiro:profile-changed"));
  }
}

/** After sign-in, pull the account's encrypted sync document and apply it locally, so the user's add-ons,
 *  library, and metadata keys come over from their other Noiro devices. Fail-soft: a missing or
...
// doc.addons (the installed add-on list). It NEVER writes doc.noiro.* (app-authoritative). Every write
...
async function pushSettings(session: Session, s: Settings): Promise<void> {
  try {
    await mutateSyncDoc(session, (doc) => {
      ...
      const noiro = (doc.noiro && typeof doc.noiro === "object" ? doc.noiro : {}) as Record<string, unknown>;
      const profiles = (Array.isArray(noiro.profiles) ? noiro.profiles : []) as Record<string, unknown>[];
...
async function pushAddons(
  ...
  try {
    ...
    await mutateSyncDoc(session, (doc) => {
      // doc.addons is the WEB-ONLY sibling channel. Exclude app-owned URLs (doc.noiro.addons): the webapp's
      ...
      const appUrls = new Set(
        (Array.isArray(noiro.addons) ? noiro.addons : []).map((a) => addonUrl(a)).filter(Boolean) as string[],
      );
      ...
      // folds into its uninstall set (NoiroSyncManager: "web agent owns this write; we only READ it").
