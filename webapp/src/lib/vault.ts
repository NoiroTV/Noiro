// Archived legacy login client. It is fail-closed behind an unavailable Noiro
// edge adapter and is not the canonical passwordless Vortexo account service.
//
// ZERO-KNOWLEDGE MODEL
// --------------------
// Email + password + username, end to end encrypted. The password derives the account key on THIS
// device (PBKDF2-SHA256, 210k iterations), so the legacy server adapter and any future self-hosted
// node only ever see:
...
// desktop client, and the website (noiro-site/src/lib/vault.ts), and is verified by the Worker's
...
const API = "https://vortexo.app/api/noiro/v1/edge/legacy-password-account";
const ITERS = 210_000;
...
//     (NoiroSyncCrypto.sealDocument/openDocument) and noiro-site vault.ts byte-for-byte: the sync `document`
...
//     change. MIGRATION: ship dual-READ on EVERY client first (app, noiro-site dashboard, AND this webapp),
...
function sawV2(accountId: string): boolean {
  return localStorage.getItem("noiro.sawV2." + accountId) === "1";
}
function markSawV2(accountId: string): void {
  localStorage.setItem("noiro.sawV2." + accountId, "1");
}
function syncFloor(accountId: string): number {
  return Number(localStorage.getItem("noiro.syncVer." + accountId) || "0");
}
function bumpSyncFloor(accountId: string, version: number): void {
  if (version > syncFloor(accountId)) localStorage.setItem("noiro.syncVer." + accountId, String(version));
}
...
// Vortexo Studio. SESSION_KEY is shared across this origin's web clients and must not change.
const SESSION_KEY = "noiro.session.v1";
const DATAKEY_KEY = "noiro.dk.v1";
 ...
 *  sibling keys (doc.profileEdits, doc.apiKeys, doc.addons) and NEVER doc.noiro.* (app-authoritative). */
export async function mutateSyncDoc(
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    // the whole account at a winning version. Fail loudly instead (mirrors the app + noiro-site).
...
// ephemeral X25519 key agreement, HKDF-SHA256 (salt "noiro-pairing-salt-v1", info "noiro-pairing-v1")
...
const PAIR_SALT = enc("noiro-pairing-salt-v1");
const PAIR_INFO = enc("noiro-pairing-v1");
...
// but with a DISTINCT HKDF salt AND info ("noiro-household-salt-v1" / "noiro-household-v1") so a
...
//                             salt = utf8("noiro-household-salt-v1"),
//                             info = utf8("noiro-household-v1"), L = 32)
...
const HOUSEHOLD_SALT = enc("noiro-household-salt-v1");
const HOUSEHOLD_INFO = enc("noiro-household-v1");
