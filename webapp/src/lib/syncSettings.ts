...
// settings the user sets in a Noiro app or Vortexo Studio show on the web, and vice-versa.
//
// Two doc shapes carry the same values:
//   - doc.noiro.profiles[main].settings        -> app-authored mirror, top-level key `accentID`
...
// than the noiro mirror, matching the roster overlay rule), and we WRITE the dashboard-compatible shape
...
export function mainProfileId(doc: Obj): string | null {
  const noiro = obj(doc.noiro);
  const profiles = Array.isArray(noiro.profiles) ? (noiro.profiles as unknown[]) : [];
...
export function effectiveMainSettings(doc: Obj): Obj {
  ...
  if (!mainId) return {};
  const profiles = (Array.isArray(noiro.profiles) ? noiro.profiles : []) as Obj[];
  const base = obj(profiles.find((p) => obj(p).id != null && String(obj(p).id) === mainId)?.settings);

  let overlay: Obj = {};
  const edits = obj(doc.profileEdits);
  const newer = (num(edits.editedAt) ?? 0) > (num(noiro.updatedAt) ?? 0);
