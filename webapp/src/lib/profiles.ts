...
// locally (localStorage); the account-synced roster (doc.noiro.profiles) + per-profile library + PIN are
...
const KEY = "noiro.web.profiles.v1";
const ACTIVE_KEY = "noiro.web.activeProfile.v1";
...
export function setActiveProfile(id: string): void {
  ...
  if (typeof window !== "undefined") window.dispatchEvent(new Event("noiro:profile-changed"));
}
...
/** A synced profile from the account doc (doc.noiro.profiles, normalized by account.ts): the roster the
