// Archived preview adapter for the rights-review-gated Noiro skip service. The same service the
...
//
// Read contract (mirrors SkipTimestampService in the Apple app): GET /skip?key=<key> where the
...
const SKIP_HOST = "https://vortexo.app/api/noiro/v1/edge/skip";
...
export async function fetchSkipSegments(id: string, season?: number, episode?: number): Promise<SkipSegment[]> {
  ...
  try {
    ...
    // hot path): a stalled service degrades to no segments after SKIP_TIMEOUT_MS rather than the
