...
const STORAGE_KEY = "noiro.web.addons.v1";
...
const LIBRARY_KEY = "noiro.web.library.v1";
 ...
 *  Used by account hydration so a signed-in user's add-ons from their other Noiro devices appear here.
 ...
 *  deleted that still lives in another channel (the app's noiro.addons, a Stremio re-import) would be
...
const HIDDEN_RAILS_KEY = "noiro.web.hiddenRails.v1";
...
const RECENT_KEY = "noiro.web.recent.v1";
const RECENT_MAX = 8;
...
const BACKUP_KEYS = [
  "noiro.web.settings.v1",
  "noiro.web.addons.v1",
  "noiro.web.library.v1",
  "noiro.web.cw.v1",
  "noiro.web.recent.v1",
];
...
export function exportBackup(): string {
  ...
  const settings = data["noiro.web.settings.v1"];
...
function validBackupValue(key: string, val: unknown): boolean {
  switch (key) {
    case "noiro.web.settings.v1":
      return !!val && typeof val === "object" && !Array.isArray(val);
    case "noiro.web.addons.v1":
    ...
    case "noiro.web.recent.v1":
      return Array.isArray(val) && val.every((s) => typeof s === "string");
    case "noiro.web.library.v1":
    case "noiro.web.cw.v1":
