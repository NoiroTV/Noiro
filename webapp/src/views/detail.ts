...
interface DetailState {
  ...
  trailerUrl: string | null; // resolved backend full-trailer mp4 url (rights-reviewed trailer source), null until/if resolved
}
...
export async function openDetail(host: HTMLElement, installed: Addon[], type: string, id: string): Promise<void> {
  ...
  void loadTrailer(meta); // full trailer via rights-reviewed trailer source; repaints when it resolves (fail-soft)
  render();
}

/** Resolve the FULL trailer for this title from the Noiro trailer backend (the same /yt resolver the native
...
function render(): void {
  ...
  document.title = `${state.meta.name} · Noiro`;
  ...
  if (!top && !state.streamsLoading) {
    ...
    const explain = onlyTorrents
      ? `The only sources found are torrents, which the web app can't play on its own (no streaming
         ...
         for instant direct links, or open this title in the Noiro app.`
...
async function playTrailer(): Promise<boolean> {
  if (!state?.meta) return true;
  // PRIMARY: the backend full trailer (progressive mp4 via rights-reviewed trailer source), played in the web player -
