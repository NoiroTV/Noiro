...
function accountSection(): string {
  ...
  if (acct) {
    ...
    return group("Account", body, "Your library, add-ons, and settings sync across every Noiro device, end to end encrypted.");
  }
  const body = `<a class="btn-primary settings-signin" href="#/login">Sign in to Noiro</a>`;
  return group(
    "Account",
    body,
    "Sign in to sync your library, add-ons, and settings across every Noiro device (end to end encrypted). You can keep using the web app signed out, stored only on this device.",
  );
}
...
function streamsSection(s: Settings): string {
  ...
  return group(
    "Streams",
    body,
    "When add-on ranking order is off, Noiro ranks sources by quality. Filters apply to every source list. Source priority and torrent / Usenet filters carry over to your other Noiro devices; torrents are shown here but cannot play in the browser.",
  );
}
...
function aboutSection(): string {
  const body =
    row("Version", `<span class="settings-row-sub">Noiro for Web · 0.1</span>`) +
    row("Player", `<span class="settings-row-sub">hls.js · native &lt;video&gt;</span>`) +
    row("Website", `<a class="inline-link" href="https://vortexo.app" target="_blank" rel="noopener">vortexo.app</a>`);
...
export function handleSettingsClick(target: EventTarget | null): boolean {
  ...
  switch (hit.action) {
    case "set-accent":
      return commit({ accentID: d.accent ?? "noiro" });
    case "set-bg":
